...
class Usage_Tracker {
	...
	private static function normalize_usage_payload( array $usage_data ): array {
		...
		$used = 0;
		foreach ( array( 'used', 'credits_used', 'creditsUsed' ) as $used_key ) {
		...
		$limit = 50;
		foreach ( array( 'limit', 'credits_total', 'creditsTotal', 'creditsLimit', 'total_limit', 'monthly_limit' ) as $limit_key ) {
		...
		$remaining = null;
		foreach ( array( 'remaining', 'credits_remaining', 'creditsRemaining' ) as $remaining_key ) {
	...
	public static function consume_local_monthly_credits( int $count = 1 ): ?array {
		...
		if ( ! in_array( $source, array( 'local_snapshot' ), true ) ) {
			return null;
		}
	 ...
	 * @return array<string,mixed>|null Updated usage payload or null when not applicable.
	 */
	public static function record_local_generation_history_usage( int $used, array $base = array() ): ?array {
		$used = max( 0, (int) $used );
		if ( $used <= 0 ) {
			return null;
		}
		...
		$usage     = array_merge( is_array( $current ) ? $current : array(), $base );
