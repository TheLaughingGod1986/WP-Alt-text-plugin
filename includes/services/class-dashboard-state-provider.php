...
final class Dashboard_State_Provider {
	...
	public static function build( Core $core ): array {
		...
		if ( is_array( $job ) ) {
			...
			$in_progress = in_array( $status, [ 'queued', 'processing' ], true ) && ! empty( $job['active'] );
		...
		$usage = is_callable( [ Usage_Tracker::class, 'get_stats_display' ] ) ? (array) Usage_Tracker::get_stats_display() : [];
