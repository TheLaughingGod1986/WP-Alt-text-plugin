...
class Usage_Logs {
	 ...
	 */
	public static function record_usage_event( $user_id, $tokens_used, $action_type = 'generate', $context = array() ) {
		if ( ! self::table_exists() ) {
			self::create_table();
		} else {
		...
		$allowed_actions = array( 'generate', 'regenerate', 'bulk', 'api', 'bulk-generate' );
	 ...
	 */
	public static function get_usage_events( $filters = array() ) {
		if ( ! self::table_exists() ) {
			return array(
				'events' => array(),
				'total'  => 0,
