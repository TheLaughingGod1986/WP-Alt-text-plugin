...
class Debug_Log {
	 ...
	 * Persist a log entry.
	 */
	public static function log( $level, $message, $context = array(), $source = 'core', $meta = '', $user_id = null ) {
	...
	public static function get_logs( $args = array() ) {
		...
		$logs = array_map( array( self::class, 'format_log' ), $rows );
	...
	public static function get_recent_errors( $limit = 5 ) {
		...
		$result = array_map( array( self::class, 'format_log' ), is_array( $rows ) ? $rows : array() );
		BBAI_Cache::set( 'logs', $cache_suffix, $result, BBAI_Cache::DEFAULT_TTL );
		return $result;
	}
	...
	private static function extract_context_timing_ms( $context ) {
		...
		if ( isset( $context['timing'] ) && is_array( $context['timing'] ) ) {
			foreach ( array( 'ms', 'duration_ms', 'response_time_ms', 'elapsed_ms' ) as $key ) {
	...
	private static function sanitize_context( $context ) {
		...
		$sensitive_keys = array( 'password', 'pass', 'pwd', 'secret', 'token', 'api_key', 'apikey', 'auth', 'authorization', 'jwt', 'bearer' );
	...
	private static function decode_context_data( $context_value ) {
		...
		return is_array( $decoded_context ) ? $decoded_context : array();
	}
