...
class API_Client_V2 {
	 ...
	 */
	private function make_request( $endpoint, $method = 'GET', $data = null, $timeout = null, $include_user_id = false, $extra_headers = array() ) {
		...
		if ( $data && in_array( $method, array( 'POST', 'PUT', 'PATCH' ) ) ) {
			...
			if ( ! preg_match( '/^[{\[]/', $json_body ) ) {
				$this->log_api_event(
					'error',
					'JSON body does not start with valid JSON character',
					array(
						'endpoint'   => $endpoint,
	...
	private function should_retry_api_error( $error ) {
		...
		$retryable_codes = array( 'server_error', 'api_timeout', 'api_unreachable' );
	...
	public function register( $email, $password ) {
		...
		$backend_data = $response['data'] ?? array();
		...
		if ( ! $response['success'] && $status_code >= 400 && $status_code < 500 ) {
			...
			if ( in_array( $normalized_code, array( 'free_plan_exists', 'free_plan_already_used' ), true )
	...
	public function login( $email, $password ) {
		...
		if ( ! $response['success'] && $status_code >= 400 && $status_code < 500 ) {
			...
			if ( in_array( $normalized_code, array( 'site_has_license', 'site_already_connected', 'site_already_linked' ), true )
				|| strpos( $error_message_lower, 'already connected' ) !== false
				|| strpos( $error_message_lower, 'site has license' ) !== false ) {
				return new \WP_Error(
					'site_has_license',
	...
	public function get_user_info() {
		...
		for ( $attempt = 0; $attempt <= $max_retries; $attempt++ ) {
			...
			if ( isset( $response['status'] ) && $response['status'] >= 500 && $attempt < $max_retries ) {
				$this->log_api_event(
					'info',
					'Retrying /auth/me after server error',
					array(
						'attempt' => $attempt + 1,
	...
	public function get_usage() {
		...
		if ( isset( $response['success'] ) && $response['success'] && isset( $backend_response['usage'] ) ) {
			...
			return is_array( $usage ) ? array_merge( $usage, array( 'source' => $usage['source'] ?? 'remote_usage' ) ) : $usage;
		}
	...
	public function get_dashboard_state_truth() {
		...
		if ( is_string( $fixture ) && '' !== trim( $fixture ) ) {
			...
			if ( is_array( $decoded ) && ! empty( $decoded['state'] ) ) {
				$this->log_api_event(
					'info',
					'Using dashboard state truth fixture',
					array(
		...
		foreach ( $candidates as $endpoint ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$this->log_api_event(
					'warning',
					'Dashboard state truth request failed',
					array(
						'endpoint'   => $endpoint,
						'error_code' => $response->get_error_code(),
			...
			$payload          = is_array( $backend_response ) ? ( $backend_response['data'] ?? $backend_response ) : array();
			...
			$this->log_api_event(
				'warning',
				'Dashboard state truth payload invalid',
				array(
					'endpoint'  => $endpoint,
	 ...
	 */
	public function sync_media_inventory_chunk( array $items, array $meta = array() ) {
		if ( empty( $items ) ) {
			return array(
				'skipped' => true,
		...
		$this->log_api_event(
			'info',
			'Dashboard bootstrap inventory sync request prepared',
			array(
		...
		foreach ( $endpoints as $endpoint ) {
			...
			if ( is_wp_error( $response ) ) {
				$this->log_api_event(
					'warning',
					'Dashboard bootstrap inventory sync request failed before response',
					array(
						'endpoint'   => $endpoint,
						'error_code' => $response->get_error_code(),
						'message'    => $response->get_error_message(),
			...
			if ( ! empty( $response['success'] ) ) {
				...
				$this->log_api_event(
					'info',
					'Dashboard bootstrap inventory sync request accepted',
					array(
						'endpoint'        => $endpoint,
			...
			$route_missing = in_array( $status_code, array( 404, 405 ), true )
				|| in_array( $error_code, array( 'NOT_FOUND', 'ROUTE_NOT_FOUND' ), true )
			...
			if ( $route_missing ) {
				$this->log_api_event(
					'warning',
					'Dashboard bootstrap inventory sync endpoint missing',
					array(
						'endpoint'    => $endpoint,
						'status_code' => $status_code,
						'error_code'  => $error_code,
			...
			$this->log_api_event(
				'error',
				'Dashboard bootstrap inventory sync endpoint returned an error',
				array(
					'endpoint'     => $endpoint,
					'status_code'  => $status_code,
					'error_code'   => $error_code,
	 ...
	 */
	private function build_media_inventory_sync_body( array $items, array $meta = array() ): array {
		$site_id     = $this->get_site_id();
		$site_url    = get_site_url();
		...
		if ( ! in_array( $scope, array( 'full_site', 'partial' ), true ) ) {
			$scope = 'full_site';
		}
	 ...
	 */
	private function sync_license_usage_snapshot( $usage, $organization = array(), $site_data = array() ) {
	...
	public function generate_alt_text( $image_id, $context = array(), $regenerate = false ) {
		...
		if ( $this->is_regenerate_debug_enabled() ) {
			\BeepBeepAI\AltTextGenerator\Debug_Log::log(
				'info',
				'API client generation request context',
				array(
					'image_id'   => $image_id,
					'image_url'  => $image_url,
					'filename'   => $filename,
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
			\BeepBeepAI\AltTextGenerator\Debug_Log::log(
				'debug',
				'API Request Payload (BEFORE sending to backend)',
				array(
				...
				if ( is_array( $cached_usage ) && isset( $cached_usage['remaining'] ) && is_numeric( $cached_usage['remaining'] ) && $cached_usage['remaining'] > 0 ) {
					...
					if ( ! is_wp_error( $fresh_usage ) && is_array( $fresh_usage ) && isset( $fresh_usage['remaining'] ) && is_numeric( $fresh_usage['remaining'] ) && $fresh_usage['remaining'] > 0 ) {
						...
						if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
							\BeepBeepAI\AltTextGenerator\Debug_Log::log(
								'warning',
								'Backend returned quota response but cache and fresh API check show credits available',
								array(
		...
		$success_data = isset( $response['data'] ) && is_array( $response['data'] ) ? $response['data'] : array();
	...
	private function sync_usage_after_successful_generation( array $response_data, bool $has_license = false ): void {
		...
		if ( is_array( $usage ) && ! empty( $usage ) ) {
			...
			if ( $has_license ) {
				$this->sync_license_usage_snapshot(
					$usage,
					is_array( $response_data['organization'] ?? null ) ? $response_data['organization'] : array(),
					is_array( $response_data['site'] ?? null ) ? $response_data['site'] : array()
				);
			}
	...
	private function generation_response_has_alt_text( array $response_data ): bool {
		foreach ( array( 'altText', 'alt_text', 'alt' ) as $key ) {
	...
	public function get_plans() {
		...
		for ( $attempt = 0; $attempt <= $max_retries; $attempt++ ) {
			...
			if ( isset( $response['status'] ) && $response['status'] >= 500 && $attempt < $max_retries ) {
				$this->log_api_event(
					'info',
					'Retrying /billing/plans after server error',
					array(
						'attempt' => $attempt + 1,
						'status'  => $response['status'],
					)
	...
	public function create_alt_text_bulk_job( array $job_items ) {
		...
		$response = $this->make_request( 'api/jobs', 'POST', $payload, $timeout, false, array() );
	...
	public function get_alt_text_job_status( string $job_id ) {
		...
		$response = $this->make_request( $endpoint, 'GET', null, $timeout, false, array() );
