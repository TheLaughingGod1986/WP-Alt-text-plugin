...
function bbai_banner_get_banner_state( int $remaining_credits, int $missing_count, int $needs_review_count, int $total_images ): array {
	...
	return array_merge( bbai_banner_get_content( $state, $payload ), array( 'state' => $state ) );
}
...
function bbai_banner_get_banner_state_from_snapshot( array $s, string $page_context = '' ): array {
	...
	return array_merge( bbai_banner_get_content( $state, $merged, $page_context ), array( 'state' => $state ) );
}
...
function bbai_banner_action_scan_manual( string $page_context ): array {
	$attrs = array( 'data-action' => 'rescan-media-library' );
	if ( BBAI_BANNER_CTX_LIBRARY === $page_context ) {
 ...
 * }
 */
function bbai_banner_get_config( string $page_context, array $s, array $opts = array() ): array {
	...
	switch ( $state ) {
		...
		default:
			...
			return array_merge(
				$base,
				array(
					'title'            => __( 'OpptiAI', 'beepbeep-ai-alt-text-generator' ),
					'body'             => '',
					'primary_action'   => bbai_banner_action_scan( $page_context ),
 ...
 * @return array<string, mixed>
 */
function bbai_banner_build_command_hero( string $page_context, array $s, array $opts = array() ): array {
	...
	if ( BBAI_BANNER_CTX_LIBRARY === $page_context ) {
		if ( null !== $primary_a ) {
			$primary_a['attributes'] = bbai_banner_merge_library_attrs( $primary_a['attributes'] ?? array() );
		}
		if ( null !== $secondary_a ) {
			$secondary_a['attributes'] = bbai_banner_merge_library_attrs( $secondary_a['attributes'] ?? array() );
		}
	}
	...
	$section_data                                   = isset( $opts['section_data_attrs'] ) && is_array( $opts['section_data_attrs'] ) ? $opts['section_data_attrs'] : array();
	...
	$hero = array(
		...
		'icon_wrapper_attrs'      => isset( $opts['icon_wrapper_attrs'] ) && is_array( $opts['icon_wrapper_attrs'] ) ? $opts['icon_wrapper_attrs'] : array(),
		'headline_attrs'          => isset( $opts['headline_attrs'] ) && is_array( $opts['headline_attrs'] ) ? $opts['headline_attrs'] : array(),
		'subtext_attrs'           => isset( $opts['subtext_attrs'] ) && is_array( $opts['subtext_attrs'] ) ? $opts['subtext_attrs'] : array(),
...
function bbai_banner_inline_payload_to_command_hero( array $ib ): array {
	...
	if ( '' !== $variant && in_array( $variant, array( 'success', 'warning', 'info', 'neutral' ), true ) ) {
	...
	if ( ! in_array( $bv, array( 'success', 'warning', 'info', 'neutral' ), true ) ) {
		$bv                   = 'neutral';
	...
	$ch['heading_tag'] = in_array( $ht, array( 'h1', 'h2' ), true ) ? $ht : 'h1';
...
function bbai_banner_library_surface_attr_opts(): array {
	return array(
		'icon_wrapper_attrs' => array(
			'data-bbai-library-surface-icon' => '1',
		),
		'headline_attrs'     => array(
