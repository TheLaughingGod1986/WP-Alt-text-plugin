...
function bbai_plan_top_banner_resolve( array $input ): array {
	...
	$view_plans_secondary_guest = array(
 ...
 */
function bbai_plan_top_banner_build_command_hero( string $page_context, array $input, array $opts = array() ): array {
	...
	if ( ! empty( $opts['heading_tag'] ) ) {
		...
		if ( in_array( $ht, array( 'h1', 'h2' ), true ) ) {
			$hero['heading_tag'] = $ht;
		}
	}
