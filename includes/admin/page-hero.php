...
function bbai_page_hero_resolve_variant( string $tone, string $semantic_state, string $banner_variant, string $explicit = '' ): string {
	...
	if ( in_array( $explicit, array( 'success', 'warning', 'neutral', 'info' ), true ) ) {
		return $explicit;
	}

	if ( in_array( $semantic_state, array( 'empty', 'first_scan', 'usage_billing' ), true ) ) {
		return 'neutral';
	}
...
function bbai_page_hero_library_command_hero( array $args ): array {
	...
	foreach ( array( 'missing_library_url', 'needs_review_library_url' ) as $url_key ) {
 ...
 * @return array<string, mixed>
 */
function bbai_page_hero_usage_command_hero( array $args = array() ): array {
	require_once BEEPBEEP_AI_PLUGIN_DIR . 'includes/admin/banner-system.php';
	...
	$reserved = array( 'billing_portal_url', 'upgrade_url', 'overview_anchor', 'wrapper_extra_class', 'section_data_attrs' );
	...
	$hero                         = bbai_banner_build_command_hero(
		...
		array(
			...
			'section_data_attrs'  => array_merge(
				array( 'data-bbai-usage-page-hero' => '1' ),
				isset( $args['section_data_attrs'] ) && is_array( $args['section_data_attrs'] ) ? $args['section_data_attrs'] : array()
			),
		)
	);
