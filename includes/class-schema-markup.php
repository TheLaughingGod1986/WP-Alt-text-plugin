...
class BBAI_Schema_Markup {
	...
	public static function init() {
		...
		add_filter( 'wp_get_attachment_image_attributes', array( __CLASS__, 'add_image_attributes' ), 10, 3 );

		// Output schema JSON-LD in head (standard location for structured data)
		add_action( 'wp_head', array( __CLASS__, 'output_schema_json_ld' ), 99 );
	 ...
	 * @var array
	 */
	private static $images_for_schema = array();
	...
	public static function add_image_attributes( $attr, $attachment, $size ) {
		...
		self::$images_for_schema[ $attachment->ID ] = array(
			'url'         => $image_url[0],
			'width'       => $image_url[1],
			'height'      => $image_url[2],
			'alt'         => $alt_text,
