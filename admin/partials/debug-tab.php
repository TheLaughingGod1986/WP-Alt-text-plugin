...
<?php else : ?>
	<?php
		$bbai_debug_logs       = $debug_bootstrap['logs'] ?? array();
		$bbai_debug_stats      = $debug_bootstrap['stats'] ?? array();
		$bbai_debug_pagination = $debug_bootstrap['pagination'] ?? array();
		$bbai_system_status    = $debug_bootstrap['system_status'] ?? array();
		$bbai_service_status   = $debug_bootstrap['service_status'] ?? array();
		$bbai_recent_errors    = $debug_bootstrap['recent_errors'] ?? array();
		$bbai_copy_debug_info  = $debug_bootstrap['copy_debug_info'] ?? array();
	...
	<div class="bbai-debug-page" data-bbai-debug-panel>
		...
		<section class="bbai-card bbai-mb-6" data-debug-recent-errors-section 
		<?php
		if ( empty( $bbai_recent_errors ) ) :
			?>
			hidden<?php endif; ?>>
						...
						<?php if ( ! empty( $bbai_recent_errors ) ) : ?>
							<?php foreach ( $bbai_recent_errors as $bbai_recent_error ) : ?>
								<?php
									...
									$bbai_recent_context         = $bbai_recent_error['context'] ?? array();
