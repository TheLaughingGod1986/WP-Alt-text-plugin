 ...
 * Modernized to match OpptiAI SaaS style with unified.css components
 */
...
$bbai_workspace_status_map   = array(
	'missing'      => 'missing',
	'optimized'    => 'optimized',
	'weak'         => 'weak',
	'needs_review' => 'weak',
...
if ( $bbai_use_library_workspace ) {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$bbai_library_full_images = $wpdb->get_results(
		$wpdb->prepare(
	...
	$bbai_library_normalized_rows = array();
	...
	if ( false === $bbai_all_images ) {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$bbai_all_images = $wpdb->get_results(
			$wpdb->prepare(
...
$bbai_coverage = isset( $this ) && method_exists( $this, 'get_alt_text_coverage_scan' ) ? $this->get_alt_text_coverage_scan( true ) : array();
...
<div class="bbai-library-container bbai-container" data-bbai-empty-filter="<?php echo esc_attr__( 'No images match this filter.', 'beepbeep-ai-alt-text-generator' ); ?>" data-bbai-empty-filter-hint="<?php echo esc_attr__( 'Try another filter or show all images.', 'beepbeep-ai-alt-text-generator' ); ?>" data-bbai-library-is-pro="<?php echo $bbai_is_pro ? '1' : '0'; ?>" data-bbai-settings-url="<?php echo esc_url( admin_url( 'admin.php?page=bbai-settings' ) ); ?>">
	...
	$bbai_trial_status = $bbai_is_guest_trial ? Trial_Quota::get_status() : array();
	...
	$bbai_is_premium          = in_array( $bbai_plan, array( 'pro', 'growth', 'agency' ), true );
	...
	$bbai_stats               = isset( $bbai_stats ) ? $bbai_stats : array(
		'total'    => $bbai_total_images,
		'with_alt' => $bbai_with_alt_count,
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		...
		error_log(
			'[BBAI counts] library_source ' . wp_json_encode(
				array(
	...
	<div id="bbai-alt-coverage-card"
		...
		<div class="bbai-library-summary-card__head">
			<div>
				...
				<h2 class="bbai-library-summary-card__title"><span data-bbai-library-progress-label>
				<?php
				echo esc_html(
					sprintf(
		...
		<div class="bbai-library-summary-card__foot">
			...
			<strong data-bbai-coverage-score-inline>
			<?php
			echo esc_html(
				sprintf(
				/* translators: %s: optimization percentage. */
	...
	bbai_ui_render( 'bbai-banner', array( 'command_hero' => $bbai_command_hero ) );
	?>

	<?php
	$bbai_library_filter_items = array(
		array(
			'key'               => 'all',
					...
					<tbody id="bbai-library-table-body">
						<?php foreach ( $bbai_all_images as $bbai_image ) : ?>
							...
							if ( $bbai_has_alt ) {
								...
								if ( in_array( $bbai_quality_class, array( 'excellent', 'good' ), true )
							...
							<tr class="bbai-library-row"
								...
								if ( $bbai_breakdown ) :
									?>
									data-quality-breakdown="<?php echo esc_attr( wp_json_encode( $bbai_breakdown ) ); ?>"<?php endif; ?>
								<?php
								if ( ! empty( $bbai_suggestions ) ) :
									?>
									data-quality-suggestions="<?php echo esc_attr( wp_json_encode( array_slice( $bbai_suggestions, 0, 4 ) ) ); ?>"<?php endif; ?>
								<?php
								if ( ! empty( $bbai_analysis['issues'] ) ) :
									?>
									data-quality-issues="<?php echo esc_attr( wp_json_encode( array_slice( $bbai_analysis['issues'], 0, 5 ) ) ); ?>"<?php endif; ?>>
			...
			<div class="bbai-card bbai-text-center bbai-p-12">
				...
				bbai_ui_render(
					...
					array(
						...
						'body'         => __( 'Upload images to your media library and OpptiAI will generate alt text here.', 'beepbeep-ai-alt-text-generator' ),
