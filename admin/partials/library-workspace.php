...
if ( ! in_array( $bbai_default_review_filter, array( 'all', 'missing', 'optimized', 'weak' ), true ) ) {
	$bbai_default_review_filter = 'all';
}
...
$bbai_command_hero = bbai_page_hero_library_command_hero(
	array(
		...
		'library_url'              => add_query_arg( array( 'page' => 'bbai-library' ), admin_url( 'admin.php' ) ),
		'missing_library_url'      => add_query_arg(
			array(
				'page'   => 'bbai-library',
				'status' => 'missing',
			),
...
if ( ! isset( $bbai_command_hero['section_data_attrs'] ) || ! is_array( $bbai_command_hero['section_data_attrs'] ) ) {
	$bbai_command_hero['section_data_attrs'] = array();
}
...
if ( ! isset( $bbai_library_row_states ) || ! is_array( $bbai_library_row_states ) ) {
	...
	$bbai_library_images_for_table = isset( $bbai_all_images ) && is_array( $bbai_all_images ) ? $bbai_all_images : array();
...
if ( ! isset( $bbai_table_filter_counts ) || ! is_array( $bbai_table_filter_counts ) ) {
	$bbai_table_filter_counts = array(
...
foreach ( $bbai_library_row_states as $bbai_rs_hint ) {
	if ( ! empty( $bbai_rs_hint['status'] ) && in_array( $bbai_rs_hint['status'], array( 'missing', 'weak' ), true ) ) {
		++$bbai_page_missing_or_weak_count;
	}
}

// Status filters: All → Missing → Needs review → Optimized (data-filter weak = needs review queue).
$bbai_library_workspace_filter_items = array(
	array(
		'key'               => 'all',
...
<div
	...
	data-bbai-library-url="<?php echo esc_url( add_query_arg( array( 'page' => 'bbai-library' ), admin_url( 'admin.php' ) ) ); ?>"
	data-bbai-missing-library-url="
	<?php
	echo esc_url(
		add_query_arg(
			array(
...
>
	...
	<div
		...
		data-bbai-free-plan-limit="<?php echo esc_attr( (int) ( ( $bbai_coverage ?? array() )['free_plan_limit'] ?? 50 ) ); ?>"
	>
		<?php bbai_ui_render( 'bbai-banner', array( 'command_hero' => $bbai_command_hero ) ); ?>
		...
		<div class="bbai-sr-only" aria-hidden="true">
			...
			<span data-bbai-coverage-score-inline>
			<?php
			echo esc_html(
				sprintf(
			...
			<?php if ( ! empty( $bbai_all_images ) ) : ?>
				<div class="bbai-library-selection-bar" id="bbai-library-selection-bar" aria-live="polite" hidden>
					...
					<div class="bbai-library-selection-bar__actions">
						<button type="button" class="bbai-btn bbai-btn-primary bbai-btn-sm<?php echo $bbai_limit_reached_state ? ' bbai-is-locked' : ''; ?>" id="bbai-batch-generate" data-action="generate-selected" data-bbai-lock-preserve-label="1"
						<?php
						if ( $bbai_limit_reached_state ) :
							?>
							data-bbai-action="<?php echo esc_attr( $bbai_locked_quota_action ); ?>" data-bbai-locked-cta="1" data-bbai-lock-reason="generate_missing" data-bbai-locked-source="library-selection-bar-generate" aria-disabled="true"<?php echo $bbai_locked_auth_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static escaped fragment. ?><?php endif; ?>><?php echo esc_html( bbai_copy_cta_generate_alt() ); ?></button>
						<button type="button" class="bbai-btn bbai-btn-secondary bbai-btn-sm<?php echo $bbai_limit_reached_state ? ' bbai-is-locked' : ''; ?>" id="bbai-batch-regenerate" data-action="regenerate-selected" data-bbai-lock-preserve-label="1"
						<?php
						if ( $bbai_limit_reached_state ) :
							?>
							data-bbai-action="<?php echo esc_attr( $bbai_locked_quota_action ); ?>" data-bbai-locked-cta="1" data-bbai-lock-reason="reoptimize_all" data-bbai-locked-source="library-selection-bar-regenerate" aria-disabled="true"<?php echo $bbai_locked_auth_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static escaped fragment. ?><?php endif; ?>><?php esc_html_e( 'Regenerate ALT', 'beepbeep-ai-alt-text-generator' ); ?></button>
					...
					<div class="bbai-table-wrap bbai-table__scroll">
						<div id="bbai-library-table-body" class="bbai-library-review-queue bbai-table__body" role="list">
								<?php foreach ( $bbai_all_images as $bbai_row_idx => $bbai_image ) : ?>
									...
									$bbai_ai_source       = in_array( $bbai_ai_source_raw, array( 'ai', 'openai' ), true ) ? 'ai' : '';
									...
									<article class="bbai-library-row bbai-library-review-card bbai-row"
										...
										<div class="bbai-library-card__select">
											<input type="checkbox" class="bbai-checkbox bbai-library-row-check bbai-image-checkbox" value="<?php echo esc_attr( $bbai_attachment_id ); ?>" data-attachment-id="<?php echo esc_attr( $bbai_attachment_id ); ?>" aria-label="
											<?php
											printf(
													...
													<div class="bbai-library-card__tool-row bbai-library-card__action-cluster bbai-row-actions">
														<div class="bbai-library-card__quick-actions" aria-label="<?php esc_attr_e( 'Row actions', 'beepbeep-ai-alt-text-generator' ); ?>">
															<button type="button"
																...
																<?php
																if ( 'regenerate-single' === $bbai_q1_action ) :
																	?>
																	data-bbai-lock-preserve-label="1"<?php endif; ?>
																<?php
																if ( $bbai_limit_reached_state && 'regenerate-single' === $bbai_q1_action ) :
																	?>
																	data-bbai-action="<?php echo esc_attr( $bbai_locked_quota_action ); ?>" data-bbai-locked-cta="1" data-bbai-lock-reason="regenerate_single" data-bbai-locked-source="library-row-regenerate-quick" aria-disabled="true"<?php echo $bbai_locked_auth_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static escaped fragment. ?><?php endif; ?>>
															...
															<button type="button"
																...
																<?php
																if ( 'regenerate-single' === $bbai_q2_action ) :
																	?>
																	data-bbai-lock-preserve-label="1"<?php endif; ?>
																<?php
																if ( $bbai_limit_reached_state && 'regenerate-single' === $bbai_q2_action ) :
																	?>
																	data-bbai-action="<?php echo esc_attr( $bbai_locked_quota_action ); ?>" data-bbai-locked-cta="1" data-bbai-lock-reason="regenerate_single" data-bbai-locked-source="library-row-regenerate-quick" aria-disabled="true"<?php echo $bbai_locked_auth_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static escaped fragment. ?><?php endif; ?>>
