...
$bbai_plan_slug = sanitize_key((string) ($bbai_plan_data['plan_slug'] ?? 'free'));
$bbai_is_free = !empty($bbai_plan_data['is_free']);
$bbai_is_growth = !empty($bbai_plan_data['is_growth']);
$bbai_analytics_is_locked = !in_array($bbai_plan_slug, ['starter', 'pro', 'growth', 'agency', 'enterprise'], true);
...
$bbai_before_after_story_description = 0 === $bbai_total_images
    ? __('This section becomes a proof-of-value snapshot once OpptiAI Alt Text starts optimizing your library.', 'beepbeep-ai-alt-text-generator')
...
<?php
if ($bbai_analytics_is_locked) :
?>
<div class="bbai-paywall-preview bbai-paywall-preview--analytics">
...
?>
<?php if ($bbai_analytics_is_locked) : ?>
    </div>
    <div class="bbai-paywall-preview__overlay" role="region" aria-label="<?php esc_attr_e('Analytics upgrade prompt', 'beepbeep-ai-alt-text-generator'); ?>">
        <span class="bbai-paywall-preview__eyebrow"><?php esc_html_e('Growth feature', 'beepbeep-ai-alt-text-generator'); ?></span>
        <h2><?php esc_html_e('Unlock Analytics', 'beepbeep-ai-alt-text-generator'); ?></h2>
        <p><?php esc_html_e('Upgrade to see coverage trends, optimisation performance, and credit insights across your image library.', 'beepbeep-ai-alt-text-generator'); ?></p>
        <div class="bbai-paywall-preview__actions">
            ...
            <a class="bbai-btn bbai-btn-secondary" href="<?php echo esc_url(admin_url('admin.php?page=bbai')); ?>"><?php esc_html_e('Back to dashboard', 'beepbeep-ai-alt-text-generator'); ?></a>
        </div>
    </div>
</div>
<?php endif; ?>
