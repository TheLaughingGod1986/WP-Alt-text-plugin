...
$bbai_stats       = isset( $bbai_stats ) && is_array( $bbai_stats ) ? $bbai_stats : array();
$bbai_usage_stats = isset( $bbai_usage_stats ) && is_array( $bbai_usage_stats ) ? $bbai_usage_stats : array();
...
$bbai_historical_coverage = array(
	array(
		'date'     => 'Week 1',
		'coverage' => max( 0, $bbai_coverage_percent - 24 ),
	),
	array(
		'date'     => 'Week 2',
		'coverage' => max( 0, $bbai_coverage_percent - 20 ),
	),
	array(
		'date'     => 'Week 3',
		'coverage' => max( 0, $bbai_coverage_percent - 16 ),
	),
	array(
		'date'     => 'Week 4',
		'coverage' => max( 0, $bbai_coverage_percent - 12 ),
	),
	array(
		'date'     => 'Week 5',
		'coverage' => max( 0, $bbai_coverage_percent - 8 ),
	),
	array(
		'date'     => 'Current',
		'coverage' => max( 0, $bbai_coverage_percent - 4 ),
	),
	array(
		'date'     => 'Week 6',
		'coverage' => max( 0, $bbai_coverage_percent ),
	),
);
...
$bbai_make_button_action = static function ( string $label, string $variant, array $attributes = array(), string $class = '' ): array {
	return array(
		'label'      => $label,
		'variant'    => $variant,
		'size'       => 'sm',
...
$bbai_make_link_action = static function ( string $label, string $href, string $variant = 'secondary', array $attributes = array(), string $class = '' ): array {
	return array(
		'label'      => $label,
		'href'       => $href,
		'variant'    => $variant,
...
$bbai_render_action = static function ( ?array $action ): string {
	...
	$attribute_pairs = array( 'class="' . esc_attr( trim( implode( ' ', array_filter( $classes ) ) ) ) . '"' );
...
$bbai_before_after_actions = array(
	$bbai_make_button_action( __( 'Download report', 'beepbeep-ai-alt-text-generator' ), 'secondary', array( 'id' => 'bbai-export-report' ) ),
...
if ( BBAI_BANNER_STATE_HEALTHY === $bbai_analytics_banner_logic ) {
	...
	array( BBAI_BANNER_STATE_NEEDS_ATTENTION, BBAI_BANNER_STATE_LOW_CREDITS, BBAI_BANNER_STATE_OUT_OF_CREDITS ),
	true
) ) {
...
if ( ! isset( $bbai_analytics_command_hero['section_data_attrs'] ) || ! is_array( $bbai_analytics_command_hero['section_data_attrs'] ) ) {
	$bbai_analytics_command_hero['section_data_attrs'] = array();
}
...
bbai_ui_render( 'metrics-grid', array( 'phase' => 'close' ) );
?>
...
bbai_ui_render( 'workspace-grid', array( 'phase' => 'main_open' ) );
bbai_ui_render(
	'section-card',
	array(
		'phase' => 'open',
...
bbai_ui_render( 'workspace-grid', array( 'phase' => 'main_close' ) );
bbai_ui_render(
	'sidebar-card',
	array(
...
bbai_ui_render( 'workspace-grid', array( 'phase' => 'close' ) );
bbai_ui_render( 'page-shell', array( 'phase' => 'close' ) );
?>
