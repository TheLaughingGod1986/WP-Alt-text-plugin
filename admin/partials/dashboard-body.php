<?php
/**
 * Dashboard shell: resolves guest (no connected SaaS account) vs connected account output.
 *
 * No connected account: only guest funnel (dashboard-hero.php + optional exhausted library block). No logged-in page.
...
if ($bbai_has_connected_account || $bbai_is_guest_trial) :
    ...
    $bbai_plan_slug_ladder = $bbai_has_connected_account
        ? strtolower( (string) ( $bbai_usage_stats['plan'] ?? $bbai_usage_stats['plan_type'] ?? ( $bbai_plan_data['plan_slug'] ?? 'free' ) ) )
        : 'free';
    if ( 'growth' === $bbai_plan_slug_ladder ) {
        $bbai_plan_slug_ladder = 'pro';
    }
    $bbai_is_agency = !$bbai_is_guest_trial && !empty($bbai_plan_data['is_agency']);
    $bbai_usage_can_autopilot = ! empty( $bbai_usage_stats['can_autopilot'] );
    $bbai_is_premium = !$bbai_is_guest_trial && (!empty($bbai_plan_data['is_pro']) || $bbai_is_agency || in_array($bbai_plan_slug_ladder, ['pro', 'agency'], true) || $bbai_usage_can_autopilot);
    ...
    $bbai_dashboard_state = [
        ...
        'optimizedLibraryUrl' => $bbai_optimized_library_url,
	...
	$bbai_guest_preview_actionable_count = max(
		$bbai_dashboard_root_missing_count,
    ...
    <div
        ...
        data-bbai-optimized-library-url="<?php echo esc_url( $bbai_optimized_library_url ); ?>"
