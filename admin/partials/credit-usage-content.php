...
$bbai_limit         = max(1, (int) ($bbai_usage_surface['creditsLimit'] ?? ($current_usage['limit'] ?? 25)));
...
$bbai_usage_plan_slug     = sanitize_key((string) ($current_usage['plan'] ?? ($bbai_usage_surface['planSlug'] ?? 'free')));
$bbai_usage_is_locked    = !in_array($bbai_usage_plan_slug, ['starter', 'pro', 'growth', 'agency', 'enterprise'], true);
...
<?php if ($bbai_usage_is_locked) : ?>
<div class="bbai-paywall-preview bbai-paywall-preview--usage">
...
<?php if ($bbai_usage_is_locked) : ?>
    ...
    <div class="bbai-paywall-preview__overlay" role="region" aria-label="<?php esc_attr_e('Usage upgrade prompt', 'beepbeep-ai-alt-text-generator'); ?>">
        <span class="bbai-paywall-preview__eyebrow"><?php esc_html_e('Growth feature', 'beepbeep-ai-alt-text-generator'); ?></span>
        <h2><?php esc_html_e('Unlock Usage insights', 'beepbeep-ai-alt-text-generator'); ?></h2>
        <p><?php esc_html_e('Upgrade to monitor credit usage, activity history, billing cycles, and team contributors from one place.', 'beepbeep-ai-alt-text-generator'); ?></p>
