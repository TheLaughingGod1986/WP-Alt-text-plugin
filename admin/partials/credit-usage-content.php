...
?>
<?php
$bbai_usage_surface            = isset( $usage_surface ) && is_array( $usage_surface ) ? $usage_surface : array();
$bbai_user_details             = isset( $user_details ) && is_array( $user_details ) ? $user_details : null;
$bbai_backend_user_activity    = isset( $backend_user_activity ) && is_array( $backend_user_activity ) ? $backend_user_activity : array();
...
$bbai_usage_activity_items     = isset( $usage_activity['items'] ) && is_array( $usage_activity['items'] ) ? $usage_activity['items'] : array();
...
$bbai_usage_insights = isset( $usage_insights ) && is_array( $usage_insights ) ? $usage_insights : array();
...
if ( ! in_array( $bbai_ins_tone, array( 'healthy', 'warning', 'danger' ), true ) ) {
	$bbai_ins_tone = 'healthy';
}
$bbai_ins_chips = isset( $bbai_usage_insights['chips'] ) && is_array( $bbai_usage_insights['chips'] ) ? $bbai_usage_insights['chips'] : array();
?>
				...
				<div class="bbai-credit-filter-row">
					...
					<div class="bbai-credit-filter-field">
						...
						<select id="bbai-source-filter" name="source" class="bbai-credit-filter-select bbai-select">
							<?php foreach ( ( $source_options ?? array() ) as $bbai_source_value => $bbai_source_label ) : ?>
