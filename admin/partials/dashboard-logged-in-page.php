...
$bbai_daily_ring_label = 'complete' === $bbai_daily_focus
	? __( 'Optimised', 'beepbeep-ai-alt-text-generator' )
	: ( 'missing' === $bbai_daily_focus ? __( 'Missing ALT', 'beepbeep-ai-alt-text-generator' ) : __( 'Needs review', 'beepbeep-ai-alt-text-generator' ) );
...
$bbai_daily_optimized_library_url = isset( $bbai_optimized_library_url )
	? $bbai_daily_anchor_url( (string) $bbai_optimized_library_url )
	: add_query_arg(
		[
			'page'     => 'bbai-library',
			'status'   => 'optimized',
...
$bbai_daily_segment_pct = static function ( int $count ) use ( $bbai_daily_total ): string {
	return $bbai_daily_total > 0 ? (string) round( min( 100, max( 0, ( 100 * $count ) / $bbai_daily_total ) ), 4 ) : '0';
};
$bbai_daily_optimised_angle = min( 360, (float) $bbai_daily_segment_pct( $bbai_daily_optimised ) * 3.6 );
$bbai_daily_review_angle    = min( 360, $bbai_daily_optimised_angle + ( (float) $bbai_daily_segment_pct( $bbai_daily_review ) * 3.6 ) );
$bbai_daily_donut_background = $bbai_daily_total > 0
	? sprintf(
		'conic-gradient(#16a34a 0deg %.4Fdeg, #dc2626 %.4Fdeg %.4Fdeg, #d97706 %.4Fdeg 360deg)',
		$bbai_daily_optimised_angle,
		$bbai_daily_optimised_angle,
		$bbai_daily_review_angle,
		$bbai_daily_review_angle
	)
	: 'conic-gradient(#e2e8f0 0deg 360deg)';
$bbai_daily_upload_url  = admin_url( 'upload.php' );
$bbai_daily_settings_autopilot_url = admin_url( 'admin.php?page=bbai-settings#bbai-enable-on-upload' );
$bbai_daily_live_can_autopilot = ! empty( $bbai_usage_can_autopilot )
	|| ! empty( $bbai_usage_stats['can_autopilot'] )
	|| ! empty( $bbai_li_state['usage']['can_autopilot'] )
	|| ! empty( $bbai_li_state['entitlement']['can_autopilot'] );
$bbai_daily_live_quota_limit = max(
	0,
	(int) ( $bbai_usage_stats['token_limit'] ?? 0 ),
	(int) ( $bbai_usage_stats['limit'] ?? 0 ),
	(int) ( $bbai_usage_stats['credits_total'] ?? 0 ),
	(int) ( $bbai_dashboard_root_credits_total ?? 0 ),
	(int) ( $bbai_li_state['usage']['limit'] ?? 0 ),
	(int) ( $bbai_li_state['usage']['credits_total'] ?? 0 )
);
$bbai_daily_is_pro_plan = ! empty( $bbai_state_is_pro_plan )
	|| ! empty( $bbai_is_premium )
	|| $bbai_daily_live_can_autopilot
	|| $bbai_daily_live_quota_limit >= 1000
	|| in_array( strtolower( (string) ( $bbai_plan_slug_ladder ?? '' ) ), [ 'pro', 'agency' ], true );
$bbai_daily_autopilot_action = $bbai_daily_is_pro_plan
	? [
		'label'  => __( 'Enable Autopilot', 'beepbeep-ai-alt-text-generator' ),
		'href'   => $bbai_daily_settings_autopilot_url,
		'action' => 'navigate',
	]
	: [
		'label'  => __( 'Enable Autopilot', 'beepbeep-ai-alt-text-generator' ),
...
$bbai_daily_review_phrase = sprintf(
	...
	_n( '%s image needs review', '%s images need review', $bbai_daily_review, 'beepbeep-ai-alt-text-generator' ),
...
$bbai_daily_primary = $bbai_daily_complete
	...
	: [
		...
		'busy_label'  => $bbai_daily_missing > 0 ? __( 'Generating ALT text…', 'beepbeep-ai-alt-text-generator' ) : '',
	];

if ( $bbai_daily_missing > 0 && $bbai_daily_remaining <= 0 ) {
	$bbai_daily_primary = [
		'label'  => __( 'Upgrade to continue generating', 'beepbeep-ai-alt-text-generator' ),
		'href'   => '#',
		'action' => 'show-upgrade-modal',
	];
}
...
$bbai_daily_upgrade = [
	'label'  => $bbai_daily_is_pro_plan ? __( 'Manage Autopilot', 'beepbeep-ai-alt-text-generator' ) : __( 'Upgrade plan', 'beepbeep-ai-alt-text-generator' ),
	'href'   => $bbai_daily_is_pro_plan ? $bbai_daily_settings_autopilot_url : '#',
	'action' => $bbai_daily_is_pro_plan ? 'navigate' : 'show-upgrade-modal',
];
...
>
	...
	<div class="bbai-daily-top-grid">
		<section class="bbai-daily-hero-card bbai-daily-card <?php echo esc_attr( $bbai_daily_card_mod ); ?>" aria-labelledby="bbai-daily-hero-title" data-bbai-daily-hero-card="1">
			<div class="bbai-daily-hero-main">
				<div
					class="bbai-daily-donut"
					data-bbai-daily-donut="1"
					data-bbai-daily-donut-focus="<?php echo esc_attr( $bbai_daily_focus ); ?>"
					data-bbai-daily-generated="<?php echo esc_attr( (string) $bbai_daily_optimised ); ?>"
					data-bbai-daily-review="<?php echo esc_attr( (string) $bbai_daily_review ); ?>"
					data-bbai-daily-missing="<?php echo esc_attr( (string) $bbai_daily_missing ); ?>"
					data-bbai-daily-total="<?php echo esc_attr( (string) $bbai_daily_total ); ?>"
					style="--bbai-daily-donut-background: <?php echo esc_attr( $bbai_daily_donut_background ); ?>"
					aria-hidden="true"
				>
					<span class="bbai-daily-donut-value" data-bbai-daily-donut-value="1"><?php echo esc_html( $bbai_daily_ring_count ); ?></span>
					<span class="bbai-daily-donut-label" data-bbai-daily-donut-label="1"><?php echo esc_html( $bbai_daily_ring_label ); ?></span>
				</div>
			...
			<div class="bbai-daily-hero-actions">
				<a class="bbai-daily-btn bbai-daily-btn--primary<?php echo $bbai_daily_missing > 0 && $bbai_daily_remaining <= 0 ? ' bbai-daily-btn--premium-gated' : ''; ?>" data-bbai-daily-primary-cta="1" <?php echo $bbai_daily_button_attrs( $bbai_daily_primary ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					<span aria-hidden="true"><?php echo esc_html( $bbai_daily_missing > 0 && $bbai_daily_remaining <= 0 ? '🔒' : ( $bbai_daily_missing > 0 ? '☁' : '✓' ) ); ?></span><?php echo esc_html( $bbai_daily_primary['label'] ); ?>
				</a>
			...
			<div class="bbai-daily-generation-progress" data-bbai-daily-generation-progress="1" aria-live="polite" hidden>
				<div class="bbai-daily-generation-progress__meta">
					<span data-bbai-daily-generation-progress-label><?php esc_html_e( 'Preparing image generation…', 'beepbeep-ai-alt-text-generator' ); ?></span>
					<strong data-bbai-daily-generation-progress-percent><?php esc_html_e( 'Starting', 'beepbeep-ai-alt-text-generator' ); ?></strong>
				</div>
				<div
					class="bbai-daily-generation-progress__track"
					data-bbai-daily-generation-progress-track
					role="progressbar"
					aria-valuemin="0"
					aria-valuemax="100"
					aria-valuenow="0"
				...
				>
					<span class="bbai-daily-generation-progress__fill" data-bbai-daily-generation-progress-fill></span>
				</div>
			</div>
			...
			<div class="bbai-daily-queue-row">
				<a class="bbai-daily-queue-link<?php echo $bbai_daily_missing <= 0 ? ' bbai-daily-queue-link--disabled' : ''; ?>" href="<?php echo esc_url( $bbai_daily_missing > 0 ? $bbai_daily_missing_library_url : $bbai_daily_library_url ); ?>" data-action="navigate" data-bbai-dashboard-status-filter="missing" <?php echo $bbai_daily_missing <= 0 ? 'aria-disabled="true"' : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					<i class="bbai-daily-dot bbai-daily-dot--amber" aria-hidden="true"></i><strong data-bbai-daily-queue-missing="1"><?php echo esc_html( number_format_i18n( $bbai_daily_missing ) ); ?></strong> <span data-bbai-daily-queue-missing-label="1"><?php esc_html_e( 'Missing ALT', 'beepbeep-ai-alt-text-generator' ); ?></span>
				</a>
				<a class="bbai-daily-queue-link" href="<?php echo esc_url( $bbai_daily_review_library_url ); ?>" data-action="navigate" data-bbai-dashboard-status-filter="weak">
					<i class="bbai-daily-dot bbai-daily-dot--red" aria-hidden="true"></i><strong data-bbai-daily-queue-review="1"><?php echo esc_html( number_format_i18n( $bbai_daily_review ) ); ?></strong> <span data-bbai-daily-queue-review-label="1"><?php esc_html_e( 'Needs review', 'beepbeep-ai-alt-text-generator' ); ?></span>
				</a>
				<a class="bbai-daily-queue-link" href="<?php echo esc_url( $bbai_daily_optimized_library_url ); ?>" data-action="navigate" data-bbai-dashboard-status-filter="optimized">
					<i class="bbai-daily-dot bbai-daily-dot--green" aria-hidden="true"></i><strong data-bbai-daily-queue-optimised="1"><?php echo esc_html( number_format_i18n( $bbai_daily_optimised ) ); ?></strong> <span data-bbai-daily-queue-optimised-label="1"><?php esc_html_e( 'Optimised', 'beepbeep-ai-alt-text-generator' ); ?></span>
				</a>
			</div>
		...
		<section class="bbai-daily-usage-panel bbai-daily-card" aria-label="<?php esc_attr_e( 'Usage and automation', 'beepbeep-ai-alt-text-generator' ); ?>">
			<div class="bbai-daily-usage-head">
				...
				<a class="bbai-daily-switch" <?php echo $bbai_daily_button_attrs( $bbai_daily_autopilot_action ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> role="button" aria-pressed="false"><span></span></a>
			</div>
			...
			<div class="bbai-daily-credit-box">
					...
					?></strong>
				<a <?php echo $bbai_daily_button_attrs( $bbai_daily_autopilot_action ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html( $bbai_daily_autopilot_label ); ?><br><?php echo esc_html( $bbai_daily_is_pro_plan ? __( 'Manage automatic generation', 'beepbeep-ai-alt-text-generator' ) : __( '(up to 1,000 images/month)', 'beepbeep-ai-alt-text-generator' ) ); ?></a>
			</div>
		</section>
	</div>
	...
	<section class="bbai-daily-optimised-strip bbai-daily-card <?php echo esc_attr( $bbai_daily_strip_mod ); ?>" aria-label="<?php esc_attr_e( 'Optimisation summary', 'beepbeep-ai-alt-text-generator' ); ?>" data-bbai-daily-strip="1">
		...
		<div>
			...
			<div
				class="bbai-daily-meter bbai-daily-meter--segmented bbai-daily-meter--<?php echo esc_attr( $bbai_daily_optimised_meter_state ); ?>"
				data-bbai-daily-strip-meter="1"
				role="img"
				aria-label="<?php echo esc_attr( sprintf(
					/* translators: 1: optimised count, 2: review count, 3: missing count. */
					__( '%1$s images optimised, %2$s need review, %3$s missing ALT text', 'beepbeep-ai-alt-text-generator' ),
					number_format_i18n( $bbai_daily_optimised ),
					number_format_i18n( $bbai_daily_review ),
					number_format_i18n( $bbai_daily_missing )
				) ); ?>"
			>
				<span class="bbai-daily-meter__segment bbai-daily-meter__segment--optimised" data-bbai-daily-strip-segment="optimised" style="width: <?php echo esc_attr( $bbai_daily_segment_pct( $bbai_daily_optimised ) ); ?>%;"></span>
				<span class="bbai-daily-meter__segment bbai-daily-meter__segment--review" data-bbai-daily-strip-segment="review" style="width: <?php echo esc_attr( $bbai_daily_segment_pct( $bbai_daily_review ) ); ?>%;"></span>
				<span class="bbai-daily-meter__segment bbai-daily-meter__segment--missing" data-bbai-daily-strip-segment="missing" style="width: <?php echo esc_attr( $bbai_daily_segment_pct( $bbai_daily_missing ) ); ?>%;"></span>
			</div>
		</div>
		<a class="bbai-daily-btn bbai-daily-btn--primary" <?php echo $bbai_daily_button_attrs( $bbai_daily_autopilot_action ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><span aria-hidden="true">⚡</span><?php echo esc_html( $bbai_daily_autopilot_label ); ?></a>
