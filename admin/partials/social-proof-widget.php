...
if ( ! function_exists( 'bbai_get_wporg_reviews_for_social_proof' ) ) {
	...
	function bbai_get_wporg_reviews_for_social_proof( $limit = 6 ) {
		...
		$snapshot      = file_exists( $snapshot_file ) ? include $snapshot_file : array();
		...
		if ( is_wp_error( $response ) ) {
			return is_array( $snapshot ) ? bbai_prepare_social_proof_reviews( $snapshot, $limit ) : array();
		}
...
$bbai_fallback_testimonials = file_exists( $bbai_testimonials_file ) ? require $bbai_testimonials_file : array();
$bbai_fallback_testimonials = is_array( $bbai_fallback_testimonials ) ? $bbai_fallback_testimonials : array();
	...
	<div class="bbai-trust-badges">
		...
		<div class="bbai-trust-badge bbai-trust-badge--users">
			...
			<span class="bbai-trust-badge-text">
				...
				<span class="bbai-trust-badge-subtitle"><?php esc_html_e( 'Using OpptiAI', 'beepbeep-ai-alt-text-generator' ); ?></span>
			</span>
		</div>
	</div>
