...
$bbai_status_row_preview_only = ! empty( $bbai_status_row_preview_only ) && empty( $bbai_has_connected_account );
$bbai_status_active           = 'all';
$bbai_status_interaction_mode = ( ! empty( $bbai_is_anonymous_trial ) && empty( $bbai_has_connected_account ) )
	? 'filter'
	: 'navigate';
if ( $bbai_status_row_preview_only ) {
	$bbai_status_interaction_mode = 'filter';
}
...
<section
	...
	class="bbai-dashboard-status-card<?php echo $bbai_status_row_is_hero ? ' bbai-dashboard-status-card--hero' : ''; ?><?php echo $bbai_status_row_preview_only ? ' bbai-dashboard-status-card--guest-preview' : ''; ?>"
	data-bbai-dashboard-status-card="1"
	data-bbai-status-guest-preview="<?php echo $bbai_status_row_preview_only ? '1' : '0'; ?>"
...
>
	<h2 id="bbai-dashboard-status-card-heading" class="screen-reader-text"><?php echo esc_html( $bbai_status_row_preview_only ? __( 'Preview: image status in your library (sign in to filter)', 'beepbeep-ai-alt-text-generator' ) : __( 'Image status', 'beepbeep-ai-alt-text-generator' ) ); ?></h2>
	<?php
	bbai_ui_render(
		'filter-group',
		array(
			'id'               => 'bbai-dashboard-status-nav',
			'aria_label'       => $bbai_status_row_preview_only
				? __( 'Preview stats — not interactive until you create an account', 'beepbeep-ai-alt-text-generator' )
			...
			'root_class'       => 'bbai-dashboard-status-card__filters' . ( $bbai_status_row_is_hero ? ' bbai-dashboard-status-card__filters--hero' : '' ) . ( $bbai_status_row_preview_only ? ' bbai-dashboard-status-card__filters--guest-preview' : '' ),
			'root_attrs'       => array(
				...
				'data-bbai-guest-preview-chips'     => $bbai_status_row_preview_only ? '1' : '0',
			),
