...
$bbai_rn_primary = $bbai_rn['primary'] ?? array();
$bbai_rn_tel     = isset( $bbai_rn['telemetry'] ) && is_array( $bbai_rn['telemetry'] ) ? $bbai_rn['telemetry'] : array();

$bbai_rn_attrs = static function ( array $action ): string {
	$href  = isset( $action['href'] ) && '' !== (string) $action['href'] ? (string) $action['href'] : '#';
	$parts = array( 'href="' . esc_url( $href ) . '"' );
