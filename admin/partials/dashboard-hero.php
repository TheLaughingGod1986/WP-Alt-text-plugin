...
if ( 'exhausted' === $bbai_guest_hero_variant ) {
	...
	$bbai_secondary_login = [
		'label'    => __( 'Log in', 'beepbeep-ai-alt-text-generator' ),
	...
	$bbai_left_helper        = $bbai_state_missing_count > 0
		? sprintf(
			...
			__( '%s images need ALT text', 'beepbeep-ai-alt-text-generator' ),
	...
	$n_rem = max( 0, (int) $bbai_gt_remaining );
	$bbai_show_generate_primary = $n_rem > 0 && $bbai_state_missing_count > 0;
	$bbai_primary_generate = [
		'label'       => sprintf(
			/* translators: %d: generations left */
			__( 'Generate %d remaining images', 'beepbeep-ai-alt-text-generator' ),
			$n_rem
		),
		'class'       => 'bbai-btn bbai-btn-primary bbai-li-btn-primary',
	...
	$bbai_primary_register = [
		'label'       => __( 'Create free account', 'beepbeep-ai-alt-text-generator' ),
		'class'       => ( ! $bbai_show_generate_primary && $n_rem > 0 )
			? 'bbai-btn bbai-btn-primary bbai-li-btn-primary'
			: 'bbai-btn bbai-btn-secondary bbai-li-btn-secondary',
		'action'      => 'show-dashboard-auth',
		'auth_tab'    => 'register',
		'analytics'   => ( ! $bbai_show_generate_primary && $n_rem > 0 ) ? 'guest_hero_primary_register_mid' : 'guest_hero_secondary_register',
		'is_button'   => false,
	];
	$bbai_secondary_login = [
	...
	$bbai_guest_body  = __( 'Generate your first ALT text for free. No account needed.', 'beepbeep-ai-alt-text-generator' );

	$bbai_show_generate_primary = $bbai_gt_remaining > 0;
	$bbai_primary_generate    = [
...
>
		...
		<div class="bbai-li-donut-area">
			...
			>
				...
				<span class="bbai-command-donut__center bbai-li-donut__center">
					...
					<span class="bbai-command-donut__center-label bbai-li-donut__sub-label" data-bbai-funnel-donut-label">
			...
			<p class="bbai-li-donut__helper"><?php echo $bbai_left_helper ? esc_html( $bbai_left_helper ) : '&nbsp;'; ?></p>
		</div>
	</div>
	...
	<div class="bbai-li-card bbai-li-card--content">
		...
		<div class="bbai-li-card-section bbai-li-card-section--intro">
			...
			<?php if ( isset( $bbai_state_weak_count ) && (int) $bbai_state_weak_count > 0 && 'fresh' === $bbai_guest_hero_variant ) : ?>
				<p class="bbai-guest-hero__meta-note">
					<?php
					echo esc_html(
						sprintf(
							/* translators: %s: number of images flagged for review */
							__( '%s descriptions could use a closer look after you sign in.', 'beepbeep-ai-alt-text-generator' ),
		...
		<div class="bbai-li-card-section bbai-li-card-section--monetisation bbai-guest-hero__trial-blurb">
			<?php if ( 'exhausted' !== $bbai_guest_hero_variant ) : ?>
				<div class="bbai-guest-hero__trial-copy">
					<p class="bbai-guest-hero__trial-line">
						<?php
						echo esc_html(
							sprintf(
								/* translators: %s: number of free trial generations */
					...
					<p class="bbai-guest-hero__trial-line bbai-guest-hero__trial-line--muted">
						<?php
						echo esc_html(
							sprintf(
								/* translators: %d: monthly generations after free signup */
								__( 'Create a free account to unlock %d/month', 'beepbeep-ai-alt-text-generator' ),
								(int) $bbai_free_plan_monthly
							)
						);
						?>
			...
			<?php else : ?>
				<p class="bbai-guest-hero__trial-line bbai-guest-hero__trial-line--muted"><?php esc_html_e( 'No credit card required to create an account.', 'beepbeep-ai-alt-text-generator' ); ?></p>
			<?php endif; ?>
		</div>
	</div>
</section>
