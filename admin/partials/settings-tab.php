...
?>
            ...
            <div class="bbai-container bbai-settings-page">
                <?php
                    ...
                    $bbai_billing_info = [];
                    if ($this->api_client->is_authenticated() && method_exists($this->api_client, 'get_billing_info')) {
                        $bbai_billing_response = $this->api_client->get_billing_info();
                        if (!is_wp_error($bbai_billing_response) && is_array($bbai_billing_response)) {
                            $bbai_billing_payload = $bbai_billing_response['data'] ?? $bbai_billing_response;
                            if (isset($bbai_billing_payload['data']['billing']) && is_array($bbai_billing_payload['data']['billing'])) {
                                $bbai_billing_info = $bbai_billing_payload['data']['billing'];
                            } elseif (isset($bbai_billing_payload['billing']) && is_array($bbai_billing_payload['billing'])) {
                                $bbai_billing_info = $bbai_billing_payload['billing'];
                            } elseif (isset($bbai_billing_payload['customerId']) || isset($bbai_billing_payload['subscriptionId'])) {
                                $bbai_billing_info = $bbai_billing_payload;
                            }
                        }
                    }
                ...
                $bbai_live_can_autopilot = !empty($bbai_usage_box['can_autopilot']);
                $bbai_live_quota_limit = max(
                    0,
                    (int) ($bbai_usage_box['token_limit'] ?? 0),
                    (int) ($bbai_usage_box['limit'] ?? 0),
                    (int) ($bbai_usage_box['credits_total'] ?? 0)
                );
                if ('growth' === strtolower((string) $bbai_plan)) {
                    $bbai_plan = 'pro';
                }
                if ('free' === strtolower((string) $bbai_plan) && ($bbai_live_can_autopilot || $bbai_live_quota_limit >= 1000)) {
                    $bbai_plan = 'pro';
                }

                $bbai_plan_normalized = strtolower((string) $bbai_plan);
                $bbai_is_starter = in_array($bbai_plan_normalized, ['starter', 'basic'], true);
                $bbai_is_pro = ($bbai_plan_normalized === 'pro' || $bbai_plan_normalized === 'growth');
                $bbai_is_agency = $bbai_plan_normalized === 'agency';
                $bbai_is_growth_plan = ($bbai_is_pro || $bbai_is_agency);
                $bbai_has_paid_plan = ($bbai_is_starter || $bbai_is_growth_plan);
                $bbai_can_use_upload_generation = method_exists($this, 'current_account_can_use_upload_generation')
                    ...
                    : $bbai_is_growth_plan;
                $bbai_can_use_upload_generation = $bbai_can_use_upload_generation || $bbai_live_can_autopilot || $bbai_is_growth_plan || $bbai_live_quota_limit >= 1000;
                if (!$bbai_can_use_upload_generation) {
                    $bbai_o['enable_on_upload'] = false;
                }

                if ($bbai_is_agency) {
                    $bbai_plan_label = esc_html__('Agency', 'beepbeep-ai-alt-text-generator');
                } elseif ($bbai_is_pro) {
                    $bbai_plan_label = esc_html__('Growth', 'beepbeep-ai-alt-text-generator');
                } elseif ($bbai_is_starter) {
                    $bbai_plan_label = esc_html__('Starter', 'beepbeep-ai-alt-text-generator');
                } else {
                    $bbai_plan_label = esc_html__('Free', 'beepbeep-ai-alt-text-generator');
                }
                ...
                $bbai_billing_customer_id = '';
                $bbai_billing_subscription_id = '';
                if (is_array($bbai_billing_info)) {
                    $bbai_billing_customer_id = (string) (
                        $bbai_billing_info['customerId']
                        ?? $bbai_billing_info['customer_id']
                        ?? $bbai_billing_info['stripe_customer_id']
                        ?? ''
                    );
                    $bbai_billing_subscription_id = (string) (
                        $bbai_billing_info['subscriptionId']
                        ?? $bbai_billing_info['subscription_id']
                        ?? $bbai_billing_info['stripe_subscription_id']
                        ?? ''
                    );
                }
                $bbai_license_org = (is_array($bbai_license_data) && isset($bbai_license_data['organization']) && is_array($bbai_license_data['organization']))
                    ? $bbai_license_data['organization']
                    : [];
                $bbai_billing_customer_id = $bbai_billing_customer_id ?: (string) (
                    $bbai_license_org['customerId']
                    ?? $bbai_license_org['customer_id']
                    ?? $bbai_license_org['stripe_customer_id']
                    ?? ''
                );
                $bbai_billing_subscription_id = $bbai_billing_subscription_id ?: (string) (
                    $bbai_license_org['subscriptionId']
                    ?? $bbai_license_org['subscription_id']
                    ?? $bbai_license_org['stripe_subscription_id']
                    ?? ''
                );
                ...
                $bbai_last_contacted_support_at = get_user_meta(get_current_user_id(), 'bbai_last_contacted_support_at', true);
                $bbai_last_contacted_support_label = '';
                if (!empty($bbai_last_contacted_support_at)) {
                    $bbai_last_contacted_support_ts = strtotime((string) $bbai_last_contacted_support_at);
                    if ($bbai_last_contacted_support_ts) {
                        $bbai_last_contacted_support_absolute = date_i18n(
                            get_option('date_format') . ' ' . get_option('time_format'),
                            $bbai_last_contacted_support_ts
                        );
                        $bbai_last_contacted_support_relative = human_time_diff($bbai_last_contacted_support_ts, current_time('timestamp')) . ' ' . __('ago', 'beepbeep-ai-alt-text-generator');
                        $bbai_last_contacted_support_label = sprintf(
                            ...
                            $bbai_last_contacted_support_absolute,
                            $bbai_last_contacted_support_relative
                        );
                    }
                }
                ...
                <div class="bbai-card bbai-settings-card bbai-page-section">
                    ...
                    <?php if (!$bbai_has_paid_plan) : ?>
                    <div class="bbai-settings-account-info-banner">
                        <span><?php esc_html_e('Subscription tools appear here after upgrading.', 'beepbeep-ai-alt-text-generator'); ?></span>
                    </div>
                    <?php else : ?>
                    <?php 
                    ...
                    if ($bbai_is_authenticated_for_account) :
                        // Authenticated paid account - let the backend resolve the Stripe portal from account/site billing state.
                    ?>
                    <div class="bbai-settings-account-actions">
                        <button type="button" class="bbai-btn bbai-btn-secondary" data-action="manage-subscription" data-bbai-tooltip="<?php esc_attr_e('View invoices, update payment method, downgrade, or cancel your subscription', 'beepbeep-ai-alt-text-generator'); ?>" data-bbai-tooltip-position="bottom">
                        ...
                        <div class="bbai-settings-account-action-info">
                            ...
                            <ul>
                                ...
                                <li><?php esc_html_e('Downgrade, cancel, or modify subscription', 'beepbeep-ai-alt-text-generator'); ?></li>
                            </ul>
                        </div>
                    </div>
                    <?php else :
                        // Manual membership/license grant - no Stripe portal exists.
                    ?>
                    <div class="bbai-settings-account-actions">
                        <button
                            type="button"
                            ...
                            data-bbai-contact-subject="<?php echo esc_attr($bbai_support_subject); ?>"
                            data-bbai-contact-message="<?php echo esc_attr($bbai_support_message); ?>"
                            data-bbai-tooltip="<?php esc_attr_e('Request a downgrade or cancellation for this manually managed membership', 'beepbeep-ai-alt-text-generator'); ?>"
                            data-bbai-tooltip-position="bottom"
                        >
                            <span><?php esc_html_e('Request downgrade or cancellation', 'beepbeep-ai-alt-text-generator'); ?></span>
                        </button>
                        <div class="bbai-settings-account-action-info">
                            <p><strong><?php esc_html_e('Membership managed manually', 'beepbeep-ai-alt-text-generator'); ?></strong></p>
                            <p><?php esc_html_e('This membership is active, but it is not linked to a Stripe subscription. There is no Stripe billing portal for this membership.', 'beepbeep-ai-alt-text-generator'); ?></p>
                            <ul>
                                <li><?php esc_html_e('Use the support request to downgrade or cancel this membership', 'beepbeep-ai-alt-text-generator'); ?></li>
                                <li><?php esc_html_e('Include your site URL so support can identify the correct account', 'beepbeep-ai-alt-text-generator'); ?></li>
                                <?php if ($bbai_is_license_only) : ?>
                                <li><?php esc_html_e('If this license is managed by another administrator, contact that license owner', 'beepbeep-ai-alt-text-generator'); ?></li>
                                <?php endif; ?>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="bbai-card bbai-settings-card bbai-page-section">
                    <div class="bbai-settings-card-header">
                        <div class="bbai-settings-card-header-icon">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
                                <path d="M10 18C14.4183 18 18 14.4183 18 10C18 5.58172 14.4183 2 10 2C5.58172 2 2 5.58172 2 10C2 11.2736 2.29755 12.4778 2.8263 13.5462L2 18L6.45382 17.1737C7.52224 17.7025 8.72638 18 10 18Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/>
                                <path d="M7 8.5H13M7 11.5H11" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
                            </svg>
                        </div>
                        <h3 class="bbai-settings-card-title"><?php esc_html_e('Contact Support', 'beepbeep-ai-alt-text-generator'); ?></h3>
                    </div>
                    <div class="bbai-settings-account-actions">
                        <button
                            type="button"
                            class="bbai-btn bbai-btn-secondary"
                            data-action="open-contact-modal"
                            data-bbai-contact-subject="<?php esc_attr_e('Support request', 'beepbeep-ai-alt-text-generator'); ?>"
                            data-bbai-contact-message="<?php esc_attr_e('Please describe what happened and what you expected to happen.', 'beepbeep-ai-alt-text-generator'); ?>"
                        >
                            <span><?php esc_html_e('Contact support', 'beepbeep-ai-alt-text-generator'); ?></span>
                        </button>
                        <div class="bbai-settings-account-action-info">
                            <p><?php esc_html_e('Send a support request with your debug logs attached automatically. Your WordPress user email is filled in for replies.', 'beepbeep-ai-alt-text-generator'); ?></p>
                            <p
                                class="bbai-settings-support-last-contact"
                                data-bbai-support-last-contact
                                <?php echo $bbai_last_contacted_support_label ? '' : 'hidden'; ?>
                            >
                                <?php echo esc_html($bbai_last_contacted_support_label); ?>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form method="post" action="options.php" autocomplete="off">
                    ...
                    <?php if (!$bbai_has_paid_plan) : ?>
                    <div class="bbai-paywall-preview bbai-paywall-preview--settings-alt">
                        <div class="bbai-paywall-preview__content" aria-hidden="true">
                    <?php endif; ?>
                    <div class="bbai-card bbai-settings-card bbai-page-section">
                    ...
                    <?php if (!$bbai_has_paid_plan) : ?>
                        ...
                        <div class="bbai-paywall-preview__overlay" role="region" aria-label="<?php esc_attr_e('ALT text settings upgrade prompt', 'beepbeep-ai-alt-text-generator'); ?>">
                            <span class="bbai-paywall-preview__eyebrow"><?php esc_html_e('Growth feature', 'beepbeep-ai-alt-text-generator'); ?></span>
                            <h2><?php esc_html_e('Unlock ALT text settings', 'beepbeep-ai-alt-text-generator'); ?></h2>
                            <p><?php esc_html_e('Upgrade to control generation style, add custom instructions, and automate ALT text for new uploads.', 'beepbeep-ai-alt-text-generator'); ?></p>
                            <div class="bbai-paywall-preview__actions">
                                <button type="button" class="bbai-btn bbai-btn-primary" data-action="show-upgrade-modal"><?php esc_html_e('Upgrade plan', 'beepbeep-ai-alt-text-generator'); ?></button>
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
                ...
                <?php
                $bbai_debug_embedded = true;
                $debug_bootstrap = isset($bbai_debug_bootstrap) && is_array($bbai_debug_bootstrap)
                    ? $bbai_debug_bootstrap
                    : (method_exists($this, 'get_debug_payload') ? $this->get_debug_payload(['per_page' => 10, 'page' => 1]) : []);
                $bbai_debug_partial = BEEPBEEP_AI_PLUGIN_DIR . 'admin/partials/debug-tab.php';
                if (file_exists($bbai_debug_partial)) {
                    include $bbai_debug_partial;
                } else {
