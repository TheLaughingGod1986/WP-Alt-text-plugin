...
trait Core_Export {
	...
	public function handle_usage_export() {
		...
		$this->bbai_output_contents( $this->bbai_csv_row( array( 'Attachment ID', 'Title', 'ALT Text', 'Source', 'Model', 'Generated At' ) ) );
		foreach ( $rows as $row ) {
			$this->bbai_output_contents(
				$this->bbai_csv_row(
					array(
	...
	public function handle_debug_log_export() {
		...
		$this->bbai_prepare_download_stream();
		$this->bbai_output_contents( $this->bbai_csv_row( array( 'Timestamp', 'Level', 'Message', 'Source', 'Context' ) ) );
		foreach ( $rows as $row ) {
			$context = $row['context'] ?? '';
			$this->bbai_output_contents(
				$this->bbai_csv_row(
