...
/**
 * AJAX Billing Trait
 * Handles Stripe checkout and billing portal AJAX endpoints
 *
 * @package BeepBeep_AI
 * @since 5.0.0
 */
...
trait Ajax_Billing {
    ...
    /**
     * AJAX handler: Get subscription info
     */
    public function ajax_get_subscription_info() {
        ...
        $user_info = $this->api_client->is_authenticated() ? $this->api_client->get_user_info() : null;
        ...
        wp_send_json_success([
            'usage' => $usage,
            'user' => is_wp_error($user_info) ? null : $user_info,
            'is_authenticated' => $this->api_client->is_authenticated()
        ]);
    }
}
