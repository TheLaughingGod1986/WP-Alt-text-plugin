...
trait Core_Assets {
	...
	private function enqueue_logout_bridge_script(): void {
		...
		wp_register_script( $handle, '', array(), BEEPBEEP_AI_VERSION, true );
		wp_enqueue_script( $handle );
		wp_localize_script(
			$handle,
			'bbaiLogoutBridgeData',
			array(
				...
				'admin_logout_redirect' => add_query_arg( array( 'tab' => 'admin' ), admin_url( 'upload.php?page=bbai' ) ),
...
JS,
	...
	private function enqueue_media_library_assets( string $hook, string $base_url, string $base_path ): void {
		...
		if ( file_exists( $base_path . $toast_file ) ) {
			...
			wp_register_script( 'bbai-toast', '', array(), BEEPBEEP_AI_VERSION, true );
		}
		...
		if ( file_exists( $base_path . $bbai_banner_message_js ) ) {
			...
			wp_register_script( 'bbai-banner-message', '', array(), BEEPBEEP_AI_VERSION, true );
		}
		...
		if ( file_exists( $base_path . $bbai_bg_job_js ) ) {
			wp_enqueue_script(
				...
				array( 'bbai-job-state', 'bbai-job-widget' ),
	...
	private function enqueue_dashboard_assets( string $base_url, string $base_path ): void {
		...
		wp_enqueue_style(
			...
			array( 'bbai-modern', 'bbai-admin-wizard' ),
		...
		wp_enqueue_style(
			...
			array( 'bbai-upgrade-modal-refresh' ),
		...
		wp_enqueue_style(
			...
			array( 'bbai-admin-ui-components' ),
		...
		$section_header_deps = array( 'bbai-admin-micro-motion' );
		if ( file_exists( $base_path . $card_rhythm_css ) ) {
		...
		if ( file_exists( $base_path . $admin_controls_css ) ) {
			$admin_controls_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_interactions_css ) ) {
			$admin_ix_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_product_states_css ) ) {
			$admin_ps_deps = array( 'bbai-admin-interactions' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_surfaces_css ) ) {
			$admin_surfaces_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_design_system_css ) ) {
			$admin_ds_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_semantic_status_css ) ) {
			$admin_semantic_status_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
		...
		if ( file_exists( $base_path . $admin_table_workspace_css ) ) {
			$admin_table_ws_deps = array( 'bbai-section-header' );
			if ( file_exists( $base_path . $admin_foundation_tokens_css ) ) {
			...
			if ( file_exists( $base_path . $admin_page_adoption_css ) ) {
				$admin_page_adoption_deps = array( 'bbai-admin-table-workspace' );
				if ( file_exists( $base_path . $admin_surfaces_css ) ) {
			...
			if ( file_exists( $base_path . $admin_surface_taxonomy_css ) ) {
				$admin_surface_taxonomy_deps = array( 'bbai-admin-table-workspace' );
				if ( file_exists( $base_path . $admin_page_adoption_css ) ) {
					$admin_surface_taxonomy_deps = array( 'bbai-admin-page-adoption' );
				}
				wp_enqueue_style(
			...
			if ( file_exists( $base_path . $admin_library_workspace_polish_css ) ) {
				wp_enqueue_style(
					...
					array( 'bbai-admin-table-workspace' ),
		...
		if ( file_exists( $base_path . $admin_button_system_css ) ) {
			$admin_button_system_deps  = array( 'bbai-saas-consistency' );
			...
			if (
				...
				$admin_button_system_deps = array( 'bbai-admin-library-workspace-polish' );
			} elseif ( file_exists( $base_path . $admin_table_ws_path ) ) {
				$admin_button_system_deps = array( 'bbai-admin-table-workspace' );
			}
			wp_enqueue_style(
		...
		if ( file_exists( $base_path . $dashboard_js ) ) {
			wp_enqueue_script(
				...
				array( 'jquery', 'wp-api-fetch', 'wp-i18n', 'bbai-toast', 'bbai-banner-message', 'bbai-telemetry', 'bbai-admin' ),
				$asset_version( $dashboard_js, '3.0.4' ),
				true
			);
		} else {
			// Register an empty stub so dependent scripts don't break WP's dependency resolver.
			wp_register_script( 'bbai-dashboard', '', array(), BEEPBEEP_AI_VERSION, true );
		}
		...
		if ( file_exists( $base_path . $auth_js ) ) {
			wp_enqueue_script(
				'bbai-auth',
				$base_url . $auth_js,
				array( 'jquery', 'wp-i18n', 'bbai-dashboard' ),
		...
		if ( file_exists( $base_path . $logger_js ) ) {
			...
			wp_register_script( 'bbai-logger', '', array(), BEEPBEEP_AI_VERSION, true );
		}
		...
		if ( file_exists( $base_path . $modal_js ) ) {
			wp_enqueue_script(
				'bbai-modal',
				$base_url . $modal_js,
				array( 'jquery', 'bbai-logger', 'wp-i18n' ),
		...
		if ( file_exists( $base_path . $usage_bridge_js ) ) {
			wp_enqueue_script(
				...
				array( 'bbai-dashboard', 'wp-element' ),
		...
		if ( file_exists( $base_path . $bbai_li_controller_js ) && $this->is_dashboard_tab() && apply_filters( 'bbai_enqueue_logged_in_dashboard_controller', false ) ) {
			wp_enqueue_script(
				...
				array( 'bbai-admin', 'bbai-dashboard' ),
		...
		if ( file_exists( $base_path . $bbai_composition_css ) ) {
			wp_enqueue_style(
				...
				array( 'bbai-logged-in-state', 'bbai-dashboard-header' ),
	...
	private function localize_dashboard_scripts( array $stats_data, array $usage_data, array $checkout_prices, array $l10n_common ): void {
		...
		if ( $bbai_has_connected_upgrade_path && in_array( $bbai_plan_slug_cta, array( 'free', 'trial' ), true ) && null !== $bbai_rem_cta && $bbai_rem_cta <= 0 ) {
		...
		wp_localize_script(
			...
			array(
				...
				'can_manage'             => $this->user_can_manage(),
				'logout_redirect'        => admin_url( 'admin.php?page=bbai' ),
				'stripe_links'           => method_exists( $this, 'get_checkout_stripe_links' ) ? $this->get_checkout_stripe_links() : self::DEFAULT_STRIPE_LINKS,
				'is_us_checkout'         => method_exists( $this, 'is_us_checkout_client' ) && $this->is_us_checkout_client(),
				'is_guest_trial'         => $bbai_upgrade_path_guest,
				'use_licensed_bulk_jobs' => method_exists( $this, 'should_use_licensed_bulk_jobs_api' ) && $this->should_use_licensed_bulk_jobs_api(),
				'anon_cookie_name'       => $anon_cookie_name,
			)
	...
	private function get_monetisation_bootstrap_for_localize(): array {
		...
		return function_exists( 'bbai_monetisation_client_bootstrap' ) ? bbai_monetisation_client_bootstrap() : array();
	}
	...
	private function enqueue_posthog_layer( string $base_url, string $base_path ): void {
		...
		if ( ! is_readable( $path ) ) {
			...
			wp_register_script( 'bbai-posthog', '', array(), BEEPBEEP_AI_VERSION, true );
			return;
		}
	...
	private function enqueue_telemetry_layer( string $base_url, string $base_path ): void {
		...
		if ( ! is_readable( $path ) ) {
			...
			wp_register_script( 'bbai-telemetry', '', array(), BEEPBEEP_AI_VERSION, true );
			return;
		}
	...
	private function get_telemetry_client_config(): array {
		...
		$license_data        = ( isset( $this->api_client ) && method_exists( $this->api_client, 'get_license_data' ) )
			...
			: array();
		$identity_context    = $this->get_posthog_identity_context(
			is_array( $sanitized_user_data ) ? $sanitized_user_data : array(),
			is_array( $license_data ) ? $license_data : array(),
			$site_hash,
			$plan
		);
