...
class Credit_Usage_Page {
	...
	public static function render_page_content() {
		...
		$allowed_sources = array( '', 'ajax', 'dashboard', 'bulk', 'bulk-regenerate', 'upload', 'metadata', 'update', 'save', 'queue', 'onboarding', 'library', 'manual', 'unknown' );
		...
		$allowed_views = array( 'summary', 'user_detail' );
	...
	private static function map_source_to_usage_driver_bucket( string $source ): string {
		...
		if ( in_array( $source, array( 'upload', 'queue', 'metadata' ), true ) ) {
			return 'automation';
		}
		if ( in_array( $source, array( 'manual', 'library', 'dashboard', 'ajax', 'save', 'update', 'onboarding' ), true ) ) {
			return 'manual';
		}
		return 'other';
	}
	...
	private static function enrich_backend_user_activity( $backend_result ) {
		if ( ! is_array( $backend_result ) || empty( $backend_result['users'] ) || ! is_array( $backend_result['users'] ) ) {
			return is_array( $backend_result ) ? $backend_result : array();
		}
		...
		foreach ( $backend_result['users'] as &$hero ) {
			...
			foreach ( array( 'wp_user_id', 'wordpress_user_id', 'wordpress_id', 'wp_id', 'user_id', 'site_user_id', 'local_user_id' ) as $id_key ) {
			...
			if ( empty( $hero['display_name'] ) ) {
				foreach ( array( 'user_name', 'name', 'username', 'user_email', 'email' ) as $name_key ) {
	...
	private static function resolve_backend_hero_wp_user( array $hero ) {
		...
		$user_id_keys = array( 'wp_user_id', 'wordpress_user_id', 'wordpress_id', 'wp_id', 'user_id', 'site_user_id', 'local_user_id' );
		...
		$email_keys = array( 'user_email', 'email' );
		...
		$backend_user_id_keys = array( 'backend_user_id', 'id', 'user_id' );
		...
		$username_keys = array( 'username', 'user_name', 'user_login', 'login' );
