...
class Core {
	...
	private function build_generation_posthog_identity_context(): array {
		...
		$user_data    = isset( $this->api_client ) ? $this->sanitize_api_user_data_for_localize( $this->api_client->get_user_data() ) : array();
		...
		return $this->get_posthog_identity_context(
			is_array( $user_data ) ? $user_data : array(),
	...
	private function resolve_alt_generation_mode( string $source ): string {
		...
		if ( in_array( $source, array( 'bulk', 'bulk-regenerate' ), true ) ) {
			return 'bulk';
		}

		if ( in_array( $source, array( 'auto', 'upload', 'metadata', 'update', 'save', 'queue' ), true ) ) {
			return 'auto';
		}
	...
	private function resolve_alt_generation_source_page( string $source ): string {
		...
		if ( ! empty( $page ) && ! in_array( $page, array( 'unknown', 'other' ), true ) ) {
			return $page;
		}
	 ...
	 */
	private function maybe_emit_alt_generated_event( int $attachment_id, string $alt_text, string $source, string $model, array $context = array(), array $response_meta = array() ): void {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'maybe_guard_guest_access' ), 19 );
		add_action( 'admin_init', array( $this, 'maybe_guard_library_access' ), 20 );
		// One-time legacy cache key cleanup (consolidate to bbai_usage_cache / bbai_quota_cache)
		add_action( 'init', array( __CLASS__, 'maybe_clean_legacy_cache_keys' ), 1 );
		// One-time legacy settings options cleanup (after migration to bbai_settings)
		add_action( 'init', array( __CLASS__, 'maybe_clean_legacy_settings_options' ), 2 );
	}
	 ...
	 * @return array<string, mixed>
	 */
	private function get_trial_usage_payload( array $overrides = array() ): array {
	...
	private function is_upload_automation_source( $source ): bool {
		...
		return in_array( $source_key, array( 'auto', 'upload', 'metadata', 'update', 'save' ), true );
	}
	...
	private function is_upload_generation_enabled( ?array $opts = null ): bool {
		...
		$opts = is_array( $opts ) ? $opts : get_option( self::OPTION_KEY, array() );
	 ...
	 * @return array<string, mixed>
	 */
	private function get_default_debug_payload( array $args = array() ): array {
	 ...
	 * @return array<string, mixed>
	 */
	public function get_debug_payload( array $args = array() ): array {
		$defaults   = array(
			'level'     => '',
			'search'    => '',
			'date'      => '',
	...
	public function activate() {
		...
		if ( function_exists( 'bbai_telemetry_emit' ) && ! get_option( 'bbai_telemetry_plugin_installed_logged', false ) ) {
			...
			bbai_telemetry_emit( 'plugin_installed', array() );
		}
	...
	public function add_settings_page() {
		...
		if ( $bbai_has_connected_account ) {
			...
			foreach ( array( 'bbai-analytics', 'bbai-credit-usage', 'bbai-settings' ) as $bbai_hidden_guest_route ) {
				add_submenu_page(
					'',
					ucfirst( str_replace( array( 'bbai-', '-' ), array( '', ' ' ), $bbai_hidden_guest_route ) ),
					ucfirst( str_replace( array( 'bbai-', '-' ), array( '', ' ' ), $bbai_hidden_guest_route ) ),
					$cap,
...
<?php elseif ( $bbai_tab === 'settings' ) : ?>
			...
			<?php else : ?>
				...
				<div class="bbai-container bbai-unauth-container">
					...
					printf(
						...
						esc_html( $bbai_tab ),
						esc_html( implode( ', ', array_keys( $bbai_tabs ?? array() ) ) )
					);
					?>
					</p>
			...
			<div class="bbai-footer">
				...
				<?php
		endif; // End tab check (dashboard/library/help/usage/settings/admin views)
	...
	public function get_media_stats() {
		try {
			...
			if ( ! empty( $usage['last_request'] ) ) {
	            ...
	            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				$latest_generated_input = $wpdb->get_var(
					$wpdb->prepare(
	                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.UnescapedDBParameter -- Table identifiers come from trusted core $wpdb properties; value placeholders remain prepared.
	...
	private function build_alt_coverage_scan_job_payload( array $scan_state, array $coverage_payload = array() ): array {
	...
	private function process_alt_coverage_scan_job_batch( array $scan_state ): array {
		...
		foreach ( $rows as $row ) {
			...
			if ( in_array( $source_value, array( 'ai', 'openai' ), true ) ) {
				$scan_state['ai_source_count'] = max( 0, (int) ( $scan_state['ai_source_count'] ?? 0 ) ) + 1;
			}

			$row_state_obj = (object) array(
				'ID'       => $attachment_id,
				'alt_text' => $alt_text,
			);
	...
	public function get_dashboard_stats_payload( bool $force_refresh = false ): array {
		...
		$payload                        = is_array( $stats ) ? $stats : array();
	...
	public function ajax_start_alt_coverage_scan() {
		...
		if ( ! $this->user_can_manage() ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'beepbeep-ai-alt-text-generator' ) ), 403 );
			return;
		}
		...
		try {
			...
			$payload = $this->build_alt_coverage_scan_job_payload(
				$scan_state,
				is_array( $scan_state['payload'] ?? null ) ? $scan_state['payload'] : array()
			);
	...
	public function ajax_poll_alt_coverage_scan() {
		...
		if ( ! $scan_state ) {
			wp_send_json_error( array( 'message' => __( 'Scan session expired. Please start again.', 'beepbeep-ai-alt-text-generator' ) ), 404 );
			return;
		}
	...
	public function prepare_attachment_snapshot( $attachment_id ) {
		...
		$row      = $this->get_library_workspace_row_state(
			(object) array(
				'ID'       => $attachment_id,
				'alt_text' => $alt,
			)
		...
		$issues   = ( is_array( $analysis ) && isset( $analysis['issues'] ) && is_array( $analysis['issues'] ) ) ? $analysis['issues'] : array();
	...
	public function get_media_inventory_sync_total(): int {
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
			Debug_Log::log(
				'info',
				'Dashboard bootstrap media inventory total resolved',
				array(
					'total_images' => $total,
	...
	public function get_media_inventory_sync_items( int $limit = 100, int $offset = 0 ): array {
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
			Debug_Log::log(
				'info',
				'Dashboard bootstrap media inventory page collected',
				array(
					'limit'                 => $limit,
					'offset'                => $offset,
	...
	private function map_media_inventory_sync_state( string $status_hint, bool $has_alt, bool $user_approved ): string {
		...
		if ( ! $has_alt || in_array( $normalized, array( 'missing', 'missing_alt' ), true ) ) {
			return 'MISSING';
		}

		if ( in_array( $normalized, array( 'needs_review', 'weak', 'review' ), true ) ) {
			return 'NEEDS_REVIEW';
		}
	...
	private function get_trial_dashboard_preview_rows( int $limit = 3 ): array {
		...
		foreach ( array( 'missing', 'weak', 'optimized' ) as $bucket_key ) {
	...
	private function build_trial_dashboard_preview_row_payload( int $attachment_id ): ?array {
		...
		if ( ! in_array( $status, array( 'missing', 'weak', 'optimized' ), true ) ) {
			$status = 'missing';
		}
	...
	private function get_usage_rows( $limit = 10, $include_all = false ) {
		...
		$formatted = array_map(
			function ( $row ) use ( $date_time_format, $upload_dir ) {
	...
	public function maybe_generate_on_upload( $attachment_id ) {
		$opts = get_option( self::OPTION_KEY, array() );
		if ( ! $this->is_upload_generation_enabled( $opts ) ) {
			return;
		}
	...
	public function generate_and_save( $attachment_id, $source = 'manual', int $retry_count = 0, array $feedback = array(), $regenerate = false, bool $skip_trial_gate = false ) {
		$opts = get_option( self::OPTION_KEY, array() );
		...
		if ( $use_openai_direct ) {
			...
			if ( ! is_wp_error( $vision_out ) && is_array( $vision_out ) && ! empty( $vision_out['alt'] ) ) {
				...
				$usage_vision      = isset( $vision_out['usage'] ) && is_array( $vision_out['usage'] ) ? $vision_out['usage'] : array(
					'prompt'     => 0,
					'completion' => 0,
					'total'      => 0,
				);
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			try {
				...
				$bbai_dbg_usage_before = array( 'error' => $e->getMessage() );
			}
		...
		if ( empty( $alt_text ) ) {
			...
			if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
				Debug_Log::log(
					'error',
					'Alt text generation failed - missing altText/alt_text in response',
					array(
						'attachment_id'    => $attachment_id,
		...
		if ( ! empty( $bbai_usage_data ) && ( isset( $bbai_usage_data['used'] ) || isset( $bbai_usage_data['remaining'] ) ) ) {
			...
			if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
				Debug_Log::log(
					'info',
					'Updating usage cache after generation',
					array(
			...
			if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
				Debug_Log::log(
					'info',
					'Generation response missing credits info, fetching fresh usage from API',
					array(
			...
			if ( is_array( $fresh_usage ) ) {
				...
				if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
					Debug_Log::log(
						'info',
						'Successful generation reflected in local usage cache',
						array(
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			try {
				...
				$usage_after_generate = array( 'error' => $e->getMessage() );
			}
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			try {
				...
				$after = array( 'error' => $e->getMessage() );
			}
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log(
		...
		if ( $bbai_has_license && ! empty( $bbai_usage_data ) ) {
			...
			if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
				Debug_Log::log(
					'info',
					'Updating license organization data with usage',
					array(
		...
		$this->maybe_emit_alt_generated_event(
			...
			(string) $model,
			is_array( $context ) ? $context : array(),
			isset( $api_response['meta'] ) && is_array( $api_response['meta'] ) ? $api_response['meta'] : array()
		);
	...
	public function ajax_check_milestone() {
		...
		if ( ! $this->user_can_manage() ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}

		$milestone = isset( $_POST['milestone'] ) ? absint( wp_unslash( $_POST['milestone'] ) ) : 0;
		if ( $milestone <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid milestone.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Onboarding' ) ) {
			...
			wp_send_json_success( array( 'new_milestone' => $new_milestone ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Onboarding class not found.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	}
	...
	public function ajax_track_milestone() {
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Onboarding' ) ) {
			...
			wp_send_json_success( array( 'message' => __( 'Milestone tracked.', 'beepbeep-ai-alt-text-generator' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Onboarding class not found.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	...
	public function ajax_complete_onboarding() {
		wp_send_json_success( array( 'message' => __( 'Onboarding completed.', 'beepbeep-ai-alt-text-generator' ) ) );
	}
	...
	public function ajax_dismiss_reset_modal() {
		...
		wp_send_json_success( array( 'message' => __( 'Reset modal dismissed.', 'beepbeep-ai-alt-text-generator' ) ) );
	}
	...
	public function ajax_review_prompt_action() {
		...
		$allowed = array( 'shown', 'leave_review', 'already_reviewed', 'remind_later', 'dismiss' );
		$action  = sanitize_key( wp_unslash( $_POST['review_action'] ?? '' ) );
		if ( ! in_array( $action, $allowed, true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid action.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	...
	public function ajax_telemetry() {
		...
		if ( ! is_array( $decoded ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid payload.', 'beepbeep-ai-alt-text-generator' ) ), 400 );
			return;
		}
	...
	public function ajax_refresh_usage() {
		...
		wp_send_json_error( array( 'message' => __( 'Failed to fetch usage data', 'beepbeep-ai-alt-text-generator' ) ) );
	}
	...
	public function ajax_regenerate_single() {
		...
		if ( 0 === $attachment_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid attachment ID', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
		...
		if ( ! $updated_usage || ! is_array( $updated_usage ) || ! isset( $updated_usage['used'] ) ) {
			...
			if ( ! is_wp_error( $fresh_usage ) && is_array( $fresh_usage ) ) {
				...
				if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
					Debug_Log::log(
						'info',
						'Fetched fresh usage from API after regeneration',
						array(
				...
				if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
					Debug_Log::log(
						'warning',
						'API usage fetch failed, using local usage snapshot',
						array(
		...
		if ( class_exists( '\BeepBeepAI\AltTextGenerator\Debug_Log' ) ) {
			Debug_Log::log(
				'info',
				'Sending usage data in AJAX response',
				array(
					'usage'         => $updated_usage,
	...
	public function ajax_bulk_queue() {
		...
		$skip_schedule       = in_array( $skip_schedule_input, array( '1', 'true', 'yes' ), true );
		$allowed_sources     = array( 'bulk', 'bulk-regenerate', 'dashboard', 'library', 'manual', 'onboarding', 'queue', 'unknown' );
		...
		if ( empty( $ids ) ) {
			wp_send_json_error( array( 'message' => __( 'No valid images found', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	...
	public function ajax_get_attachment_ids() {
		...
		if ( ! $this->user_can_manage() ) {
			...
			$scope       = in_array( $scope_input, array( 'missing', 'all', 'needs-review' ), true ) ? $scope_input : 'missing';
	...
	private function generate_preview_alt_for_attachment( int $attachment_id ) {
		...
		if ( $alt_text === '' ) {
			return new \WP_Error(
				...
				array( 'code' => 'api_response_invalid' )
			);
		}
	...
	public function ajax_generate_preview_alt() {
		...
		$ids_input = isset( $_POST['attachment_ids'] ) ? array_map( 'absint', (array) wp_unslash( $_POST['attachment_ids'] ) ) : array();
	...
	public function ajax_apply_alt_batch() {
		...
		foreach ( $batch_ids as $attachment_id ) {
			$result = $this->generate_and_save( (int) $attachment_id, 'onboarding', 1, array(), false );
			if ( is_wp_error( $result ) ) {
				$failed[] = array(
					'id'      => (int) $attachment_id,
	...
	public function ajax_set_woocommerce_context() {
		...
		if ( ! class_exists( '\WooCommerce' ) ) {
			wp_send_json_error( array( 'message' => __( 'WooCommerce is not active on this site.', 'beepbeep-ai-alt-text-generator' ) ), 400 );
			return;
		}

		$enabled_raw = sanitize_text_field( wp_unslash( $_POST['enabled'] ?? '' ) );
		$enabled     = in_array( strtolower( $enabled_raw ), array( '1', 'true', 'yes', 'on' ), true );
	...
	public function ajax_complete_setup_wizard() {
		...
		if ( ! class_exists( '\BeepBeepAI\AltTextGenerator\Onboarding' ) ) {
			wp_send_json_error( array( 'message' => __( 'Onboarding class not found.', 'beepbeep-ai-alt-text-generator' ) ), 500 );
			return;
		}
	...
	public function ajax_create_checkout() {
		...
		if ( is_wp_error( $result ) || $resolved_checkout_url === '' ) {
			...
			wp_send_json_error( array( 'message' => $this->get_checkout_error_message( $result ) ) );
			return;
		}
	...
	private function normalize_generation_job_status( string $status ): string {
		...
		if ( in_array( $status, array( 'queued', 'queue', 'pending', 'scheduled', 'waiting', 'paused', 'pause', 'pending_user' ), true ) ) {
			return 'queued';
		}
	...
	private function is_active_generation_job_status( string $status ): bool {
		return in_array( $this->normalize_generation_job_status( $status ), array( 'queued', 'processing' ), true );
	}
	...
	private function get_active_licensed_bulk_job_status_for_user( int $user_id ): ?array {
		...
		if ( false === $last_progress ) {
			...
			$last_progress = is_array( $last_progress ) ? $last_progress : array();
	...
	private function is_api_job_status_terminal( array $job ): bool {
		...
		return in_array(
			$s,
			array( 'complete', 'completed', 'failed', 'error', 'cancelled', 'canceled' ),
			true
		);
	}
	...
	private function count_job_items_by_phase( array $items ): array {
		...
		foreach ( $items as $item ) {
			...
			if ( in_array( $ph, array( 'completed', 'complete', 'success', 'succeeded' ), true ) ) {
				++$completed;
			} elseif ( in_array( $ph, array( 'failed', 'error' ), true ) ) {
				++$failed;
			} else {
				++$pending_like;
			}
	...
	private function resolve_job_row_attachment_id( array $row ): int {
		foreach ( array( 'attachment_id', 'attachmentId', 'image_id', 'imageId', 'id' ) as $key ) {
			if ( ! isset( $row[ $key ] ) ) {
				continue;
			}
			$v = $row[ $key ];
	...
	private function persist_bulk_job_item_if_needed( string $job_id, int $attachment_id, array $item, string $source, bool $regenerate ): bool {
		...
		$usage_summary = $api_response['tokens'] ?? $api_response['usage'] ?? array(
			'prompt'     => 0,
			'completion' => 0,
			'total'      => 0,
		);
		$usage_summary = is_array( $usage_summary ) ? $usage_summary : array(
			'prompt'     => 0,
			'completion' => 0,
			'total'      => 0,
		);
		...
		$this->maybe_emit_alt_generated_event(
			...
			(string) $source,
			(string) $model,
			is_array( $gen_ctx ) ? $gen_ctx : array(),
			isset( $api_response['meta'] ) && is_array( $api_response['meta'] ) ? $api_response['meta'] : array()
		);
	...
	private function maybe_hydrate_usage_from_job_root( array $job ): void {
		...
		$updated['organization'] = array_merge( $existing_license['organization'] ?? array(), $org );
	...
	public function ajax_bulk_job_start() {
		...
		foreach ( $attachment_ids as $id ) {
			...
			$unit    = $this->api_client->build_alt_text_job_item( $id, is_array( $context ) ? $context : array(), $regenerate );
			if ( is_wp_error( $unit ) ) {
				$preflight_errors[] = array(
					'attachment_id' => $id,
	...
	public function ajax_bulk_job_poll() {
		...
		if ( ! $this->should_use_licensed_bulk_jobs_api() ) {
			wp_send_json_error(
				array(
					'message' => __( 'Not eligible.', 'beepbeep-ai-alt-text-generator' ),
					'code'    => 'bbai_jobs_not_eligible',
				),
				400
			);
		...
		if ( function_exists( 'bbai_telemetry_emit' ) ) {
			...
			if ( $bbai_ok > 0 ) {
				...
				if ( $uid > 0 && ! get_user_meta( $uid, 'bbai_telemetry_first_alt_at', true ) ) {
					...
					bbai_telemetry_emit( 'first_alt_generated', array( 'number_of_images' => $bbai_ok ) );
				}
			}
			if ( $bbai_fail > 0 ) {
				bbai_telemetry_emit(
	...
	private function get_guest_trial_missing_alt_count( bool $force_refresh = false ): int {
		...
		if ( is_array( $dashboard_stats ) ) {
			foreach ( array( 'images_missing_alt', 'missing', 'missing_alt' ) as $key ) {
	...
	private function generate_trial_single_with_retry( int $attachment_id, bool $quota_claimed = false, bool $regenerate = false ): array {
		...
		try {
			$result = $this->generate_and_save( $attachment_id, 'trial', 1, array(), $regenerate, true );
	...
	public function ajax_trial_demo_generate_batch() {
		...
		$auth_error_codes   = array( 'auth_required', 'user_not_found', 'trial_backend_auth', 'license_required' );
	...
	public function ajax_trial_generate_single() {
		...
		if ( 0 === $attachment_id || ! $this->is_image( $attachment_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid image.', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	...
	public function ajax_debug_logs() {
		...
		if ( ! $this->user_can_manage() ) {
			wp_send_json_error( array( 'message' => __( 'Unauthorized', 'beepbeep-ai-alt-text-generator' ) ), 403 );
			return;
		}

		if ( ! $this->can_show_debug_logs_tab() ) {
			wp_send_json_error( array( 'message' => __( 'Debug logs are not available for this account.', 'beepbeep-ai-alt-text-generator' ) ), 403 );
			return;
		}
	...
	public function ajax_get_contact_submissions() {
		...
		$allowed_orderby  = array( 'created_at', 'name', 'email', 'status' );
		...
		if ( isset( $_POST['id'] ) && ! empty( $_POST['id'] ) ) {
			...
			if ( $submission ) {
				...
				wp_send_json_error( array( 'message' => __( 'Submission not found', 'beepbeep-ai-alt-text-generator' ) ) );
				return;
			}
			return;
		}
	...
	public function ajax_update_contact_submission_status() {
		...
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid submission ID', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
		...
		if ( $result ) {
			wp_send_json_success( array( 'message' => __( 'Status updated successfully', 'beepbeep-ai-alt-text-generator' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to update status', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	}
	...
	public function ajax_delete_contact_submission() {
		...
		if ( $result ) {
			wp_send_json_success( array( 'message' => __( 'Submission deleted successfully', 'beepbeep-ai-alt-text-generator' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to delete submission', 'beepbeep-ai-alt-text-generator' ) ) );
			return;
		}
	}
	...
	public function ajax_export_analytics() {
		...
		if ( false === $with_alt_count ) {
			...
			// Rows
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Metric', 'Value' ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Total Images', $this->bbai_csv_safe_cell( $bbai_total_images ) ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Images with Alt Text', $this->bbai_csv_safe_cell( $with_alt_count ) ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Images Missing Alt Text', $this->bbai_csv_safe_cell( $missing_count ) ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Coverage Percentage', $this->bbai_csv_safe_cell( $bbai_coverage_percent . '%' ) ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Alt Texts Generated', $this->bbai_csv_safe_cell( $bbai_alt_texts_generated ) ) ) );
			$this->bbai_output_contents( $this->bbai_csv_row( array( 'Export Date', $this->bbai_csv_safe_cell( current_time( 'Y-m-d H:i:s' ) ) ) ) );
			exit;
	}
	...
	private function extract_alt_text_from_response( $api_response, &$source = null ): string {
		...
		$nested_keys = array( 'data', 'result', 'output', 'response' );
	...
	private function extract_alt_text_from_array( array $data, &$source = null ): string {
		$keys = array( 'altText', 'alt_text', 'alttext', 'alt', 'description', 'text' );
		foreach ( $keys as $key ) {
			if ( ! isset( $data[ $key ] ) ) {
				continue;
			}
