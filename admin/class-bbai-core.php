...
if (!defined('BBAI_PLUGIN_FILE')) {
    $plugin_file = defined('BEEPBEEP_AI_PLUGIN_FILE') && is_string(BEEPBEEP_AI_PLUGIN_FILE) ? BEEPBEEP_AI_PLUGIN_FILE : dirname(__FILE__, 2) . '/beepbeep-ai-alt-text-generator.php';
...
<?php elseif ($tab === 'settings') : ?>
            ...
            <div class="bbai-settings-page">
                ...
                if (isset($this->api_client)) {
                    ...
                    if (is_array($live) && !empty($live)) { Usage_Tracker::update_usage($live); }
                }
        ...
        // Default to USD
        $currency = ['symbol' => '$', 'code' => 'USD', 'pro' => 14.99, 'agency' => 59.99, 'credits' => 11.99];
        
        // UK/British locales - GBP (£)
        $uk_locales = ['en_gb', 'en_im', 'en_je', 'en_gg', 'en_uk', 'en_au', 'en_nz', 'en_za'];
        if (in_array($locale_lower, $uk_locales) || strpos($locale_lower, 'en_gb') !== false) {
            $currency = ['symbol' => '£', 'code' => 'GBP', 'pro' => 12.99, 'agency' => 49.99, 'credits' => 9.99];
        }
        // European locales - EUR (€)
            ...
            $currency = ['symbol' => '€', 'code' => 'EUR', 'pro' => 12.99, 'agency' => 49.99, 'credits' => 9.99];
        }
        // US/Canada locales - USD ($)
        else if (in_array($locale_lower, ['en_us', 'en_ca']) || ($locale_prefix === 'en' && !in_array($locale_lower, $uk_locales))) {
            $currency = ['symbol' => '$', 'code' => 'USD', 'pro' => 14.99, 'agency' => 59.99, 'credits' => 11.99];
        }
