...
class Core {
    ...
    /**
     * Whether this site looks like internal or staging traffic for PostHog segmentation.
     */
    private function is_posthog_internal_environment(): bool {
        if (defined('BEEPBEEP_AI_LOCAL_ALT_STUB') && BEEPBEEP_AI_LOCAL_ALT_STUB) {
            return true;
        }

        if (function_exists('wp_get_environment_type')) {
            $environment = sanitize_key((string) wp_get_environment_type());
        ...
        return '' !== $host && 1 === preg_match('/^(localhost|127\.0\.0\.1|.*\.local|.*\.test)(:\d+)?$/', $host);
    }
    ...
    private function maybe_emit_alt_generated_event(int $attachment_id, string $alt_text, string $source, string $model, array $context = [], array $response_meta = []): void {
        ...
        $properties = [
            ...
            'site_install_id'     => $identity_context['site_install_id'] ?? '',
        ...
        if (class_exists('\BeepBeepAI\AltTextGenerator\BBAI_Telemetry')) {
            ...
            if ( apply_filters( 'bbai_posthog_ai_observability_enabled', false, $properties ) ) {
                \BeepBeepAI\AltTextGenerator\BBAI_Telemetry::capture_posthog_event(
                    '$ai_generation',
                    $distinct_id,
                    array_merge(
                        $properties,
                        [
                            '$ai_trace_id' => 'bbai:' . $fingerprint,
                            '$ai_model'    => sanitize_text_field($model),
                            '$ai_provider' => 'openai',
                            '$ai_latency'  => isset($properties['generation_latency_ms']) ? (int) $properties['generation_latency_ms'] : 0,
                            'ai_feature'   => 'alt_text_generation',
                        ]
                    )
                );
            }
    ...
    private function get_default_debug_service_status(): array {
        return [
            'connection_status' => $this->has_debug_api_connection_signal() ? 'connected' : 'failed',
    ...
    /**
     * Logs can be empty immediately after install or after clearing them; that is not
     * an API failure. Use local auth/API configuration as a minimal connection signal.
     */
    private function has_debug_api_connection_signal(): bool {
        if (!isset($this->api_client) || !is_object($this->api_client)) {
            return false;
        }

        $has_auth = false;
        if (method_exists($this->api_client, 'has_active_license') && $this->api_client->has_active_license()) {
            $has_auth = true;
        } elseif (method_exists($this->api_client, 'get_token') && '' !== (string) $this->api_client->get_token()) {
            $has_auth = true;
        } elseif (method_exists($this->api_client, 'get_license_key') && '' !== (string) $this->api_client->get_license_key()) {
            $has_auth = true;
        }

        if (!$has_auth || !method_exists($this->api_client, 'get_api_url')) {
            return false;
        }

        $api_url = (string) $this->api_client->get_api_url();
    ...
    private function enrich_debug_payload(array $payload): array {
        ...
        if (
            is_array($service_status)
            && 'failed' === strtolower((string)($service_status['connection_status'] ?? ''))
            && empty($service_status['last_api_request'])
            && empty($service_status['last_api_error_message'])
            && $this->has_debug_api_connection_signal()
        ) {
            $service_status['connection_status'] = 'connected';
        }
    ...
    private function emit_lifecycle_telemetry( string $event_name, array $properties = [] ): void {
        $properties = array_merge(
            [
                ...
                'site_hash'      => $this->get_posthog_site_hash(),
                'transport'      => 'wp_plugin_lifecycle',
                'source'         => 'wordpress_plugin_lifecycle',
            ],
            $properties
        );
    ...
    private function is_posthog_forwarded_client_event( string $event_name ): bool {
        ...
        if ( null === $allowlist ) {
            $allowlist = array_fill_keys(
                [
                    'plugin_opened',
                    'dashboard_viewed',
                    'guest_dashboard_viewed',
                    'logged_out_dashboard_viewed',
                    'alt_library_viewed',
                    'analytics_viewed',
                    'usage_viewed',
                    'settings_viewed',
                    ...
                    'trial_exhausted',
                    'guest_trial_offer_shown',
                    'guest_trial_generate_clicked',
                    'guest_trial_completed',
                    'signup_prompt_shown',
                    'trial_completion_modal_shown',
                    'trial_complete_cta_clicked',
                    'signup_cta_clicked',
                    'login_cta_clicked',
                    'login_modal_opened',
                    'login_submitted',
                    ...
                    'alt_library_item_opened',
                    'alt_library_edit_started',
                    'alt_library_edit_saved',
                    'feature_used',
                    ...
                    'upgrade_started',
                    'low_credits_banner_shown',
                    'out_of_credits_banner_shown',
                    'needs_attention_banner_shown',
                    'milestone_banner_shown',
                    'trial_complete_state_shown',
                    'logged_out_conversion_state_shown',
                    'credits_exhausted',
                    'credits_low_warning_shown',
        ...
        return ! empty( $allowlist[ $event_name ] );
    }

    private function forward_client_telemetry_to_posthog( string $event_name, array $properties ): void {
        if ( ! class_exists( BBAI_Telemetry::class ) || ! $this->is_posthog_forwarded_client_event( $event_name ) ) {
            return;
        }
        ...
        $distinct_id = '';
        foreach ( [ 'account_id', 'user_id', 'license_id', 'site_install_id', 'site_id', 'site_hash' ] as $key ) {
            if ( ! empty( $properties[ $key ] ) && is_scalar( $properties[ $key ] ) ) {
                $distinct_id = sanitize_text_field( (string) $properties[ $key ] );
                break;
            }
        }
        if ( '' === $distinct_id ) {
            $distinct_id = $this->get_posthog_site_hash();
        }

        BBAI_Telemetry::capture_posthog_event(
            $event_name,
            $distinct_id,
            array_merge(
                [
                    'transport'      => 'wp_admin_server_bridge',
                    'plugin_version' => defined( 'BEEPBEEP_AI_VERSION' ) ? (string) BEEPBEEP_AI_VERSION : '',
                    'site_install_id' => $this->get_posthog_site_install_id(),
                    'site_hash'      => $this->get_posthog_site_hash(),
                ],
    ...
    public function deactivate(){
        $this->emit_lifecycle_telemetry( 'plugin_deactivated' );
    ...
    public function activate() {
        ...
        if ( class_exists( BBAI_Telemetry::class ) ) {
            BBAI_Telemetry::record_activation_lifecycle();
        }
            ...
            <div class="bbai-container">
                <div class="bbai-page-header bbai-mb-6">
                    <div class="bbai-page-header-content">
                        ...
                        <p class="bbai-page-subtitle"><?php esc_html_e('OpptiAI Alt Text will scan your WordPress media library and find images missing alt text.', 'beepbeep-ai-alt-text-generator'); ?></p>
                    </div>
...
<?php elseif ($bbai_tab === 'settings') : ?>
            ...
            <div class="bbai-footer">
                <?php esc_html_e('OpptiAI Alt Text • WordPress AI Tools', 'beepbeep-ai-alt-text-generator'); ?> — <a href="<?php echo esc_url('https://wordpress.org/plugins/beepbeep-ai-alt-text-generator/'); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('WordPress.org Plugin', 'beepbeep-ai-alt-text-generator'); ?></a>
            <?php else : ?>
    ...
    private function generate_alt_text_via_openai_vision( int $attachment_id, array $context, string $model, string $api_key ) {
        ...
        if ( ! empty( $opts['tone'] ) ) {
            /* translators: %s: selected alt text style */
            $hints[] = sprintf( __( 'Requested style: %s', 'beepbeep-ai-alt-text-generator' ), sanitize_text_field((string) $opts['tone']) );
        }
        if ( ! empty( $opts['custom_prompt'] ) ) {
            /* translators: %s: custom alt text instructions */
            $hints[] = sprintf( __( 'Additional instructions: %s', 'beepbeep-ai-alt-text-generator' ), wp_strip_all_tags((string) $opts['custom_prompt']) );
        }
    ...
    public function ajax_telemetry() {
        ...
        foreach ( array_slice( $decoded, 0, 25 ) as $row ) {
            ...
            $this->forward_client_telemetry_to_posthog( $name, $props );
        }
    ...
    public function ajax_create_portal() {
        ...
        if (is_wp_error($result)) {
            ...
            if ($result->get_error_code() === 'billing_not_linked') {
                wp_send_json_error([
                    'message' => __('This Growth membership is active, but it is managed manually and is not linked to Stripe billing. Contact support to downgrade or cancel this membership.', 'beepbeep-ai-alt-text-generator'),
        ...
        $portal_payload = is_array($result) ? ($result['data'] ?? $result) : [];
        $portal_data = is_array($portal_payload) && isset($portal_payload['data']) && is_array($portal_payload['data'])
            ? $portal_payload['data']
            : $portal_payload;

        if (is_array($result) && array_key_exists('success', $result) && !$result['success']) {
            $error_code = is_array($portal_payload) ? (string) ($portal_payload['code'] ?? $portal_payload['error_code'] ?? '') : '';
            $error_message = is_array($portal_payload) ? (string) ($portal_payload['message'] ?? $portal_payload['error'] ?? '') : '';
            if ('billing_not_linked' === $error_code) {
                wp_send_json_error([
                    'message' => __('This Growth membership is active, but it is managed manually and is not linked to Stripe billing. Contact support to downgrade or cancel this membership.', 'beepbeep-ai-alt-text-generator'),
                    'code' => 'billing_not_linked',
                ]);
            ...
            wp_send_json_error([
                'message' => $error_message ?: __('Unable to open the billing portal. Please try again or contact support.', 'beepbeep-ai-alt-text-generator'),
                'code' => $error_code ?: 'billing_portal_unavailable',
            ]);
            return;
        }

        $portal_url = is_array($portal_data) ? (string) ($portal_data['url'] ?? '') : '';
        if ('' === $portal_url) {
            wp_send_json_error([
                'message' => __('Unable to open the billing portal. Please try again or contact support.', 'beepbeep-ai-alt-text-generator'),
    ...
    public function ajax_send_contact_form() {
        ...
        $submission_id = \BeepBeepAI\AltTextGenerator\Contact_Submissions::save_submission($contact_data);

        $debug_attachment = $this->create_contact_debug_attachment($contact_data);
        $mail_result = $this->send_contact_support_email_via_backend($contact_data, $debug_attachment);
        if (is_wp_error($mail_result)) {
            $mail_result = $this->send_contact_support_email_via_resend($contact_data, $debug_attachment, $mail_result);
        }
        if (is_wp_error($mail_result)) {
            $mail_result = $this->send_contact_support_email($contact_data, $debug_attachment);
        }
        if ($debug_attachment && file_exists($debug_attachment)) {
            wp_delete_file($debug_attachment);
        }

        if (is_wp_error($mail_result)) {
            if (class_exists('\BeepBeepAI\AltTextGenerator\Debug_Log')) {
                \BeepBeepAI\AltTextGenerator\Debug_Log::log('warning', 'Contact form email delivery failed after local save', [
            ...
            $mail_result = [
                'success' => true,
                'message' => __('Your message has been saved. This WordPress site could not send email automatically, so please email benoats86@gmail.com if the issue is urgent.', 'beepbeep-ai-alt-text-generator'),
            ];
        }

        $last_contacted_at = current_time('mysql');
        update_user_meta($user_id, 'bbai_last_contacted_support_at', $last_contacted_at);
        ...
        // Success
        wp_send_json_success([
            'message' => !empty($mail_result['message'])
                ? $mail_result['message']
                : __('Your message has been sent successfully. Debug logs were attached automatically.', 'beepbeep-ai-alt-text-generator'),
            'last_contacted_support' => [
                'raw' => $last_contacted_at,
                'label' => $this->format_last_contacted_support_label($last_contacted_at),
            ],
        ]);
    }

    private function format_last_contacted_support_label($timestamp) {
        ...
        $relative = human_time_diff($time, current_time('timestamp')) . ' ' . __('ago', 'beepbeep-ai-alt-text-generator');

        return sprintf(
            /* translators: 1: absolute date/time, 2: relative time, for example "5 minutes ago". */
            __('Last contacted support: %1$s (%2$s)', 'beepbeep-ai-alt-text-generator'),
            $absolute,
            $relative
        );
    }

    private function get_contact_support_recipient() {
        return apply_filters('bbai_contact_support_recipient', 'benoats86@gmail.com');
    }

    private function send_contact_support_email($contact_data, $debug_attachment = '') {
        $recipient = sanitize_email($this->get_contact_support_recipient());
        if (!$recipient || !is_email($recipient)) {
            return new \WP_Error('bbai_support_recipient_invalid', __('Support email recipient is not configured.', 'beepbeep-ai-alt-text-generator'));
        }

        $reply_to = sanitize_email($contact_data['email'] ?? '');
        $name = sanitize_text_field($contact_data['name'] ?? '');
        $subject = sanitize_text_field($contact_data['subject'] ?? __('Support request', 'beepbeep-ai-alt-text-generator'));
        $site_url = esc_url_raw($contact_data['site_url'] ?? get_site_url());
        $license_hash = !empty($contact_data['license_key']) ? wp_hash((string) $contact_data['license_key']) : '';

        $body = '<h2>OpptiAI Alt Text support request</h2>';
        $body .= '<p><strong>Name:</strong> ' . esc_html($name) . '</p>';
        $body .= '<p><strong>Email:</strong> ' . esc_html($reply_to) . '</p>';
        $body .= '<p><strong>Site:</strong> ' . esc_html($site_url) . '</p>';
        $body .= '<p><strong>WordPress:</strong> ' . esc_html($contact_data['wp_version'] ?? get_bloginfo('version')) . '</p>';
        $body .= '<p><strong>Plugin:</strong> ' . esc_html($contact_data['plugin_version'] ?? BEEPBEEP_AI_VERSION) . '</p>';
        if ($license_hash) {
            $body .= '<p><strong>License hash:</strong> ' . esc_html($license_hash) . '</p>';
        }
        $body .= '<h3>Message</h3>';
        $body .= '<p>' . nl2br(esc_html($contact_data['message'] ?? '')) . '</p>';
        $body .= '<p>Recent debug logs are attached when available.</p>';
        ...
        $attachments = [];
        if ($debug_attachment && file_exists($debug_attachment)) {
            $attachments[] = $debug_attachment;
        }

        $sent = wp_mail(
            $recipient,
            '[OpptiAI Alt Text Support] ' . $subject,
            $body,
            $headers,
            $attachments
        );
        ...
        if (!$sent) {
            return new \WP_Error('bbai_support_mail_failed', __('Could not send the support email. Please check WordPress mail delivery and try again.', 'beepbeep-ai-alt-text-generator'));
        }

        return ['success' => true];
    }

    private function send_contact_support_email_via_backend($contact_data, $debug_attachment = '', $previous_error = null) {
        if (!is_object($this->api_client) || !method_exists($this->api_client, 'send_contact_email')) {
            return is_wp_error($previous_error)
                ? $previous_error
                : new \WP_Error('bbai_support_backend_unavailable', __('Support email delivery is unavailable. Please try again later.', 'beepbeep-ai-alt-text-generator'));
        }

        $payload = $this->prepare_contact_payload_with_debug_excerpt($contact_data, $debug_attachment);

        $result = $this->api_client->send_contact_email($payload);
        if (is_wp_error($result)) {
            $previous_message = is_wp_error($previous_error) ? $previous_error->get_error_message() : '';
            return new \WP_Error(
                'bbai_support_mail_failed',
                $previous_message
                    ? sprintf(
                        /* translators: 1: WordPress mail failure, 2: backend mail failure. */
                        __('Could not send the support email. WordPress mail failed: %1$s Backend mail failed: %2$s', 'beepbeep-ai-alt-text-generator'),
                        $previous_message,
        ...
        return [
            'success' => true,
            'message' => __('Your message has been sent successfully. Recent debug logs were included automatically.', 'beepbeep-ai-alt-text-generator'),
        ];
    }

    private function send_contact_support_email_via_resend($contact_data, $debug_attachment = '', $previous_error = null) {
        $resend_path = BEEPBEEP_AI_PLUGIN_DIR . 'includes/services/class-resend-service.php';
        if (!class_exists('\BeepBeepAI\AltTextGenerator\Resend_Service') && file_exists($resend_path)) {
            require_once $resend_path;
        }

        if (!class_exists('\BeepBeepAI\AltTextGenerator\Resend_Service')) {
            return is_wp_error($previous_error)
                ? $previous_error
                : new \WP_Error('bbai_support_resend_unavailable', __('Support email delivery is unavailable. Please try again later.', 'beepbeep-ai-alt-text-generator'));
        }

        $payload = $this->prepare_contact_payload_with_debug_excerpt($contact_data, $debug_attachment);
        $service = new \BeepBeepAI\AltTextGenerator\Resend_Service();
        $result = $service->send_contact_email($payload);

        if (is_wp_error($result)) {
            $previous_message = is_wp_error($previous_error) ? $previous_error->get_error_message() : '';
            return new \WP_Error(
                'bbai_support_resend_failed',
                $previous_message
                    ? sprintf(
                        /* translators: 1: previous mail failure, 2: Resend failure. */
                        __('Support email delivery failed: %1$s Resend failed: %2$s', 'beepbeep-ai-alt-text-generator'),
                        $previous_message,
                        $result->get_error_message()
                    )
                    : $result->get_error_message()
    ...
    private function prepare_contact_payload_with_debug_excerpt($contact_data, $debug_attachment = '') {
        $payload = $contact_data;
        $debug_excerpt = $this->read_contact_debug_attachment_excerpt($debug_attachment);
        if ($debug_excerpt) {
            $payload['message'] = rtrim((string) ($payload['message'] ?? ''))
                . "\n\n--- Recent OpptiAI Alt Text debug log ---\n"
                . $debug_excerpt;
        }
    ...
    private function read_contact_debug_attachment_excerpt($debug_attachment) {
        if (!$debug_attachment || !file_exists($debug_attachment) || !is_readable($debug_attachment)) {
            return '';
        }

        $contents = file_get_contents($debug_attachment);
        ...
        $max_length = 20000;
        if (strlen($contents) > $max_length) {
            $contents = substr($contents, 0, $max_length) . "\n[debug log truncated]";
        }

        return $contents;
    }

    private function create_contact_debug_attachment($contact_data) {
        if (!class_exists('\BeepBeepAI\AltTextGenerator\Debug_Log')) {
            require_once BEEPBEEP_AI_PLUGIN_DIR . 'includes/class-debug-log.php';
        }

        $logs_payload = class_exists('\BeepBeepAI\AltTextGenerator\Debug_Log')
            ? \BeepBeepAI\AltTextGenerator\Debug_Log::get_logs(['per_page' => 100, 'page' => 1])
            : ['logs' => []];
        $logs = is_array($logs_payload) && isset($logs_payload['logs']) && is_array($logs_payload['logs'])
            ? $logs_payload['logs']
            : [];

        $lines = [];
        $lines[] = 'OpptiAI Alt Text Support Debug Log';
        $lines[] = 'Generated: ' . gmdate('c');
        $lines[] = 'Site: ' . get_site_url();
        $lines[] = 'WordPress: ' . get_bloginfo('version');
        $lines[] = 'Plugin: ' . BEEPBEEP_AI_VERSION;
        $lines[] = 'PHP: ' . PHP_VERSION;
        $lines[] = 'User ID: ' . get_current_user_id();
        $lines[] = 'Contact email: ' . sanitize_email($contact_data['email'] ?? '');
        $lines[] = str_repeat('-', 72);

        if (empty($logs)) {
            $lines[] = 'No debug log entries found.';
        } else {
            foreach ($logs as $log) {
                $lines[] = sprintf(
                    '[%s] %s %s: %s',
                    sanitize_text_field($log['timestamp'] ?? $log['created_at'] ?? ''),
                    strtoupper(sanitize_key($log['level'] ?? 'info')),
                    sanitize_text_field($log['source'] ?? 'core'),
                    sanitize_text_field($log['message'] ?? '')
                );
                if (!empty($log['context']) && is_array($log['context'])) {
                    $lines[] = wp_json_encode($log['context'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
                }
                $lines[] = '';
            }
        }

        $path = wp_tempnam('bbai-debug-log-' . gmdate('Ymd-His') . '.txt');
        if (!$path) {
            return '';
        }
