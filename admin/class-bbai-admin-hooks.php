...
class Admin_Hooks {
	...
	public function register() {
		add_filter( 'admin_body_class', array( $this->core, 'filter_admin_body_class' ) );
		...
		add_action( 'admin_init', array( __CLASS__, 'maybe_clear_usage_cache' ) );
		...
		add_action( 'admin_post_bbai_usage_export', array( $this->core, 'handle_usage_export' ) );
		add_action( 'admin_post_bbai_debug_export', array( $this->core, 'handle_debug_log_export' ) );
	...
	public static function maybe_clear_usage_cache() {
		...
		if ( ! in_array( $clear_cache_input, array( '1', 'true', 'yes' ), true ) ) {
			return;
		}
