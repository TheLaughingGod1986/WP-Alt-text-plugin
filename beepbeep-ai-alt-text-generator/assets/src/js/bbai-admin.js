...
(function($) {
    ...
    function handleGenerateMissing(e) {
        ...
        if (usageStats && !isPremium && !hasQuota && config.restUsage) {
            ...
            var refreshUsagePromise = $.ajax({
                ...
                if (response && response.success && response.data) {
                    ...
                    var freshRemaining = freshUsage.remaining !== undefined ? parseInt(freshUsage.remaining, 10) : 0;
                    var freshPlan = freshUsage.plan ? freshUsage.plan.toLowerCase() : 'free';
                    var freshIsPremium = freshPlan === 'pro' || freshPlan === 'agency';
                    var freshHasQuota = freshRemaining > 0;
                    
                    if (!freshIsPremium && !freshHasQuota) {
            ...
            refreshUsagePromise.done(function(shouldContinue) {
                ...
                if (shouldContinue === null) {
                    // Refresh failed, use cached check
                    handleLimitReached({
                        message: 'Monthly limit reached. Upgrade to continue generating alt text.',
                        code: 'limit_reached',
                        usage: usageStats
        ...
        } else if (usageStats && !isPremium && !hasQuota) {
