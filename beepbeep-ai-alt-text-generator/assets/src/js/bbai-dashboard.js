...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '[data-action="show-upgrade-modal"]', function(e) {
            ...
            if (typeof bbaiApp && typeof bbaiApp.showModal === 'function') {
                bbaiApp.showModal();
            } else if (typeof alttextaiShowModal === 'function') {
                alttextaiShowModal(); // Legacy fallback
            } else {
                console.error('[AltText AI] bbaiApp.showModal function not available!');
                // Direct fallback
    ...
    function loadSubscriptionInfo(forceRefresh) {
        ...
        $.ajax({
            ...
            type: 'POST',
                data: {
                action: 'bbai_get_subscription_info',
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data && response.data.url) {
                    ...
                    // Note: Authentication is no longer required for checkout
                    // Users can create account during checkout, so don't show login modal
