(function($) {
    ...
    var QueueMonitor = {
        ...
        cacheDom: function() {
            this.$card = $('.alttextai-queue-card');
        ...
        renderFailures: function(items) {
            ...
            items.forEach(function(job) {
                ...
                var $actions = $('<div/>', { 'class': 'alttextai-queue-item__meta' });
                $('<button/>', {
                    type: 'button',
                    'class': 'alttextai-queue-btn alttextai-queue-btn--inline',
        ...
        buildQueueItem: function(job, isFailure) {
            ...
            var $item = $('<div/>', {
                'class': 'alttextai-queue-item' + (status === 'failed' ? ' alttextai-queue-item--failed' : '')
            });

            var title = job.attachment_title || 'Image #' + job.attachment_id;
            var $title = $('<p/>', { 'class': 'alttextai-queue-item__title' });

            if (job.edit_url) {
                $('<a/>', {
                    href: job.edit_url,
                    'class': 'alttextai-queue-item__link',
                    text: title
                }).appendTo($title);
            } else {
                $title.text(title);
            ...
            if (metaParts.length) {
                var $meta = $('<div/>', { 'class': 'alttextai-queue-item__meta' });
            ...
            if ((isFailure || status === 'failed') && job.last_error) {
                $('<p/>', {
                    'class': 'alttextai-queue-item__error',
                    text: job.last_error
                }).appendTo($item);
            }
        ...
        reveal: function() {
            ...
            this.$card.addClass('alttextai-queue--flash');
            var self = this;
            setTimeout(function() {
                self.$card.removeClass('alttextai-queue--flash');
            }, 1200);
        },

        setLoading: function(state) {
            this.isLoading = state;
            this.$card.toggleClass('alttextai-queue--loading', !!state);
        ...
        getAjaxUrl: function() {
            if (window.alttextai_ajax) {
                if (window.alttextai_ajax.ajax_url) {
                    return window.alttextai_ajax.ajax_url;
                }
                if (window.alttextai_ajax.ajaxurl) {
                    return window.alttextai_ajax.ajaxurl;
                }
            }
        ...
        showNotification: function(message, type) {
            type = type || 'info';
            var $notice = $('<div/>', {
                'class': 'notice notice-' + type + ' is-dismissible alttextai-queue-notice'
            }).append(
                $('<p/>').text(message)
            );
    ...
    window.AltTextAIQueueMonitor = QueueMonitor;
