...
(function($) {
    ...
    const config = window.OPPTIAI_ALT_DEBUG || {};
    const $panel = $('[data-alttextai-debug-panel]');
    ...
    function addVanillaContextHandlers() {
        ...
        document.addEventListener('click', function(e) {
            ...
            const panel = document.querySelector('[data-alttextai-debug-panel]');
            if (!panel) {
    ...
    function bindEvents() {
        ...
        $panel.on('click', '.alttextai-debug-context__close', function(e) {
            e.preventDefault();
            e.stopPropagation();
            hideContext();
        });
        ...
        // Also bind to .alttextai-debug-context directly
        $panel.on('click', '.alttextai-debug-context', function(event) {
            if (event.target === this) {
                hideContext();
            }
        });
    ...
    function renderRows(logs) {
        ...
        if (!logs.length) {
            $tbody.html('<tr class="alttextai-debug-table__empty"><td colspan="4" style="text-align: center; padding: 60px 24px; color: #6b7280; font-size: 14px;">' + escapeHtml(strings.noLogs) + '</td></tr>');
            return;
        }
        ...
        const rows = logs.map(function(log, index) {
            ...
            const rowIndex = String(index);
            const rowHtml = '<tr class="alttextai-debug-table-row" style="border-bottom: 1px solid #f3f4f6; transition: background 0.15s;">' +
            ...
            if (log.context && Object.keys(log.context).length) {
                return rowHtml + '<tr class="alttextai-debug-context-row" data-row-index="' + escapeAttr(rowIndex) + '" style="display: none;"><td colspan="4" class="alttextai-debug-context-cell"><div class="alttextai-debug-context-content"><pre class="alttextai-debug-context-json"></pre></div></td></tr>';
            }
    ...
    function hideContext() {
        ...
        var $modalDirect = $panel.find('.alttextai-debug-context');
