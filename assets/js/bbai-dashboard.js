...
function initCountdownTimer() {
    ...
    function updateCountdown() {
        ...
        countdownElement.setAttribute('data-countdown', remaining.toString());
    }
    ...
    // Keep countdown accurate when the tab becomes visible again.
    ...
    // This UI only displays days/hours/minutes (no seconds). Updating every second
    // causes unnecessary DOM churn in wp-admin. Refresh every 30s, and pause when
    // the tab is hidden.
...
bbaiRunWithJQuery(function($) {
    ...
    function syncStatsToRoot(stats) {
        ...
        // If SSR/local scan already populated counts, treat them as canonical (matches ALT Library).
        // Do not allow remote stats/truth payloads to overwrite them.
        var preserveLocalCounts = root.getAttribute('data-bbai-has-scan-results') === '1'
            || currentTotal > 0
            || currentMissing > 0
            || currentWeak > 0
            || currentOptimized > 0;
        ...
        // If we've already run a local coverage scan (same source as ALT Library),
        // do not overwrite the dashboard's root coverage counts with remote stats.
    ...
    function getAccessibilityImpact(data) {
        ...
        impact.altText = sprintf(
            __('Shareable accessibility badge showing a %1$s%% accessibility score and %2$s optimized images powered by OpptiAI Alt Text', 'beepbeep-ai-alt-text-generator'),
        ...
        impact.shareText = sprintf(
            __('My WordPress site has an accessibility score of %1$s%% with %2$s optimized images powered by OpptiAI Alt Text.', 'beepbeep-ai-alt-text-generator'),
            impact.coverageLabel,
            impact.optimizedLabel
        );
    ...
    function buildAccessibilityBadgeSvg(impact) {
        var titleText = __('Accessibility badge powered by OpptiAI Alt Text', 'beepbeep-ai-alt-text-generator');
        ...
        return [
            ...
            '<text x="36" y="196" fill="#166534" font-family="Segoe UI, Arial, sans-serif" font-size="15" font-weight="600">', escapeSvgText(__('Powered by OpptiAI Alt Text', 'beepbeep-ai-alt-text-generator')), '</text>',
            '</svg>'
        ].join('');
    }
            ...
            if (upgradeLeadNode) {
                upgradeLeadNode.textContent = planRemaining <= 0
                    ...
                    : (String(data.quotaState || '').toLowerCase() === 'near_limit'
                        ...
                        : __('Try OpptiAI Alt Text before creating an account', 'beepbeep-ai-alt-text-generator'));
            }
            if (upgradeSubNode) {
