...
(function ($) {
    ...
    var posthogAllowlist = {
        ...
        guest_trial_offer_shown: true,
        guest_trial_generate_clicked: true,
        guest_trial_completed: true,
        signup_prompt_shown: true,
        trial_completion_modal_shown: true,
        trial_complete_cta_clicked: true,
    ...
    function baseProps() {
        ...
        return $.extend({
            ...
            auth_state: c.is_logged_in === true ? 'authenticated' : 'anonymous',
            account_state: c.is_logged_in === true ? 'connected_account' : 'anonymous_trial',
            is_signed_in: c.is_logged_in === true,
            is_saas_authenticated: c.is_logged_in === true,
            quota_type: c.is_logged_in === true ? 'monthly_account' : 'trial',
    ...
    function inferGenerationFailureEvent(props) {
        ...
        props.error_code = 'internal_unknown_error';
    ...
    function normalizeTelemetryEvent(eventName, props) {
        ...
        } else if (name === 'alt_generated_failed' || name === 'generation_failed_unknown') {
            name = 'generation_failed';
            payload.error_code = normalizeFailureCode(payload.error_code || 'internal_unknown_error');
            payload.provider = payload.provider || 'unknown';
    ...
    function track(eventName, properties) {
        ...
        var isGenerationEvent = eventName === 'generation_started' ||
            eventName === 'generation_completed' ||
            eventName === 'alt_generated' ||
            eventName === 'generation_failed' ||
            eventName.indexOf('generation_failed_') === 0 ||
            eventName.indexOf('batch_generation_') === 0;

        if (isGenerationEvent) {
            var anonymousGeneration = props.auth_state === 'anonymous' ||
                props.user_state === 'guest' ||
                props.is_signed_in === false ||
                props.is_logged_in === false;
            props.auth_state = anonymousGeneration ? 'anonymous' : 'authenticated';
            props.account_state = anonymousGeneration ? 'anonymous_trial' : 'connected_account';
            props.is_signed_in = !anonymousGeneration;
            props.is_saas_authenticated = !anonymousGeneration;
            props.is_anonymous_generation = anonymousGeneration;
            props.generation_actor = anonymousGeneration ? 'anonymous_guest' : 'signed_in_account';
            props.quota_type = anonymousGeneration ? 'trial' : (props.quota_type || 'monthly_account');
        }
        ...
        if (eventName === 'generation_completed' || eventName === 'generation_failed' || eventName.indexOf('generation_failed_') === 0) {
        ...
        if (eventName === 'generation_started' || eventName === 'generation_completed' || eventName === 'alt_generated' || eventName === 'generation_failed' || eventName.indexOf('generation_failed_') === 0 || eventName.indexOf('batch_generation_') === 0) {
