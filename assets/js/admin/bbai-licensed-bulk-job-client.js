...
(function (window, $) {
    ...
    var MAX_ACTIVE_JOB_AGE_MS = 6 * 60 * 60 * 1000;
    var MAX_ACTIVE_JOB_HEARTBEAT_AGE_MS = 2 * 60 * 1000;
    ...
    function getJobCount(job, keys) {
        var i;
        var value;
        var parsed;

        if (!job || typeof job !== 'object' || !Array.isArray(keys)) {
            return 0;
        }
    ...
    function mergeJobAggregateCounts(job, itemCounts) {
        var counts = itemCounts && typeof itemCounts === 'object' ? itemCounts : {};
        return {
            completed: Math.max(
                0,
                parseInt(counts.completed, 10) || 0,
                getJobCount(job, ['completed', 'completed_count', 'successes', 'success_count', 'succeeded_count'])
            ),
            failed: Math.max(
                0,
                parseInt(counts.failed, 10) || 0,
                getJobCount(job, ['failed', 'failed_count', 'failures', 'failure_count'])
            )
        };
    }
    ...
    function isRecoverableActiveJob(active) {
        ...
        var heartbeat = parseInt(active.updatedAt || active.last_checked_at || active.startedAt || '0', 10) || 0;
        if (started && Date.now() - started > MAX_ACTIVE_JOB_AGE_MS) {
            return false;
        }
        if (heartbeat && Date.now() - heartbeat > MAX_ACTIVE_JOB_HEARTBEAT_AGE_MS) {
            return false;
        }
        return true;
    }

    function clearStaleRecoveredUi(active) {
        var heartbeat = active ? (parseInt(active.updatedAt || active.last_checked_at || active.startedAt || '0', 10) || 0) : 0;
        if (active && heartbeat && Date.now() - heartbeat <= MAX_ACTIVE_JOB_HEARTBEAT_AGE_MS) {
            return;
        }
        clearActiveJob();
        if (window.bbaiJobState && typeof window.bbaiJobState.getState === 'function') {
        ...
        function pollRecovered() {
            ...
            $.ajax({
                ...
                .done(function (pollRes) {
                    ...
                    var counts = mergeJobAggregateCounts(job, mapJobItemsToLibraryRows(items, null));
                    ...
                    if (isTerminalStatus(job.status)) {
                        ...
                        if (window.bbaiJobState) {
                            window.bbaiJobState.complete({
                                status: counts.failed > 0 ? (counts.completed > 0 ? 'partial' : 'error') : 'complete',
                                successes: counts.completed,
                                failures: counts.failed,
                                skipped: 0
                            });
