...
(function($) {
    ...
    function formatDashboardHeroCreditContext(rem, heroLiState) {
        ...
        if (rem <= 10) {
            low = __('Running low — upgrade or top up before you run out.', 'beepbeep-ai-alt-text-generator');
        }
        switch (state) {
            case 'MISSING_ALT':
            case 'MIXED_ATTENTION':
                hint = __('Manual generation uses credits.', 'beepbeep-ai-alt-text-generator');
                break;
            case 'NEEDS_REVIEW':
                hint = __('Reviewing does not use credits.', 'beepbeep-ai-alt-text-generator');
                break;
            case 'ALL_CLEAR':
                hint = __('Credits are ready for your next uploads.', 'beepbeep-ai-alt-text-generator');
                break;
            case 'PROCESSING':
            case 'QUEUED':
                hint = __('Manual generation uses credits.', 'beepbeep-ai-alt-text-generator');
        ...
        return (hint ? hint : '') + (hint && low ? ' ' : '') + (low ? low : '');
    }

    function formatDashboardHeroCreditHelper(rem) {
        rem = Math.max(0, parseInt(rem, 10) || 0);
        if (rem === 0) {
            return __('Add credits to continue generating ALT text.', 'beepbeep-ai-alt-text-generator');
        }
        return '';
    }
    ...
    function updateGlobalCreditsSurfacesFromCounts(used, limit, remaining) {
        var rem = Math.max(0, parseInt(remaining, 10) || 0);
        ...
        if (heroUsage) {
            ...
            heroUsage.classList.add('bbai-credit-usage--just-updated');
            window.setTimeout(function() {
                heroUsage.classList.remove('bbai-credit-usage--just-updated');
            }, 700);
            ...
            if (helper) {
                var helperText;
                if (rem === 0) {
                    helperText = formatDashboardHeroCreditHelper(rem);
                } else {
                    helperText = '';
                }
                if (helperText) {
    ...
    function coerceLoggedInDashboardStateData(stateData) {
        ...
        if (nextState === 'MIXED_ATTENTION') {
            ...
            heroData.support = __('Generate missing ALT text first, then review the suggested descriptions before they go live.', 'beepbeep-ai-alt-text-generator');
            heroData.primary_cta = { label: __('Generate missing ALT text', 'beepbeep-ai-alt-text-generator'), busy_label: __('Starting...', 'beepbeep-ai-alt-text-generator'), action: 'generate-missing', href: '#' };
            heroData.secondary_cta = { label: sprintf(_n('Review %s image', 'Review %s images', counts.review, 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(counts.review)), action: 'navigate', href: getDashboardReviewLibraryHref() };
            ...
            heroData.primary_cta = { label: __('Generate missing ALT text', 'beepbeep-ai-alt-text-generator'), busy_label: __('Starting...', 'beepbeep-ai-alt-text-generator'), action: 'generate-missing', href: '#' };
            heroData.secondary_cta = { label: __('Open ALT Library', 'beepbeep-ai-alt-text-generator'), action: 'navigate', href: getDashboardLibraryHref() };
    ...
    function updateLoggedInHeroDonut(hero, stateData) {
        ...
        var actionLabel = stateId === 'NEEDS_REVIEW'
        ...
        if (reviewBlock && reviewLinkNode && reviewPromptNode) {
            if (stateId === 'NEEDS_REVIEW' && secondary && secondary.href) {
