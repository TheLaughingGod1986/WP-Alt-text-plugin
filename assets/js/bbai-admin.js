...
(function($) {
    ...
    var bbaiDashboardApproveAllInFlight = false;
    ...
    // Global generation lock (prevents duplicate jobs / double clicks).
    ...
    window.bbaiGenerationInProgress = window.bbaiGenerationInProgress || false; // backwards compat
    ...
    function bbaiAcquireGenerationLock(event, source, jobId) {
        ...
        bbaiApplyGenerationLockUI();
        return true;
    }

    function bbaiApplyGenerationLockUI() {
        var on = bbaiIsGenerationLocked();
        var note = document.querySelector('[data-bbai-gen-running-note="1"]');
        if (note) {
        ...
        // Mark nodes as locked for styling / analytics if needed.
        var lockedNodes = Array.prototype.slice.call(document.querySelectorAll(
            [
                '[data-bbai-generation-action]',
        ...
        lockedNodes.forEach(function(node) {
            ...
            node.setAttribute('data-bbai-generation-locked', '1');
            node.setAttribute('data-bbai-generation-action', '1');
            node.classList && node.classList.add('is-disabled', 'bbai-generate-button');
        });
        ...
        bbaiRestoreGenerationDisabled = function() {
            ...
            lockedNodes.forEach(function(node) {
                ...
                node.removeAttribute('data-bbai-generation-locked');
                node.classList && node.classList.remove('is-disabled');
            });
        };
    }

    function bbaiReleaseGenerationLockUI() {
    ...
    function bbaiClearGenerationLock() {
        ...
        bbaiReleaseGenerationLockUI();
    }

    function setGenerationInProgress(nextValue) {
        if (nextValue) {
            bbaiSetGenerationLock('unknown', null);
        } else {
    ...
    // apply the generation lock immediately so CTAs are disabled until the job ends.
    if (document && document.querySelector) {
        var liveProgress = document.querySelector('[data-bbai-li-live-progress="1"]');
        var bulkModalActive = document.querySelector('#bbai-bulk-progress-modal.active');
        if (liveProgress || bulkModalActive) {
            bbaiSetGenerationLock && bbaiSetGenerationLock('bootstrap_detected_live_progress', null);
        }
    }

    function markDashboardMainHydrated() {
        var root = document.getElementById('bbai-dashboard-main');
        if (!root || root.getAttribute('data-hydrated') === 'true') {
            return;
        }
        root.setAttribute('data-hydrated', 'true');
    }
    window.bbaiMarkDashboardMainHydrated = markDashboardMainHydrated;
    ...
    function readCreditLimitFromDashboardDom() {
        var root = document.querySelector('[data-bbai-dashboard-state-root="1"]');
        if (!root) {
            return NaN;
        }
        var lim = parseInt(root.getAttribute('data-bbai-credits-total'), 10);
        if (!isNaN(lim) && lim > 0) {
            return lim;
        }
        var offer = parseInt(root.getAttribute('data-bbai-free-plan-offer'), 10);
    ...
    function readFreePlanOfferFromDashboardDom() {
        var root = document.querySelector('[data-bbai-dashboard-state-root="1"]');
        if (!root) {
            return NaN;
        }
        var v = parseInt(root.getAttribute('data-bbai-free-plan-offer'), 10);
    ...
    function normalizeUsagePayload(rawUsage) {
        ...
        if (isNaN(limit) || limit <= 0) {
            limit = readCreditLimitFromDashboardDom();
        }
        ...
        if (isNaN(lowCreditThreshold)) {
            lowCreditThreshold = quotaType === 'trial'
                ? (limit > 0 ? Math.min(2, Math.max(1, limit - 1)) : 1)
        ...
        freePlanOffer = isNaN(freePlanOffer) ? readFreePlanOfferFromDashboardDom() : freePlanOffer;
        freePlanOffer = isNaN(freePlanOffer) ? 0 : Math.max(0, parseInt(freePlanOffer, 10));
    ...
    var bbaiDashboardBackendGenerationActive = false;

    function normalizeBackendGenerationStatus(status) {
        status = String(status || '').toLowerCase();
        if (status === 'queued' || status === 'queue' || status === 'pending' || status === 'scheduled' || status === 'waiting') {
            return 'queued';
        }
        if (status === 'processing' || status === 'running' || status === 'in_progress' || status === 'started' || status === 'working') {
            return 'processing';
        }
        return status;
    }

    function responseHasActiveBackendGenerationJob(responseData) {
        var job = responseData && (responseData.active_job || responseData.job || null);
        var status = job
            ? normalizeBackendGenerationStatus(job.status || job.state || '')
            : normalizeBackendGenerationStatus(responseData && (responseData.job_status || responseData.job_state));

        if (status !== 'queued' && status !== 'processing') {
            return false;
        }

        return !job || job.active === undefined || job.active === true || job.active === '1' || job.active === 1;
    }

    function syncDashboardRuntimeStateFromBackendJob(responseData) {
        bbaiDashboardBackendGenerationActive = responseHasActiveBackendGenerationJob(responseData);
        setDashboardRuntimeState(bbaiDashboardBackendGenerationActive ? 'generation_running' : 'idle');
        return bbaiDashboardBackendGenerationActive;
    }

    function setDashboardRuntimeState(runtimeState) {
        var nextRuntimeState = runtimeState || 'idle';

        if (nextRuntimeState === 'generation_running' && !bbaiDashboardBackendGenerationActive) {
            nextRuntimeState = 'generation_starting';
        }

        if (nextRuntimeState !== 'generation_running') {
            bbaiDashboardBackendGenerationActive = false;
        }
    ...
    function buildTrialExhaustedMessage() {
        return sprintf(
            ...
            Math.max(1, getFreeAccountMonthlyLimit() || readCreditLimitFromDashboardDom() || 1)
        );
    }
    ...
    /**
     * Free, connected (non-guest) dashboard: soft upgrade nudges (generate click, etc.).
     *
     * @return {boolean}
     */
    function isSoftUpgradeNudgeEligible() {
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        if (!root) {
            return false;
        }
        if (root.getAttribute('data-bbai-is-premium') === '1') {
            return false;
        }
        if (root.getAttribute('data-bbai-is-guest-trial') === '1') {
            return false;
        }
        if (root.getAttribute('data-bbai-signup-required') === '1') {
            return false;
        }
        return root.getAttribute('data-bbai-has-connected-account') === '1';
    }

    function isBulkProgressModalActive() {
        var bulk = document.getElementById('bbai-bulk-progress-modal');

        return !!(bulk &&
            (bulk.classList.contains('active') || String(bulk.style.display || '').toLowerCase() === 'flex'));
    }
    ...
    function applyDashboardState(forcePreviewRefresh) {
        ...
        // Telemetry: trial-complete conversion state impression (once per page view/render).
        try {
            var root = document.querySelector('[data-bbai-dashboard-root="1"]');
            if (root && root.getAttribute('data-bbai-auth-mode') === 'guest_logged_out' && root.getAttribute('data-bbai-trial-exhausted') === '1') {
                window.bbaiTelemetrySeen = window.bbaiTelemetrySeen || new Set();
                var seenKey = 'trial_complete_state_shown:' + String(root.getAttribute('data-bbai-counts-hash') || 'v1');
                if (!window.bbaiTelemetrySeen.has(seenKey)) {
                    window.bbaiTelemetrySeen.add(seenKey);
                    dispatchAnalyticsEvent('trial_complete_state_shown', {
                        trial_used: parseInt(root.getAttribute('data-bbai-trial-used') || root.getAttribute('data-bbai-credits-used') || '0', 10) || 0,
                        trial_limit: parseInt(root.getAttribute('data-bbai-trial-limit') || root.getAttribute('data-bbai-credits-total') || '0', 10) || 0,
    ...
    var bbaiCoalescedUpgradeTimer = null;
    var bbaiGenerateMissingUpgradeNudgeTimer = null;

    /**
     * Coalesce rapid duplicate upgrade opens (locked CTAs, quota errors, etc.).
     *
     * @param {*} reasonOrUsage
     * @param {*} context
     * @return {boolean}
     */
    function openUpgradeModalCoalesced(reasonOrUsage, context) {
        if (hasVisibleLimitModal()) {
            return true;
        }
        if (bbaiCoalescedUpgradeTimer) {
            window.clearTimeout(bbaiCoalescedUpgradeTimer);
            bbaiCoalescedUpgradeTimer = null;
        }
        bbaiCoalescedUpgradeTimer = window.setTimeout(function() {
            bbaiCoalescedUpgradeTimer = null;
            if (hasVisibleLimitModal()) {
                return;
            }
            refreshDashboardStateTruth()
                .then(function(state) {
                    var usage = state && state.usage ? state.usage : null;
                    var used = usage ? parseInt(usage.used, 10) : NaN;
                    var limit = usage ? parseInt(usage.limit, 10) : NaN;
                    var remaining = usage ? parseInt(usage.remaining, 10) : NaN;
                    used = isNaN(used) ? 0 : Math.max(0, used);
                    ...
                    var reason = typeof reasonOrUsage === 'string' ? String(reasonOrUsage) : '';
                    var source = context && typeof context === 'object' && context.source ? String(context.source) : '';
                    var isAutoTrigger =
                        reason === 'low_credits_threshold' ||
                        reason === 'generate_missing_click' ||
                    ...
                    openUpgradeModal(reasonOrUsage, context);
                })
                .catch(function() {
                    // Guard: state not loaded => do not open modal (no fallbacks).
                });
        }, 380);
        return true;
    }

    window.bbaiOpenUpgradeModalCoalesced = openUpgradeModalCoalesced;
    ...
    function openUpgradeModal(reasonOrUsage, context) {
        ...
        var remaining = canonical && canonical.usage ? parseInt(canonical.usage.remaining, 10) : NaN;
        if (isNaN(remaining)) {
            var used = canonical && canonical.usage ? parseInt(canonical.usage.used, 10) : NaN;
            var limit = canonical && canonical.usage ? parseInt(canonical.usage.limit, 10) : NaN;
            used = isNaN(used) ? 0 : Math.max(0, used);
            limit = isNaN(limit) ? 0 : Math.max(0, limit);
            remaining = Math.max(0, limit - used);
        } else {
        ...
        var usage = normalizeUsagePayload({
            used: canonical.usage.used,
            limit: canonical.usage.limit,
            remaining: canonical.usage.remaining,
            plan: canonical.plan && canonical.plan.slug ? canonical.plan.slug : '',
            plan_type: canonical.plan && canonical.plan.slug ? canonical.plan.slug : '',
            source: 'dashboard_state_truth',
            trial_exhausted: canonical.trial_exhausted
        });
        if (!hasReasonContext && usage && typeof usage === 'object' && parseInt(usage.remaining, 10) <= 0) {
    ...
    function handleLockedCtaClick(trigger, event) {
        ...
        if (directUpgradeModal) {
            modalOpened = openUpgradeModalCoalesced(usage);
        } else {
            modalOpened = openUpgradeModalCoalesced(reason, {
                trigger: trigger,
                source: source,
                usage: usage,
    ...
    function handleLimitReached(errorData) {
        ...
        refreshDashboardStateTruth()
            .then(function(state) {
                var u = state && state.usage ? state.usage : null;
                var used = u ? parseInt(u.used, 10) : NaN;
                var limit = u ? parseInt(u.limit, 10) : NaN;
                used = isNaN(used) ? 0 : Math.max(0, used);
                limit = isNaN(limit) ? 0 : Math.max(0, limit);
                ...
                if (!isExhausted && errorCode !== 'bbai_trial_exhausted') {
                    if (typeof showNotification === 'function') {
                        showNotification(__('Credits are still available. Please try again.', 'beepbeep-ai-alt-text-generator'), 'info');
                    }
                    return;
                }

                // Continue with existing limit handling logic.
                handleLimitReachedWithConfirmedState(normalizedError);
            })
            .catch(function() {
                // If we cannot confirm state-truth, do not show the modal (avoid false positives).
            });
        return;
    }
    ...
    function handleLimitReachedWithConfirmedState(normalizedError) {
        ...
        var freePlanOffer = usage && usage.free_plan_offer !== undefined
            ? parseInt(usage.free_plan_offer, 10)
            : readFreePlanOfferFromDashboardDom();
        freePlanOffer = isNaN(freePlanOffer) ? 0 : Math.max(0, freePlanOffer);
        var monthlyAllowance = usage && usage.limit !== undefined
            ? Math.max(1, parseInt(usage.limit, 10) || readCreditLimitFromDashboardDom() || 1)
            : Math.max(1, readCreditLimitFromDashboardDom() || 1);
        ...
        var resetMessage = isDailyLimit
            ? __('Your daily allowance refreshes automatically. Upgrade to continue generating before it refreshes.', 'beepbeep-ai-alt-text-generator')
            : sprintf(
                        ...
                        buttons: [
                            {
                                ...
                                action: function() {
                                    window.bbaiModal.close();
                                    var modalOpened = openUpgradeModalCoalesced(usage);
    ...
    function isApproveAllControl(node) {
        return !!node && (
            node.getAttribute('data-action') === 'approve-all' ||
            node.getAttribute('data-bbai-li-action') === 'approve-all'
        );
    }

    function ensureApproveAllLoadingMarkup(node, busyLabel) {
        var content;
        var loadingLabel;
        var spinner;
        var currentLabel;

        if (!node || !isApproveAllControl(node)) {
            return;
        }

        content = node.querySelector('.bbai-btn-content');
        loadingLabel = node.querySelector('.bbai-btn-loading-label');
        spinner = node.querySelector('.bbai-btn-spinner');
        currentLabel = (content ? content.textContent : node.textContent || '').trim() || __('Approve all', 'beepbeep-ai-alt-text-generator');

        if (!content || !loadingLabel || !spinner) {
            node.textContent = '';
            content = document.createElement('span');
            content.className = 'bbai-btn-content';
            content.textContent = currentLabel;
            loadingLabel = document.createElement('span');
            loadingLabel.className = 'bbai-btn-loading-label';
            loadingLabel.setAttribute('aria-hidden', 'true');
            spinner = document.createElement('span');
            spinner.className = 'bbai-btn-spinner';
            spinner.setAttribute('aria-hidden', 'true');
            node.appendChild(content);
            node.appendChild(loadingLabel);
            node.appendChild(spinner);
        }

        loadingLabel.textContent = busyLabel || __('Approving...', 'beepbeep-ai-alt-text-generator');
    }

    function ensureDashboardCtaLoadingMarkup(node, busyLabel, fallbackLabel) {
        var label;
        var spinner;
        var currentLabel;
        ...
        // Remove any legacy/duplicate spinners first (prevents double-spinners on repeated clicks
        // or when multiple systems have touched the node).
        Array.prototype.slice.call(node.querySelectorAll('.bbai-spinner, .bbai-btn-spinner, .bbai-button-spinner')).forEach(function(el) {
            try { el.remove(); } catch (e) { /* noop */ }
        });

        currentLabel = (node.textContent || '').trim() || fallbackLabel || __('Generate ALT text', 'beepbeep-ai-alt-text-generator');

        if (!node.getAttribute('data-original-label')) {
            node.setAttribute('data-original-label', currentLabel);
        }
        ...
        label.textContent = busyLabel || BBAI_GENERATION_BUSY_LABEL;
    ...
    function setBusyStateForControls(selector, busyLabel) {
        ...
        nodes.forEach(function(node) {
            var approveAllControl = isApproveAllControl(node);

            if (!approveAllControl && !node.getAttribute('data-bbai-busy-original-html')) {
            ...
            if (approveAllControl) {
                ensureApproveAllLoadingMarkup(node, busyLabel);
            } else {
                ensureDashboardCtaLoadingMarkup(node, busyLabel, node.textContent);
            }
            node.classList.add(approveAllControl ? 'is-loading' : 'is-busy');
            node.setAttribute('aria-busy', 'true');
            node.setAttribute('aria-disabled', 'true');
            node.setAttribute('data-bbai-generation-action', '1');
            node.classList.add('bbai-generate-button');
            if ('disabled' in node) {
                node.disabled = true;
            }
        });
        ...
        return function restoreBusyState() {
            nodes.forEach(function(node) {
                ...
                if (!isApproveAllControl(node) && originalHtml !== null) {
    ...
    var libraryGenerationFeedbackTimer = 0;

    function buildRemainingCreditsFeedbackLine(usage) {
        var remaining = usage && usage.remaining !== undefined && usage.remaining !== null
            ? Math.max(0, parseInt(usage.remaining, 10) || 0)
            : NaN;
        var limit = usage && usage.limit !== undefined && usage.limit !== null
            ? Math.max(1, parseInt(usage.limit, 10) || 0)
            : NaN;
        var isTrial = !!(usage && (usage.is_trial || String(usage.auth_state || '').toLowerCase() === 'anonymous'));
        ...
        if (isTrial) {
            ...
            return sprintf(
                _n('You have %s free generation left', 'You have %s free generations left', remaining, 'beepbeep-ai-alt-text-generator'),
        ...
        return sprintf(
            _n('You have %s generation left', 'You have %s generations left', remaining, 'beepbeep-ai-alt-text-generator'),
            formatDashboardNumber(remaining)
        );
    }

    function buildGenerationFeedbackPayload(source, successes, failures) {
        var successCount = Math.max(0, parseInt(successes, 10) || 0);
        var failureCount = Math.max(0, parseInt(failures, 10) || 0);
        var usage = getUsageSnapshot(null) || getLibraryWorkspaceUsageSnapshot();
        var detail = buildRemainingCreditsFeedbackLine(usage);
        var payload = {
            type: failureCount > 0 ? 'warning' : 'success',
            message: buildDashboardGenerationMessage(source, successCount, failureCount),
            detail: '',
            markSuccess: false
        };
        ...
        if (successCount > 0 && failureCount === 0) {
            payload.message = source === 'fix-all-issues'
                ? sprintf(
                    _n('%s image optimized successfully', '%s images optimized successfully', successCount, 'beepbeep-ai-alt-text-generator'),
                    formatDashboardNumber(successCount)
                )
                : __('ALT text generated ✓', 'beepbeep-ai-alt-text-generator');
            payload.detail = detail;
            payload.markSuccess = true;
        }

        return payload;
    }

    function setDashboardRecentSuccessWindow(active, durationMs) {
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        var timeout = Math.max(0, parseInt(durationMs, 10) || 12000);
        ...
        if (!active) {
            root.removeAttribute('data-bbai-last-success-ts');
            if (typeof window.bbaiSyncDashboardStateRoot === 'function') {
        ...
        root.setAttribute('data-bbai-last-success-ts', String(Date.now()));
        if (typeof window.bbaiSyncDashboardStateRoot === 'function') {
            window.bbaiSyncDashboardStateRoot();
        }
        ...
        window.setTimeout(function() {
            ...
            var successTs = parseInt(root.getAttribute('data-bbai-last-success-ts') || '0', 10) || 0;
            if (successTs > 0 && (Date.now() - successTs) >= timeout) {
                root.removeAttribute('data-bbai-last-success-ts');
                if (typeof window.bbaiSyncDashboardStateRoot === 'function') {
                    window.bbaiSyncDashboardStateRoot();
                }
    ...
    function dispatchDashboardFeedback(type, message, options) {
        ...
        if (!shouldToast || !detail.message) {
            if (options && options.markSuccess) {
                setDashboardRecentSuccessWindow(true, detail.duration || 12000);
            } else if (options && options.clearSuccessWindow) {
                setDashboardRecentSuccessWindow(false, 0);
            }
            return;
        }
    ...
    function getOpportunityScannerGuide(payload) {
        ...
        var workflowSteps = [
            ...
            },
            {
                ...
                buttonLabel: __('Generate ALT text', 'beepbeep-ai-alt-text-generator'),
    ...
    function dispatchDashboardStatsUpdated(payload) {
        ...
        var root = getDashboardRootNode();
        var hasScanResults = normalizedStats.total_images > 0
            || normalizedStats.optimized_count > 0
            || normalizedStats.images_missing_alt > 0
            || normalizedStats.needs_review_count > 0;
        var lastScanTs = root ? (parseOptionalNonNegativeInt(root.getAttribute('data-bbai-last-scan-ts')) || 0) : 0;

        if (normalizedStats.scanned_at !== undefined) {
            lastScanTs = Math.max(0, parseInt(normalizedStats.scanned_at, 10) || 0);
        } else if (normalizedStats.scannedAt !== undefined) {
            lastScanTs = Math.max(0, parseInt(normalizedStats.scannedAt, 10) || 0);
        }
        ...
        if (root) {
            ...
            root.setAttribute('data-bbai-has-scan-history', lastScanTs > 0 ? '1' : '0');
            syncDashboardActionableStateAttributes(root);
        }
    ...
    function clearLibraryFilterAttention() {
        document.querySelectorAll('#bbai-review-filter-tabs button[data-filter]').forEach(function(button) {
            button.classList.remove('bbai-filter-group__item--attention');
        });
    }
    ...
    function syncLibraryFilterAttentionState(counts) {
        ...
        filterButtons.forEach(function(button) {
            ...
            button.classList.toggle('bbai-filter-group__item--attention', shouldAnimate);
        });
    }
    ...
    function getLibraryArrivalFocusMode() {
        try {
            return String(new URL(window.location.href).searchParams.get('bbai_focus') || '').toLowerCase();
        } catch (error) {
            return '';
        }
    }

    function getFirstVisibleLibraryRow() {
        return document.querySelector('.bbai-library-row:not(.bbai-library-row--hidden)');
    }
    ...
    function focusFirstVisibleLibraryRow() {
        var row = getFirstVisibleLibraryRow();
    ...
    function scrollToAltTable(options) {
        ...
        window.setTimeout(function() {
            ...
            if (settings.focusFirstItem) {
                if (!focusFirstVisibleLibraryRow()) {
    ...
    if (isOnLibraryPage() && window.location.hash === '#bbai-alt-table') {
        ...
        setTimeout(function() {
            scrollToAltTable({
                focusFirstItem: getLibraryArrivalFocusMode() === 'first-review'
            });
        }, 300);
    }
    ...
    function updateLibraryUsageSurface(usagePayload) {
        ...
        var limitRaw = parseInt(usagePayload.limit, 10);
        var limit = (!isNaN(limitRaw) && limitRaw > 0)
            ? limitRaw
            : Math.max(1, readCreditLimitFromDashboardDom() || 1);
        ...
        var workspaceRoot = root.closest('[data-bbai-library-workspace-root="1"]');
        var freePlanOffer = parseInt(usagePayload.free_plan_offer, 10);
        if (isNaN(freePlanOffer) && workspaceRoot) {
            freePlanOffer = parseInt(workspaceRoot.getAttribute('data-bbai-free-plan-offer'), 10);
        }
        freePlanOffer = isNaN(freePlanOffer)
            ? Math.max(0, readFreePlanOfferFromDashboardDom() || 0)
            : Math.max(0, freePlanOffer);
        var quotaUi = getLibraryQuotaUiState();
        ...
        var trialBannerRemainingNode = document.querySelector('[data-bbai-library-trial-remaining-line]');
        if (trialBannerRemainingNode) {
            trialBannerRemainingNode.textContent = remaining === 1
                ? '1 generation remaining'
                : formatDashboardNumber(remaining) + ' generations remaining';
        }
        syncLibraryTopHeroGenerateMissingCta();
        updateLibraryBannerInlineChips();
    }
    ...
    function getLibraryActiveFilter() {
        var activeButton = document.querySelector(
            '#bbai-review-filter-tabs button.bbai-filter-group__item--active'
        );
        return activeButton
    ...
    function runDashboardFixAllIssues(trigger, usageStats) {
        var button = trigger || document.querySelector('[data-bbai-fix-dashboard="1"][data-action="generate-missing"]');
        var restoreBusyState;
        var remaining;
        ...
        restoreBusyState = setBusyStateForControls(
            '[data-bbai-fix-dashboard="1"][data-action="generate-missing"]',
            BBAI_GENERATION_BUSY_LABEL
        );

        remaining = usageStats && usageStats.remaining !== undefined && usageStats.remaining !== null
            ? Math.max(0, parseInt(usageStats.remaining, 10) || 0)
            : null;
        ...
        $.when(fetchBulkImageIds('missing', 500), fetchBulkImageIds('needs-review', 500))
            .done(function(missingResponse, weakResponse) {
                ...
                if (remaining !== null && remaining > 0 && ids.length > remaining) {
                ...
                if (!ids.length) {
                    ...
                    dispatchDashboardFeedback('info', __('No ALT issues found. Your images are already up to date.', 'beepbeep-ai-alt-text-generator'));
                    return;
                }

                queueImages(ids, 'bulk-regenerate', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
                    restoreBusyState();

                    if (success && queued > 0) {
                        startGenerationFlow(processedIds || ids, {
                            source: 'fix-all-issues',
                            entry: 'runDashboardFixAllIssues',
                            responseData: responseData,
                            progressLabel: __('Preparing fixes...', 'beepbeep-ai-alt-text-generator'),
                            queued: queued
                        });
                        return;
                    }
                    ...
                    if (success && queued === 0) {
                        startGenerationFlow(processedIds || ids, {
                            source: 'fix-all-issues',
                            entry: 'runDashboardFixAllIssues_zero',
                            responseData: responseData,
                            progressLabel: __('Preparing fixes...', 'beepbeep-ai-alt-text-generator'),
                            queued: 0
                        });
                    ...
                    dispatchDashboardFeedback(
                        ...
                        error && error.message
                            ? error.message
                            : __('Failed to queue images for fixing. Please try again.', 'beepbeep-ai-alt-text-generator')
                    );
                });
            })
            .fail(function(xhr) {
    ...
    function handleGenerateMissing(e) {
        if (!bbaiAcquireGenerationLock(e, 'dashboard_generate', null)) {
            return false;
        }
        ...
        if ($btn.attr('data-bbai-busy-original-disabled') === 'true') {
            setGenerationInProgress(false);
            return false;
        }
        ...
        if ($btn.attr('data-bbai-fix-dashboard') === '1') {
            setDashboardRuntimeState('generation_starting');
            return runDashboardFixAllIssues(this, usageStats);
        }
        ...
        function continueWithGeneration() {
            ...
            if (!response || !response.ids || response.ids.length === 0) {
                ...
                var dashboardRoot = getDashboardRootNode();
                var hasConnectedAccount = !!(dashboardRoot && dashboardRoot.getAttribute('data-bbai-has-connected-account') === '1');
                var reviewUrl = buildDashboardReviewLibraryUrl(
                    (dashboardRoot && (dashboardRoot.getAttribute('data-bbai-needs-review-library-url') || dashboardRoot.getAttribute('data-bbai-library-url'))) ||
                    getAltLibraryAdminUrl()
                );
                setDashboardRuntimeState('idle');

                if (actionable.key === 'review' && actionable.reviewCount > 0) {
                    window.bbaiModal.show({
                        type: 'info',
                        title: sprintf(
                            _n('%s image ready to review', '%s images ready to review', actionable.reviewCount, 'beepbeep-ai-alt-text-generator'),
                            formatDashboardNumber(actionable.reviewCount)
                        ),
                        message: __('No missing ALT text remains. Review the generated descriptions that still need attention.', 'beepbeep-ai-alt-text-generator'),
                        buttons: [
                            {
                                text: __('Review images', 'beepbeep-ai-alt-text-generator'),
                                primary: true,
                                action: function() {
                                    window.bbaiModal.close();
            ...
            queueImages(ids, 'bulk', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
	                if (success && queued > 0) {
	                    startGenerationFlow(processedIds || ids, {
	                        ...
	                        entry: 'handleGenerateMissing',
	                        responseData: responseData,
	                        progressLabel: __('Processing your images', 'beepbeep-ai-alt-text-generator'),
	                        queued: queued
	                    });
                    ...
                    document.dispatchEvent(event);
	                    ...
	                    startGenerationFlow(processedIds || ids, {
	                        source: 'generate-missing',
	                        entry: 'handleGenerateMissing_zero',
	                        responseData: responseData,
	                        progressLabel: __('Processing your images', 'beepbeep-ai-alt-text-generator'),
	                        queued: 0
	                    });
                                ...
                                buttons: [
                                    {
                                        ...
                                        action: function() {
                                            ...
                                            bbaiSetGenerationLock && bbaiSetGenerationLock('bulk', null);

                                            queueImages(limitedIds, 'bulk', { skipSchedule: true }, function(success, queued, queueError, processedLimited, limitedResponseData) {
                                                if (success && queued > 0) {
                                                    startGenerationFlow(processedLimited || limitedIds, {
                                                        source: 'bulk',
                                                        entry: 'handleGenerateMissing_partial',
                                                        responseData: limitedResponseData,
                                                        progressLabel: sprintf(_n('Queueing %d image...', 'Queueing %d images...', remainingCount, 'beepbeep-ai-alt-text-generator'), remainingCount),
                                                        queued: queued
                                                    });
                                                } else {
                                    ...
                                    },
                                    {
                                        ...
                                        action: function() {
                                            ...
                                            if (isAnonymousTrialLimit) {
                                                ...
                                                openUpgradeModalCoalesced(usageSnapshot);
                                            }
                                        }
                                    },
	                    {
    ...
    function handleRegenerateAll(e) {
        ...
        if (!bbaiAcquireGenerationLock(e, 'regenerate-all', null)) {
            return false;
        }
        ...
        if (!isPremium && (isOutOfCredits || isButtonLocked)) {
            ...
            try {
                ...
                openUpgradeModalCoalesced(usageStats);
            }
            return false;
        }
        ...
        .done(function(response) {
            ...
            // Queue the requested image set (progress opens after queue succeeds).
            queueImages(ids, 'bulk-regenerate', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
		                if (success && queued > 0) {
		                    startGenerationFlow(processedIds || ids, {
		                        source: isWeakOnlyRun ? 'regenerate-weak' : 'regenerate-all',
		                        entry: 'proceedWithRegeneration',
		                        responseData: responseData,
		                        progressLabel: preparingMessage,
		                        queued: queued
		                    });
                    ...
                    document.dispatchEvent(event);
		                    ...
		                    startGenerationFlow(processedIds || ids, {
		                        source: isWeakOnlyRun ? 'regenerate-weak' : 'regenerate-all',
		                        entry: 'proceedWithRegeneration_zero',
		                        responseData: responseData,
		                        progressLabel: preparingMessage,
		                        queued: 0
		                    });
                                ...
                                buttons: [
                                    {
                                        ...
                                        action: function() {
                                            window.bbaiModal.close();
                                            var limitedIds = ids.slice(0, remainingCount);
                                            bbaiSetGenerationLock && bbaiSetGenerationLock('regenerate-all', null);
                                            restoreRegenerateBusyState = setBusyStateForControls('[data-action="regenerate-all"], [data-bbai-action="reoptimize_all"]', BBAI_GENERATION_BUSY_LABEL);

                                            queueImages(limitedIds, 'bulk-regenerate', { skipSchedule: true }, function(success, queued, queueError, processedLimited, limitedResponseData) {
                                                if (success && queued > 0) {
                                                    startGenerationFlow(processedLimited || limitedIds, {
                                                        source: 'regenerate-all',
                                                        entry: 'proceedWithRegeneration_partial',
                                                        responseData: limitedResponseData,
                                                        progressLabel: sprintf(_n('Queueing %d image for regeneration...', 'Queueing %d images for regeneration...', remainingCount, 'beepbeep-ai-alt-text-generator'), remainingCount),
                                                        queued: queued
                                                    });
                                                } else {
                                                    var queueErrMsg = __('Failed to queue images.', 'beepbeep-ai-alt-text-generator');
            ...
            .done(function(missingResponse, weakResponse) {
                ...
                queueImages(ids, 'bulk-regenerate', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
                    ...
                    if (success && queued > 0) {
                        startGenerationFlow(processedIds || ids, {
                            source: 'fix-all-issues',
                            entry: 'runBulkFixAllIssues',
                            responseData: responseData,
                            progressLabel: __('Optimizing ALT text...', 'beepbeep-ai-alt-text-generator'),
                            queued: queued
                        });
                        return;
                    }
                    ...
                    if (success && queued === 0) {
                        startGenerationFlow(processedIds || ids, {
                            source: 'fix-all-issues',
                            entry: 'runBulkFixAllIssues_zero',
                            responseData: responseData,
                            progressLabel: __('Optimizing ALT text...', 'beepbeep-ai-alt-text-generator'),
                            queued: 0
                        });
    ...
    function getDashboardActionableStateSnapshot() {
        var root = getDashboardRootNode();
        var fallback;
        var missing;
        var weak;
        ...
        if (typeof window.bbaiGetDashboardActionableState === 'function') {
            fallback = window.bbaiGetDashboardActionableState(root);
        ...
        actionableCount = missing + weak;
        percentage = total > 0 ? Math.max(0, Math.round((optimized / total) * 100)) : 0;
        state = 'complete';
    ...
    function syncDashboardActionableStateAttributes(root) {
        var snapshot;
        ...
        snapshot = getDashboardActionableStateSnapshot();
        root.setAttribute('data-bbai-actionable-state', String(snapshot.key || 'complete'));
        root.setAttribute('data-bbai-actionable-count', String(Math.max(0, parseInt(snapshot.actionableCount, 10) || 0)));
    }

    function buildDashboardReviewLibraryUrl(baseUrl) {
    ...
    function applyLibraryWorkspaceUsageAttributes(usage) {
        ...
        root.setAttribute('data-bbai-credits-total', String(Math.max(1, parseInt(normalized.limit, 10) || parseInt(root.getAttribute('data-bbai-credits-total'), 10) || 1)));
        ...
        root.setAttribute('data-bbai-free-plan-offer', String(Math.max(0, parseInt(normalized.free_plan_offer, 10) || parseInt(root.getAttribute('data-bbai-free-plan-offer'), 10) || 0)));
    ...
    function updateDashboardHero(statsInput) {
        ...
        if (typeof window.bbaiBuildSharedCommandHeroState === 'function') {
            ...
            var limHero = usageSnap && usageSnap.limit !== undefined
                ? Math.max(1, parseInt(usageSnap.limit, 10) || parseInt(root && root.getAttribute('data-bbai-credits-total'), 10) || 1)
                : (root ? Math.max(1, parseInt(root.getAttribute('data-bbai-credits-total'), 10) || readCreditLimitFromDashboardDom() || 1) : Math.max(1, readCreditLimitFromDashboardDom() || 1));
    ...
    function runSilentInlineGeneration(idList, source, options) {
        ...
        function finalize() {
            ...
            var finishFeedback = function() {
                var feedbackPayload = buildGenerationFeedbackPayload(source, successes, failures);
                dispatchDashboardFeedback(
                    ...
                    feedbackPayload.message,
                    {
                        duration: 12000,
                        skipToast: true,
                        detail: feedbackPayload.detail
                    }
                );

                if (getLibraryWorkspaceRoot() && successes > 0 && failures === 0) {
                    showLibraryGenerationFeedback(feedbackPayload.message, feedbackPayload.detail, feedbackPayload.type);
                }
    ...
    function openDashboardGeneratorModal() {
        ...
        generateButton.addEventListener('click', function() {
            ...
            if (estimateState.overLimit) {
                window.bbaiModal.close();
                window.setTimeout(function() {
                    openUpgradeModalCoalesced(usageState.usage || null);
                }, 80);
                return;
            }
            ...
            getGeneratorScopeIds(activeScope)
                .done(function(ids) {
                    ...
                    queueImages(normalizedIds, getGeneratorQueueSource(activeScope, normalizedIds), { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
                        if (success || (error && error.code === 'already_queued')) {
                            syncDashboardRuntimeStateFromBackendJob(responseData);
    ...
    function updateLibraryGuidance(scanData) {
        ...
        if (totalImages > 0 && missingCount > 0) {
            ...
            primaryHtml = '<button type="button" class="' + clsP + lockCls + '" data-action="generate-missing" data-bbai-lock-preserve-label="1"' + lockAttrs + '>' +
                escapeHtml(__('Generate ALT text', 'beepbeep-ai-alt-text-generator')) + '</button>';
            secondaryHtml = '';
            showSecondary = false;
    ...
    function runCoverageScanFlow(trigger, options) {
        ...
        if (typeof window.bbaiRefreshDashboardCoverage !== 'function') {
            try {
                document.dispatchEvent(new CustomEvent('bbai:scan:failed', { detail: { message: fallbackError } }));
            } catch (eventError) {
                // Ignore DOM dispatch failures.
            }
            if (modal) {
        ...
        try {
            document.dispatchEvent(new CustomEvent('bbai:scan:started'));
        } catch (eventError) {
            // Ignore DOM dispatch failures.
        }
        ...
        window.bbaiRefreshDashboardCoverage()
            .done(function(payload) {
                var finalize = function() {
                    ...
                    try {
                        document.dispatchEvent(new CustomEvent('bbai:scan:complete', { detail: { payload: payload || {} } }));
                    } catch (eventError) {
                        // Ignore DOM dispatch failures.
                    }
    ...
    function getDashboardApproveAllEndpoint() {
        var root = (
            config.restRoot ||
    ...
    function formatDashboardHeroCreditLabel(rem, lim) {
        rem = Math.max(0, parseInt(rem, 10) || 0);
        lim = Math.max(1, parseInt(lim, 10) || 1);
        var used = Math.max(0, lim - rem);
        var uStr = used.toLocaleString();
        var lStr = lim.toLocaleString();
        return sprintf(__('%1$s / %2$s used this month', 'beepbeep-ai-alt-text-generator'), uStr, lStr);
    }

    function formatDashboardHeroCreditBarAria(rem, lim, isPaid) {
        rem = Math.max(0, parseInt(rem, 10) || 0);
        lim = Math.max(1, parseInt(lim, 10) || 1);
        var used = Math.max(0, lim - rem);
        var uStr = used.toLocaleString();
        var lStr = lim.toLocaleString();
        return sprintf(__('%1$s of %2$s credits used this month', 'beepbeep-ai-alt-text-generator'), uStr, lStr);
    }

    function formatDashboardHeroCreditContext(rem, heroLiState) {
        rem = Math.max(0, parseInt(rem, 10) || 0);
        if (rem <= 0) {
    ...
    function formatDashboardHeroCreditInsight(rem, missingCount) {
        var remaining = Math.max(0, parseInt(rem, 10) || 0);
        var missing = Math.max(0, parseInt(missingCount, 10) || 0);
        if (missing <= 0) {
            return '';
        }
        if (remaining >= missing) {
            return __('✔ Enough credits to finish this batch', 'beepbeep-ai-alt-text-generator');
        }
        return sprintf(__('You can generate %s more images', 'beepbeep-ai-alt-text-generator'), remaining.toLocaleString());
    }

    function resolveDashboardHeroGrowthLimit(heroUsage, fallbackLimit) {
        var growthLimit = heroUsage ? parseInt(heroUsage.getAttribute('data-bbai-hero-growth-credit-limit') || '', 10) : NaN;
        if (isFinite(growthLimit) && growthLimit > 0) {
            return growthLimit;
        }
        return Math.max(1, parseInt(fallbackLimit, 10) || 1);
    }
    ...
    function resolveCreditsLimitFallback() {
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        var fromRoot = root ? parseInt(root.getAttribute('data-bbai-credits-total'), 10) : NaN;
        if (isFinite(fromRoot) && fromRoot > 0) {
            return fromRoot;
        }
        var fromTrial = getFreeAccountMonthlyLimit();
        if (isFinite(fromTrial) && fromTrial > 0) {
            return fromTrial;
        }
        return 0;
    }

    function updateGlobalCreditsSurfacesFromCounts(used, limit, remaining) {
        var lim = Math.max(1, parseInt(limit, 10) || 1);
        ...
        var pill = document.querySelector('[data-bbai-credits-pill]');
        if (pill) {
            pill.setAttribute('data-state', st);
            var pillText = sprintf(
                __('Used %1$s of %2$s this month', 'beepbeep-ai-alt-text-generator'),
                u.toLocaleString(),
                lim.toLocaleString()
            );
            pill.textContent = pillText;
        }
        var heroUsage = document.querySelector('[data-bbai-hero-credit-usage]');
        if (heroUsage) {
            var heroStateEl = document.querySelector('[data-bbai-li-hero="1"]');
            var heroLiState = heroStateEl ? String(heroStateEl.getAttribute('data-bbai-li-state') || '') : '';
            var dashRoot = document.querySelector('[data-bbai-dashboard-root="1"]');
            var isPaid = dashRoot && dashRoot.getAttribute('data-bbai-is-premium') === '1';
            var missingCount = dashRoot ? parseInt(dashRoot.getAttribute('data-bbai-missing-count') || '0', 10) || 0 : parseInt(heroUsage.getAttribute('data-bbai-hero-missing-count') || '0', 10) || 0;
            var reviewCount = dashRoot ? parseInt(dashRoot.getAttribute('data-bbai-weak-count') || '0', 10) || 0 : parseInt(heroUsage.getAttribute('data-bbai-hero-review-count') || '0', 10) || 0;
            var pct = lim > 0 ? Math.min(100, Math.max(0, Math.round((u / lim) * 100))) : 0;
            var creditState = 'healthy';
            if (rem === 0) {
                creditState = 'empty';
            } else if (rem <= 10) {
                creditState = 'low';
            }
            heroUsage.setAttribute('data-credit-state', creditState);
            heroUsage.classList.remove('low', 'empty');
            heroUsage.classList.toggle('bbai-credit-usage--all-clear-muted', heroLiState === 'ALL_CLEAR');
            heroUsage.setAttribute('data-bbai-hero-credits-remaining', String(rem));
            heroUsage.setAttribute('data-bbai-hero-credits-limit', String(lim));
            heroUsage.setAttribute('data-bbai-hero-credits-used', String(u));
            heroUsage.setAttribute('data-bbai-hero-missing-count', String(Math.max(0, missingCount)));
            heroUsage.setAttribute('data-bbai-hero-review-count', String(Math.max(0, reviewCount)));
            var label = heroUsage.querySelector('[data-bbai-hero-credit-label]');
            if (label) {
                label.textContent = formatDashboardHeroCreditLabel(rem, lim);
            }
            var clarity = heroUsage.querySelector('[data-bbai-hero-credit-clarity]');
            if (clarity) {
                clarity.textContent = heroLiState === 'ALL_CLEAR' && !isPaid
            ...
            var fill = heroUsage.querySelector('[data-bbai-hero-credit-fill]');
            if (fill) {
                fill.style.setProperty('--bbai-credit-percent', pct + '%');
            }
            var bar = heroUsage.querySelector('[data-bbai-hero-credit-bar]') || heroUsage.querySelector('.bbai-credit-bar');
            if (bar) {
                bar.setAttribute('aria-valuenow', String(pct));
                bar.setAttribute('aria-label', formatDashboardHeroCreditBarAria(rem, lim, isPaid));
            }
            var context = heroUsage.querySelector('[data-bbai-hero-credit-context]');
            if (context) {
                var ctxText = formatDashboardHeroCreditContext(rem, heroLiState);
                context.classList.toggle('bbai-credit-context--warning', rem < 10);
                if (ctxText) {
                    context.textContent = ctxText;
            ...
            var insight = heroUsage.querySelector('[data-bbai-hero-credit-insight]');
            if (insight) {
                var insightText = formatDashboardHeroCreditInsight(rem, missingCount);
                if (insightText) {
            ...
            var helper = heroUsage.querySelector('[data-bbai-hero-credit-helper]');
            if (helper) {
                var helperText = formatDashboardHeroCreditGrowthComparison(heroUsage, u, lim, rem);
                var helperLink = helper.querySelector('[data-bbai-hero-credit-growth-link]');
                if (helperText) {
                    if (helperLink) {
                        helperLink.textContent = helperText;
                    } else {
        ...
        var dashRootLow = document.querySelector('[data-bbai-dashboard-root="1"]');
        if (
            dashRootLow &&
            dashRootLow.getAttribute('data-bbai-is-premium') !== '1' &&
            dashRootLow.getAttribute('data-bbai-is-guest-trial') !== '1' &&
            dashRootLow.getAttribute('data-bbai-signup-required') !== '1' &&
            dashRootLow.getAttribute('data-bbai-has-connected-account') === '1' &&
            rem <= 10 &&
            rem > 0
        ) {
            try {
                document.dispatchEvent(new CustomEvent('bbai:credits-low-threshold', {
                    detail: { remaining: rem, limit: lim }
                }));
            } catch (errLow) {}
        }
    }

    function updateGlobalCreditsSurfacesFromUsage(usage) {
        if (!usage || typeof usage !== 'object') {
            return;
        }
        var used = parseInt(usage.used, 10);
        ...
        if (isNaN(limit) || limit <= 0) {
            limit = resolveCreditsLimitFallback();
        }
        ...
        remaining = Math.max(0, remaining);
        updateGlobalCreditsSurfacesFromCounts(used, limit, remaining);
    }
    ...
    function getUsageFromBanner(banner) {
        ...
        return {
            ...
            free_plan_offer: isNaN(freePlanOffer) ? null : Math.max(0, freePlanOffer),
    ...
    function syncSharedUsageBanner(usageInput) {
        var banner = getSharedBannerNode();
        var usage = getUsageSnapshot(usageInput) || getUsageFromDom() || (banner ? getUsageFromBanner(banner) : null);
        var bannerUsage = banner ? getUsageFromBanner(banner) : null;
        ...
        if (
            ...
            window.bbaiSyncDashboardState(null, {
                ...
                free_plan_offer: (function() {
                    if (usage.free_plan_offer !== undefined && usage.free_plan_offer !== null) {
                        return usage.free_plan_offer;
                    }
                    var r = getDashboardRootNode();
                    var o = r ? parseInt(r.getAttribute('data-bbai-free-plan-offer'), 10) : NaN;
                    if (isFinite(o) && o > 0) {
                        return o;
                    }
                    var limFb = resolveCreditsLimitFallback();
                    return limFb > 0 ? limFb : null;
                })(),
        ...
        if (headlineNum) {
            ...
            headlineNode.textContent = outOfCredits
                ? sprintf(__('You optimised %s images this month 🎉', 'beepbeep-ai-alt-text-generator'), used.toLocaleString())
                : sprintf(__('You optimised %s images this month', 'beepbeep-ai-alt-text-generator'), used.toLocaleString());
        }
    ...
    function getTrialPreviewPrimaryActionHtml(row) {
        var attachmentId;
        var state;
        var label;
        var title;
        ...
        attachmentId = parseInt(row.getAttribute('data-attachment-id') || row.getAttribute('data-id') || '', 10);
        if (!attachmentId || attachmentId <= 0) {
            return '';
        }

        state = getLibraryReviewState(row);
        label = __('Done', 'beepbeep-ai-alt-text-generator');
        title = __('Preview this ALT text', 'beepbeep-ai-alt-text-generator');
        classNames = 'bbai-dashboard-trial-preview__action bbai-dashboard-trial-preview__action--done';
        action = 'preview-image';
	        ...
	        if (state === 'missing') {
                ...
                label = __('Fix', 'beepbeep-ai-alt-text-generator');
                title = __('Fix this image', 'beepbeep-ai-alt-text-generator');
                classNames = 'bbai-dashboard-trial-preview__action bbai-dashboard-trial-preview__action--generate';
                action = 'regenerate-single';
            }
        } else if (state === 'weak') {
            label = __('Review', 'beepbeep-ai-alt-text-generator');
            title = __('Review this generated ALT text', 'beepbeep-ai-alt-text-generator');
            classNames = 'bbai-dashboard-trial-preview__action bbai-dashboard-trial-preview__action--review';
        }
    ...
    function showLibraryGenerationFeedback(message, detail, state) {
        var feedback = document.querySelector('[data-bbai-library-generation-feedback]');
        var titleNode;
        var detailNode;
        ...
        titleNode = feedback.querySelector('[data-bbai-library-generation-feedback-title]');
        detailNode = feedback.querySelector('[data-bbai-library-generation-feedback-detail]');

        feedback.hidden = false;
        feedback.setAttribute('data-state', state || 'success');
        ...
        if (detailNode) {
            ...
            if (detail) {
                detailNode.removeAttribute('hidden');
            } else {
                detailNode.setAttribute('hidden', '');
            }
        }

        if (libraryGenerationFeedbackTimer) {
            window.clearTimeout(libraryGenerationFeedbackTimer);
        }

        libraryGenerationFeedbackTimer = window.setTimeout(function() {
    ...
    function updateLibraryRowActions(row) {
        ...
        if (String(row.getAttribute('data-bbai-trial-preview-row') || '') === '1') {
            var previewActionRoot = row.querySelector('[data-bbai-trial-preview-row-action]');
            if (previewActionRoot) {
                previewActionRoot.innerHTML = getTrialPreviewPrimaryActionHtml(row);
            }
            return;
        }
    ...
    function runBulkGenerateSelected(trigger) {
        ...
        queueImages(ids, 'bulk', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
            ...
            if (success && queued === 0) {
                startGenerationFlow(processedIds || ids, {
                    source: 'bulk',
                    entry: 'runBulkGenerateSelected_zero',
                    responseData: responseData,
                    progressLabel: prepLabel,
                    queued: 0
                });
    ...
    function runBulkRegenerateSelected(trigger) {
        ...
        queueImages(ids, 'bulk-regenerate', { skipSchedule: true }, function(success, queued, error, processedIds, responseData) {
            ...
            if (success && queued === 0) {
                startGenerationFlow(processedIds || ids, {
                    source: 'bulk-regenerate',
                    entry: 'runBulkRegenerateSelected_zero',
                    responseData: responseData,
                    progressLabel: prepLabelRegen,
                    queued: 0
                });
    ...
    function notifyApproveAllFeedback(type, message) {
        if (!message) {
            return;
        }
    ...
    function getApproveAllErrorMessage(xhr) {
        var message = __('Unable to approve these images right now.', 'beepbeep-ai-alt-text-generator');
        if (xhr && xhr.responseJSON) {
            if (xhr.responseJSON.message) {
                message = xhr.responseJSON.message;
            } else if (xhr.responseJSON.data && xhr.responseJSON.data.message) {
    ...
    function loggedInDonutRingDegrees(missing, weak, optimized, total) {
        var tot = Math.max(1, total | 0);
        var m = Math.max(0, missing | 0);
        var w = Math.max(0, weak | 0);
        var o = Math.max(0, optimized | 0);
        var minAction = 72;
        var dm = (360 * m) / tot;
        var dw = (360 * w) / tot;
        var dopt = (360 * o) / tot;
        ...
        var f;
        if (m > 0) {
            dm = Math.max(dm, minAction);
        } else if (w > 0) {
            dw = Math.max(dw, minAction);
        }
        sum = dm + dw + dopt;
        if (sum > 360.0001) {
            if (m > 0) {
        ...
        return {
            a1: Math.round(Math.min(360, dm) * 1000) / 1000,
            a2: Math.round(Math.min(360, dm + dw) * 1000) / 1000,
            a3: Math.round(Math.min(360, dm + dw + dopt) * 1000) / 1000
        };
    }

    function buildLoggedInDonutBackground(stateId, segments) {
        var seg = segments && typeof segments === 'object' ? segments : {};
        var optimized = Math.max(0, parseInt(seg.optimized, 10) || 0);
        var weak = Math.max(0, parseInt(seg.weak, 10) || 0);
        var missing = Math.max(0, parseInt(seg.missing, 10) || 0);
        var total = Math.max(1, parseInt(seg.total, 10) || optimized + weak + missing || 1);
        var angles;
        var a1;
        var a2;
        var a3;
    ...
    function getLoggedInDonutSubLabel(stateId, donut) {
        var segments = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
        var missing = Math.max(0, parseInt(segments.missing, 10) || 0);
        var weak = Math.max(0, parseInt(segments.weak, 10) || 0);

        if (stateId === 'QUEUED' || stateId === 'ERROR' || stateId === 'NO_IMAGES') {
            return String((donut && donut.center_sub_label) || '');
        }
        if (stateId === 'PROCESSING') {
            return __('generating now', 'beepbeep-ai-alt-text-generator');
        }
        if (stateId === 'ALL_CLEAR') {
            return '';
        }
        if (stateId === 'NEEDS_REVIEW') {
            return __('Images ready for review', 'beepbeep-ai-alt-text-generator');
        }
        if (missing > 0) {
            return _n('image needs ALT text', 'images need ALT text', missing, 'beepbeep-ai-alt-text-generator');
        }
        if (weak > 0) {
            return _n('image ready for review', 'images ready for review', weak, 'beepbeep-ai-alt-text-generator');
        }

        return String((donut && donut.center_sub_label) || '');
    }

    function getLoggedInDonutMeta(stateId, donut, helperNode) {
        var seg = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
        var miss = Math.max(0, parseInt(seg.missing, 10) || 0);
        if (helperNode && !helperNode.hidden && miss > 0) {
            return '';
        }
        if (stateId === 'QUEUED') {
            return __('Start generation when you’re ready.', 'beepbeep-ai-alt-text-generator');
        }
        if (stateId === 'PROCESSING') {
            return __('This may take a few minutes', 'beepbeep-ai-alt-text-generator');
        }
        if (stateId === 'ALL_CLEAR') {
            return __('Everything is optimised and SEO-ready', 'beepbeep-ai-alt-text-generator');
        }
    ...
    function updateLoggedInDashboardUpgradeFloat(stateId) {
        ...
        show = hasConnectedAccount && !isPremium && !isGuestTrial && stateId === 'MISSING_ALT';
    ...
    function readLoggedInStateNumber(source, keys, fallback) {
        var i;
        var value;
        ...
        for (i = 0; i < keys.length; i += 1) {
            ...
            value = parseInt(source[keys[i]], 10);
            if (!isNaN(value)) {
    ...
    function getLoggedInDashboardStateCounts(stateData) {
        var direct = stateData && stateData.counts && typeof stateData.counts === 'object' ? stateData.counts : null;
        var truth = stateData && stateData.meta && stateData.meta.state_truth && typeof stateData.meta.state_truth === 'object'
            ? stateData.meta.state_truth
            : null;
        var truthCounts = truth && truth.counts && typeof truth.counts === 'object' ? truth.counts : null;
        var segments = stateData && stateData.donut && stateData.donut.segments && typeof stateData.donut.segments === 'object'
            ? stateData.donut.segments
            : null;
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        var source = truthCounts || direct || segments || {};
        var fallbackMissing = root ? parseInt(root.getAttribute('data-bbai-missing-count') || '0', 10) || 0 : 0;
        var fallbackReview = root ? parseInt(root.getAttribute('data-bbai-weak-count') || '0', 10) || 0 : 0;
        var fallbackComplete = root ? parseInt(root.getAttribute('data-bbai-optimized-count') || '0', 10) || 0 : 0;
        var fallbackTotal = root ? parseInt(root.getAttribute('data-bbai-total-count') || '0', 10) || 0 : 0;
        var missing = readLoggedInStateNumber(source, ['missing', 'missing_alt', 'missingAlt', 'images_missing_alt', 'imagesMissingAlt', 'missing_count', 'missingCount'], fallbackMissing);
        var review = readLoggedInStateNumber(source, ['review', 'to_review', 'toReview', 'needs_review', 'needsReview', 'needs_review_count', 'needsReviewCount', 'ready_for_review', 'readyForReview', 'ready_for_review_count', 'readyForReviewCount', 'weak', 'weak_count', 'weakCount'], fallbackReview);
        var complete = readLoggedInStateNumber(source, ['complete', 'optimized', 'optimised', 'optimized_count', 'optimizedCount', 'optimised_count', 'optimisedCount'], fallbackComplete);
        var total = readLoggedInStateNumber(source, ['total', 'total_images', 'totalImages', 'total_count', 'totalCount'], fallbackTotal || (missing + review + complete));
    ...
    function resolveLoggedInStateFromCounts(stateId, counts) {
        var state = String(stateId || '').toUpperCase();
        var missing = Math.max(0, parseInt(counts && counts.missing, 10) || 0);
        var review = Math.max(0, parseInt(counts && counts.review, 10) || 0);
    ...
    function getDashboardLibraryHref() {
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        return (root && root.getAttribute('data-bbai-library-url')) || 'admin.php?page=bbai-library';
    }

    function getDashboardReviewLibraryHref() {
        var root = document.querySelector('[data-bbai-dashboard-root="1"]');
        return (root && (root.getAttribute('data-bbai-needs-review-library-url') || root.getAttribute('data-bbai-library-url'))) || getDashboardLibraryHref();
    }

    function buildLoggedInDonutForCounts(stateId, counts, existingDonut) {
        var total = Math.max(1, parseInt(counts.total, 10) || (counts.missing + counts.review + counts.complete) || 1);
        var pct = Math.max(0, Math.min(100, Math.round((counts.complete / total) * 100)));
        var donut = Object.assign({}, existingDonut || {});
        donut.segments = {
        ...
        if (stateId === 'MISSING_ALT' || stateId === 'MIXED_ATTENTION') {
            donut.pct = pct;
            donut.color = 'amber';
            donut.animated = false;
            donut.center_label = String(formatDashboardNumber(counts.missing));
            donut.center_sub_label = 'missing ALT';
        } else if (stateId === 'NEEDS_REVIEW') {
            donut.pct = pct;
            donut.color = 'blue';
            donut.animated = false;
            donut.center_label = String(formatDashboardNumber(counts.review));
            donut.center_sub_label = 'to review';
        } else if (stateId === 'ALL_CLEAR') {
            donut.pct = 100;
            donut.color = 'green';
            donut.animated = false;
            donut.center_label = '✓';
            donut.center_sub_label = '';
            donut.segments = {
                optimized: Math.max(total, counts.complete),
                weak: 0,
                missing: 0,
                total: Math.max(total, counts.complete, 1)
            };
        }
        return donut;
    }

    function isLoggedInAllClearPayload(stateData) {
        var state = String(stateData && stateData.state ? stateData.state : '').toUpperCase();
        var donut = stateData && stateData.donut && typeof stateData.donut === 'object' ? stateData.donut : {};
        var segments = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
        var missing = Math.max(0, parseInt(segments.missing, 10) || 0);
        var weak = Math.max(0, parseInt(segments.weak, 10) || 0);
        var optimized = Math.max(0, parseInt(segments.optimized, 10) || 0);
    ...
    function buildLoggedInAllClearStateData(stateData, counts) {
        var nextData = Object.assign({}, stateData || {});
        var safeCounts = counts && typeof counts === 'object' ? counts : {};
        var complete = Math.max(
            0,
            parseInt(safeCounts.complete, 10) ||
            parseInt(safeCounts.total, 10) ||
            0
        );
        var total = Math.max(complete, parseInt(safeCounts.total, 10) || complete || 1);

        nextData.state = 'ALL_CLEAR';
        nextData.counts = Object.assign({}, nextData.counts || {}, {
            missing: 0,
            review: 0,
            needs_review: 0,
            needsReview: 0,
            ...
            to_review: 0,
            toReview: 0,
            complete: Math.max(complete, total),
            optimized: Math.max(complete, total),
            optimised: Math.max(complete, total),
            total: total
        });
        nextData.hero = Object.assign({}, nextData.hero || {}, {
        ...
        nextData.donut = buildLoggedInDonutForCounts('ALL_CLEAR', {
            missing: 0,
            review: 0,
            complete: total,
            total: total
    ...
    function coerceLoggedInDashboardStateData(stateData) {
        var counts = getLoggedInDashboardStateCounts(stateData);
        var requestedState = String(stateData && stateData.state ? stateData.state : '');
        var nextState = resolveLoggedInStateFromCounts(requestedState, counts);
        var heroData;
        var nextData;
        ...
        nextData = Object.assign({}, stateData || {});
        heroData = Object.assign({}, nextData.hero || {});
        nextData.state = nextState;
        nextData.donut = buildLoggedInDonutForCounts(nextState, counts, nextData.donut || {});

        if (nextState === 'MIXED_ATTENTION') {
            if (counts.missing > 20) {
                heroData.badge = { text: __('Action needed', 'beepbeep-ai-alt-text-generator'), mod: 'amber' };
            } else if (counts.missing > 5) {
                heroData.badge = { text: __('Recommended', 'beepbeep-ai-alt-text-generator'), mod: 'blue' };
            } else {
                heroData.badge = null;
            }
            heroData.headline = sprintf(__('%1$s images missing ALT text, %2$s ready for review', 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(counts.missing), formatDashboardNumber(counts.review));
            heroData.support = __('Generate ALT text first, then review the suggested descriptions before they go live.', 'beepbeep-ai-alt-text-generator');
            heroData.primary_cta = {
                label: sprintf(
                    _n('Generate ALT text for %s image', 'Generate ALT text for %s images', Math.max(1, counts.missing), 'beepbeep-ai-alt-text-generator'),
                    formatDashboardNumber(counts.missing)
                ),
                busy_label: BBAI_GENERATION_BUSY_LABEL,
                action: 'generate-missing',
                href: '#'
            };
            heroData.secondary_cta = counts.review > 0 ? { label: sprintf(_n('Review %s image', 'Review %s images', counts.review, 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(counts.review)), action: 'navigate', href: getDashboardReviewLibraryHref() } : null;
        } else if (nextState === 'MISSING_ALT') {
            if (counts.missing > 20) {
                heroData.badge = { text: __('Action needed', 'beepbeep-ai-alt-text-generator'), mod: 'amber' };
            } else if (counts.missing > 5) {
                heroData.badge = { text: __('Recommended', 'beepbeep-ai-alt-text-generator'), mod: 'blue' };
            ...
            heroData.headline = sprintf(_n('%s image is missing ALT text', '%s images are missing ALT text', counts.missing, 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(counts.missing));
            heroData.support = __('Generate the missing ALT text now to keep your library accessible, searchable, and up to date.', 'beepbeep-ai-alt-text-generator');
            heroData.primary_cta = {
                label: sprintf(
                    _n('Generate ALT text for %s image', 'Generate ALT text for %s images', Math.max(1, counts.missing), 'beepbeep-ai-alt-text-generator'),
                    formatDashboardNumber(counts.missing)
            ...
            heroData.secondary_cta = null;
        } else if (nextState === 'NEEDS_REVIEW') {
            heroData.badge = { text: __('Review ready', 'beepbeep-ai-alt-text-generator'), mod: 'blue' };
            heroData.headline = sprintf(_n('%s image is ready for review', '%s images are ready for review', counts.review, 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(counts.review));
            heroData.support = __('ALT text is ready for a quick review before it goes live.', 'beepbeep-ai-alt-text-generator');
            heroData.primary_cta = { label: __('Approve all', 'beepbeep-ai-alt-text-generator'), busy_label: __('Approving...', 'beepbeep-ai-alt-text-generator'), action: 'approve-all', href: '#' };
            heroData.secondary_cta = { label: __('Review individually →', 'beepbeep-ai-alt-text-generator'), action: 'navigate', href: getDashboardReviewLibraryHref() };
        }

        nextData.hero = heroData;
        return nextData;
    }

    function renderLoggedInHeroSummary(hero, stateData) {
        var wrap = hero.querySelector('.bbai-status-summary');
        var counts = getLoggedInDashboardStateCounts(stateData || {});
        var stateId = String(stateData && stateData.state ? stateData.state : '');
        var donut = stateData && stateData.donut && typeof stateData.donut === 'object' ? stateData.donut : {};
        var queuedTotal = Math.max(0, parseInt(donut.job_total, 10) || 0);
        var missingDisplay = (stateId === 'QUEUED' && queuedTotal > 0) ? queuedTotal : counts.missing;
        var m = String(formatDashboardNumber(missingDisplay));
        var r = String(formatDashboardNumber(counts.review));
        var mEl;
        var rEl;
        if (!wrap) {
            return;
        ...
        wrap.hidden = false;
        mEl = wrap.querySelector('.bbai-count[data-bbai-status-metric="missing"]');
        rEl = wrap.querySelector('.bbai-count[data-bbai-status-metric="review"]');
        if (mEl) {
            mEl.textContent = m;
        }
        if (rEl) {
    ...
    function updateLoggedInHeroProgress(hero, stateId) {
        var wrap = hero.querySelector('[data-bbai-li-flow="1"]');
        var mEl;
        var rEl;
        var missing;
        var review;
        ...
        var rev;
        var done;
        var hideStates = { QUEUED: 1, QUOTA_EXHAUSTED: 1, ERROR: 1, NO_IMAGES: 1 };
        if (!wrap) {
            return;
        }

        if (hideStates[stateId]) {
            wrap.hidden = true;
            return;
        }

        mEl = hero.querySelector('[data-bbai-status-metric="missing"]');
        rEl = hero.querySelector('[data-bbai-status-metric="review"]');
        missing = mEl ? parseInt(String(mEl.textContent || '').replace(/,/g, ''), 10) || 0 : 0;
        review = rEl ? parseInt(String(rEl.textContent || '').replace(/,/g, ''), 10) || 0 : 0;
        if (!mEl || !rEl) {
            wrap.hidden = true;
            return;
        }
        ...
        gen = missing > 0;
        rev = review > 0;
        done = missing === 0 && review === 0;
        wrap.hidden = false;

        ['generate', 'review', 'done'].forEach(function(key) {
            var node = wrap.querySelector('[data-bbai-flow-step="' + key + '"]');
            var on;
            if (!node) {
                return;
            }
            on = (key === 'generate' && gen) || (key === 'review' && rev) || (key === 'done' && done);
            node.classList.toggle('is-active', on);
            node.classList.toggle('is-inactive', !on);
        });
    }

    function ensureLoggedInHeroBadgeNode(hero) {
        var badge;
        var headline;
        ...
        badge = hero.querySelector('.bbai-li-state-badge');
        if (badge) {
            return badge;
        }

        headline = hero.querySelector('[data-bbai-li-hero-headline="1"]');
        if (!headline || !headline.parentNode) {
            return null;
        }

        badge = document.createElement('span');
        badge.className = 'bbai-li-state-badge bbai-li-state-badge--gray';
        badge.setAttribute('aria-hidden', 'true');
        headline.parentNode.insertBefore(badge, headline);

        return badge;
    }

    function ensureLoggedInHeroCtaNode(hero, selector, className, attrName) {
        var row;
        var node;
        ...
        node = hero.querySelector(selector);
        if (node) {
            return node;
        }

        row = hero.querySelector('.bbai-li-cta-row');
        if (!row) {
            return null;
        }
    ...
    function configureLoggedInHeroCta(node, cta, isPrimary) {
        var content;
        var loadingLabel;
        var spinner;
        ...
        if (isPrimary) {
            node.setAttribute('data-bbai-li-action', cta.action || '');
            node.setAttribute('data-busy-label', cta.action === 'generate-missing' ? BBAI_GENERATION_BUSY_LABEL : (cta.busy_label || __('Working...', 'beepbeep-ai-alt-text-generator')));
            if (cta.action === 'approve-all') {
                ...
                content.textContent = String(cta.label);
                loadingLabel.textContent = cta.busy_label || __('Approving...', 'beepbeep-ai-alt-text-generator');
                return;
            }

            node.textContent = String(cta.label);
            if (cta.action === 'generate-missing') {
                node.setAttribute('data-action', 'generate-missing');
                node.setAttribute('data-bbai-action', 'generate_missing');
                node.setAttribute('data-bbai-generation-action', '1');
                node.classList.add('bbai-generate-button');
            } else {
                node.removeAttribute('data-action');
                node.removeAttribute('data-bbai-action');
                node.removeAttribute('data-bbai-generation-action');
                node.classList.remove('bbai-generate-button');
            }
        } else {
            node.textContent = String(cta.label);
            node.setAttribute('data-action', cta.action || '');
            node.classList.toggle('bbai-li-btn-secondary--subtle-rescan', cta.action === 'rescan-media-library' && node.classList && node.classList.contains('bbai-li-btn-secondary'));
            if (cta.busy_label) {
                node.setAttribute('data-busy-label', String(cta.busy_label));
            } else {
                node.removeAttribute('data-busy-label');
            }
        }
    }

    function updateLoggedInHeroBadge(hero, badgeData) {
        var badge = ensureLoggedInHeroBadgeNode(hero);
        if (!badge || !badgeData || !badgeData.text) {
            if (badge) {
                badge.hidden = true;
            }
            return;
        ...
        badge.hidden = false;
        removeClassesWithPrefix(badge, 'bbai-li-state-badge--');
        badge.classList.add('bbai-li-state-badge--' + String(badgeData.mod || 'gray').replace(/[^a-z0-9_-]/gi, ''));
        badge.textContent = String(badgeData.text);
    }

    function updateLoggedInDonutContextAction(hero, stateId, missingN, weakN, primary, secondary) {
        var donutArea = hero ? hero.querySelector('.bbai-li-donut-area') : null;
        var actionWrap = hero ? hero.querySelector('.bbai-li-donut__action-wrap') : null;
        var actionNode = hero ? hero.querySelector('.bbai-li-donut__action-cm') : null;
        var reviewBlock = hero ? hero.querySelector('[data-bbai-li-donut-review-block="1"]') : null;
        var cfg = null;

        if ((stateId === 'MISSING_ALT' || stateId === 'MIXED_ATTENTION') && missingN > 0 && primary && primary.href) {
            cfg = {
                href: primary.href,
                action: primary.action || 'generate-missing',
                label: __('Generate ALT text →', 'beepbeep-ai-alt-text-generator')
            };
        } else if (stateId === 'MIXED_ATTENTION' && missingN === 0 && weakN > 0 && secondary && secondary.href) {
            cfg = {
                href: secondary.href,
                action: secondary.action || 'navigate',
                label: __('Review images →', 'beepbeep-ai-alt-text-generator')
            };
        } else if (stateId === 'ALL_CLEAR' && primary && primary.href) {
            cfg = {
                href: primary.href,
        ...
        if (!actionWrap && donutArea) {
            actionWrap = document.createElement('p');
            actionWrap.className = 'bbai-li-donut__action-wrap';
            if (reviewBlock && reviewBlock.parentNode === donutArea) {
                donutArea.insertBefore(actionWrap, reviewBlock);
            } else {
                donutArea.appendChild(actionWrap);
            }
        }
        ...
        if (!actionNode || actionNode.parentNode !== actionWrap) {
            actionWrap.innerHTML = '';
            actionNode = document.createElement('a');
            actionNode.className = 'bbai-li-donut__action-cm';
            actionWrap.appendChild(actionNode);
        }

        actionWrap.hidden = false;
        actionNode.setAttribute('href', cfg.href);
        actionNode.setAttribute('data-action', cfg.action);
        actionNode.textContent = cfg.label;
    }

    function updateLoggedInHeroDonut(hero, stateData) {
        var stateId = String(stateData && stateData.state ? stateData.state : '');
        var donut = stateData && stateData.donut && typeof stateData.donut === 'object' ? stateData.donut : {};
        var heroData = stateData && stateData.hero && typeof stateData.hero === 'object' ? stateData.hero : {};
        var donutNode = hero.querySelector('[data-bbai-li-donut="1"]');
        var valueNode = hero.querySelector('[data-bbai-li-donut-label="1"]');
        var subNode = hero.querySelector('[data-bbai-li-donut-sub="1"]');
        var helperNode = hero.querySelector('[data-bbai-li-donut-helper="1"]');
        var metaNode = hero.querySelector('[data-bbai-li-donut-meta="1"]') || hero.querySelector('.bbai-li-donut__meta');
        var donutCard = hero.querySelector('.bbai-li-card--donut');
        var color = String(donut.color || 'gray');
        var tone = getLoggedInHeroTone(color);
        ...
        var weakN;
        var centerText;
        seg = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
        missingN = Math.max(0, parseInt(seg.missing, 10) || 0);
        weakN = Math.max(0, parseInt(seg.weak, 10) || 0);
        var clickable = (
        ...
        var actionLabel = (missingN > 0)
            ? __('Generate ALT text', 'beepbeep-ai-alt-text-generator')
            : stateId === 'NEEDS_REVIEW'
            ? __('Review images', 'beepbeep-ai-alt-text-generator')
            : (primary && primary.label ? primary.label : __('Take action', 'beepbeep-ai-alt-text-generator'));
        var metaText;

        if (donutNode) {
            removeClassesWithPrefix(donutNode, 'bbai-command-donut--');
            donutNode.classList.add('bbai-command-donut--' + tone);
            donutNode.classList.toggle('bbai-li-donut--animated', !!donut.animated);
            donutNode.setAttribute('data-bbai-li-donut-pct', String(Math.max(0, Math.min(100, parseInt(donut.pct, 10) || 0))));
            donutNode.setAttribute('data-bbai-li-donut-color', color);
            donutNode.setAttribute('aria-label', String(donut.aria_label || ''));
            donutNode.style.background = buildLoggedInDonutBackground(stateId, donut.segments || {});
        }

        if (valueNode) {
            removeClassesWithPrefix(valueNode, 'bbai-li-donut__value--');
            seg = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
            missingN = Math.max(0, parseInt(seg.missing, 10) || 0);
            weakN = Math.max(0, parseInt(seg.weak, 10) || 0);
            centerText = donut && donut.center_label !== undefined && donut.center_label !== null ? String(donut.center_label) : '';
            if (['QUEUED', 'PROCESSING', 'ERROR', 'NO_IMAGES'].indexOf(stateId) !== -1) {
                if (centerText === '') {
                    if (stateId === 'ERROR' || stateId === 'NO_IMAGES') {
                        valueNode.textContent = '—';
                    } else {
                        valueNode.textContent = String(formatDashboardNumber(missingN));
                    }
                } else {
                ...
                if (missingN > 0) {
                    ...
                    valueNode.classList.add('bbai-li-donut__value--center-review');
                } else {
                    valueNode.classList.add('bbai-li-donut__value--center-clear');
                }
            }
        }
        ...
        if (helperNode) {
            seg = donut && donut.segments && typeof donut.segments === 'object' ? donut.segments : {};
            missingN = Math.max(0, parseInt(seg.missing, 10) || 0);
            // Match PHP: queued/processing states use donut meta + strip for trust copy, not the static missing helper.
            var suppressHelperForJob = stateId === 'QUEUED' || stateId === 'PROCESSING';
            if (missingN > 0 && !suppressHelperForJob) {
                helperNode.textContent = __('Fix these first to improve SEO and accessibility.', 'beepbeep-ai-alt-text-generator');
                helperNode.hidden = false;
            } else {
        ...
        metaText = getLoggedInDonutMeta(stateId, donut, helperNode);
        if (metaNode) {
            metaNode.textContent = metaText;
            metaNode.hidden = !metaText;
        }

        reviewBlock = hero.querySelector('[data-bbai-li-donut-review-block="1"]');
        reviewLinkNode = hero.querySelector('[data-bbai-li-donut-review-link="1"]');
        reviewPromptNode = hero.querySelector('[data-bbai-li-donut-review-prompt="1"]');
        if (reviewBlock && reviewLinkNode && reviewPromptNode) {
            if (missingN > 0 && primary && primary.href) {
                reviewBlock.hidden = false;
                reviewLinkNode.setAttribute('href', primary.href);
                reviewLinkNode.setAttribute('data-action', primary.action || 'generate-missing');
                reviewPromptNode.textContent = __('Generate ALT text to move to review.', 'beepbeep-ai-alt-text-generator');
                reviewLinkNode.textContent = __('Generate ALT text →', 'beepbeep-ai-alt-text-generator');
            } else if (weakN > 0 && missingN === 0 && secondary && secondary.href) {
                reviewBlock.hidden = false;
                reviewLinkNode.setAttribute('href', secondary.href);
                reviewLinkNode.setAttribute('data-action', secondary.action || 'navigate');
                reviewPromptNode.textContent = __('Review now to complete your optimisation', 'beepbeep-ai-alt-text-generator');
                reviewLinkNode.textContent = __('Review images →', 'beepbeep-ai-alt-text-generator');
            } else {
                reviewBlock.hidden = true;
            }
        }

        updateLoggedInDonutContextAction(hero, stateId, missingN, weakN, primary, secondary);

        if (donutCard) {
            donutCard.classList.toggle('bbai-li-card--donut-clickable', !!clickable);
            if (clickable) {
                donutCard.setAttribute('role', 'button');
                donutCard.setAttribute('tabindex', '0');
                donutCard.setAttribute('data-bbai-li-donut-card-trigger', '1');
                donutCard.setAttribute('aria-label', String(actionLabel || ''));
            } else {
                donutCard.removeAttribute('role');
                donutCard.removeAttribute('tabindex');
                donutCard.removeAttribute('data-bbai-li-donut-card-trigger');
                donutCard.removeAttribute('aria-label');
            }
        }
    }
    ...
    function clearLoggedInReviewSurface(stateId) {
        ...
        Array.prototype.forEach.call(document.querySelectorAll('[data-bbai-li-surface="ReviewQueue"]'), function(surface) {
            var table = surface.querySelector('.bbai-li-image-table__table');
            var body = surface.querySelector('[data-bbai-li-table-body]');
            var loading = surface.querySelector('[data-bbai-li-table-loading]');
            var empty = surface.querySelector('.bbai-li-image-table__empty');
            var approveCta = surface.querySelector('[data-action="approve-all"]');
            ...
            if (!empty) {
                ...
                empty.className = 'bbai-li-image-table__empty';
            ...
            empty.hidden = false;
            empty.textContent = __('No images in the review queue.', 'beepbeep-ai-alt-text-generator');
            if (approveCta) {
                approveCta.hidden = true;
                approveCta.setAttribute('aria-disabled', 'true');
            }
        });
    }

    function updateLoggedInAllClearUpgradePanel(hero, stateId) {
        var dashboardRoot = document.querySelector('[data-bbai-dashboard-root="1"]');
        var isPremium = !!(dashboardRoot && dashboardRoot.getAttribute('data-bbai-is-premium') === '1');
        var shouldShow = stateId === 'ALL_CLEAR' && !isPremium;
        var panel = hero ? hero.querySelector('[data-bbai-all-clear-upgrade="1"]') : null;
        var actionBlock;
        var creditUsage;
        var ctaRow;
        var upgradeCta;
        ...
        if (!panel && shouldShow) {
            actionBlock = hero.querySelector('.bbai-action-block');
            creditUsage = hero.querySelector('[data-bbai-hero-credit-usage]');
            ctaRow = hero.querySelector('.bbai-li-cta-row');
            if (!actionBlock || !ctaRow) {
                return;
            }

            panel = document.createElement('div');
            panel.className = 'bbai-all-clear-upgrade';
            panel.setAttribute('data-bbai-all-clear-upgrade', '1');
            panel.innerHTML =
            ...
            if (creditUsage && creditUsage.parentNode === actionBlock) {
                actionBlock.insertBefore(panel, creditUsage);
            } else {
                actionBlock.appendChild(panel);
            }
        }
        ...
        if (panel) {
            ...
            upgradeCta = panel.querySelector('[data-action="show-upgrade-modal"]');
            if (upgradeCta) {
                upgradeCta.removeAttribute('aria-hidden');
            }
        }
    }
    ...
    function renderLoggedInDashboardHeroState(stateData, options) {
        ...
        stateData = coerceLoggedInDashboardStateData(stateData);
        var stateId = String(stateData && stateData.state ? stateData.state : '');
        var heroData = stateData && stateData.hero && typeof stateData.hero === 'object' ? stateData.hero : {};
        var headlineNode;
        var supportNode;
        ...
        hero.setAttribute('data-bbai-li-state', stateId);
        hero.setAttribute('data-bbai-li-variant', String(heroData.variant || 'default'));
        (function syncActivationCounts() {
            var counts = getLoggedInDashboardStateCounts(stateData || {});
        ...
        updateLoggedInHeroBadge(hero, heroData.badge || null);

        headlineNode = hero.querySelector('[data-bbai-li-hero-headline="1"]');
        if (headlineNode && heroData.headline) {
            headlineNode.textContent = String(heroData.headline);
        }

        supportNode = hero.querySelector('[data-bbai-li-hero-support="1"]');
        if (supportNode && heroData.support) {
            supportNode.textContent = String(heroData.support);
        }

        hintNode = hero.querySelector('[data-bbai-hero-cta-hint="1"]');
        if (hintNode) {
            if (stateId === 'ALL_CLEAR') {
                ...
                hintNode.textContent = __('Complete your optimisation', 'beepbeep-ai-alt-text-generator');
                hintNode.hidden = false;
                hintNode.removeAttribute('aria-hidden');
            } else if (stateId === 'MISSING_ALT' || stateId === 'MIXED_ATTENTION' || stateId === 'QUEUED') {
                hintNode.textContent = __('Generate ALT text to move to review.', 'beepbeep-ai-alt-text-generator');
                hintNode.hidden = false;
                hintNode.removeAttribute('aria-hidden');
            } else {
                hintNode.textContent = '';
        ...
        if (stateId === 'ALL_CLEAR') {
            ...
            hero.classList.remove('bbai-all-clear-state', 'bbai-all-clear-state--entered');
            hero.removeAttribute('data-bbai-all-clear-hydration-applied');
        }

        hero.classList.toggle('bbai-li-hero--needs-review', stateId === 'NEEDS_REVIEW');
        updateLoggedInAllClearUpgradePanel(hero, stateId);
        ...
        if (!opts.skipCtas) {
            ...
            // PROCESSING without a pausable job omits primary_cta in PHP, so the whole
            // .bbai-li-cta-primary-col (including inline "Review individually") is never
            // SSR'd. Recreate the column + link when entering NEEDS_REVIEW.
            var ctaRow = hero.querySelector('.bbai-li-cta-row');
            var primaryCol = hero.querySelector('.bbai-li-cta-primary-col');
            if (stateId === 'NEEDS_REVIEW' && ctaRow) {
                if (!primaryCol) {
                    primaryCol = document.createElement('div');
                    primaryCol.className = 'bbai-li-cta-primary-col';
                    ctaRow.insertBefore(primaryCol, ctaRow.firstChild);
                }
                if (primaryCta && primaryCta.parentNode !== primaryCol) {
                    primaryCol.appendChild(primaryCta);
                }
            }
            configureLoggedInHeroCta(primaryCta, heroData.primary_cta || null, true);

            var inlineSecondary = hero.querySelector('[data-bbai-li-secondary-inline-cta="1"]');
            if (stateId === 'NEEDS_REVIEW' && primaryCol && !inlineSecondary) {
                inlineSecondary = document.createElement('a');
                inlineSecondary.className = 'bbai-hero-review-inline-link';
                inlineSecondary.setAttribute('data-bbai-li-secondary-inline-cta', '1');
                primaryCol.appendChild(inlineSecondary);
            }
            var rowSecondary = hero.querySelector('[data-bbai-li-secondary-cta]');

            if (stateId === 'NEEDS_REVIEW') {
                if (rowSecondary) {
                    rowSecondary.hidden = true;
                    rowSecondary.setAttribute('aria-hidden', 'true');
                }
                if (inlineSecondary) {
                    configureLoggedInHeroCta(inlineSecondary, heroData.secondary_cta || null, false);
                }
            } else if (stateId === 'ALL_CLEAR') {
                if (inlineSecondary) {
                    inlineSecondary.hidden = true;
                    inlineSecondary.setAttribute('aria-hidden', 'true');
                    inlineSecondary.setAttribute('aria-disabled', 'true');
                }
                if (rowSecondary) {
                    rowSecondary.hidden = true;
                    rowSecondary.setAttribute('aria-hidden', 'true');
                }
                var allClearRescan = hero.querySelector('[data-bbai-li-all-clear-rescan="1"]');
                if (allClearRescan) {
                    configureLoggedInHeroCta(allClearRescan, heroData.secondary_cta || null, false);
                    if (heroData.secondary_cta && heroData.secondary_cta.label) {
                        allClearRescan.hidden = false;
                        allClearRescan.removeAttribute('aria-hidden');
                    } else {
                        allClearRescan.hidden = true;
                        allClearRescan.setAttribute('aria-hidden', 'true');
                    }
                }
                var allClearLibrary = hero.querySelector('[data-bbai-li-all-clear-library="1"]');
                if (allClearLibrary) {
                    if (heroData.library_cta && heroData.library_cta.href) {
                        configureLoggedInHeroCta(allClearLibrary, heroData.library_cta, false);
                        allClearLibrary.hidden = false;
                        allClearLibrary.removeAttribute('aria-hidden');
                    } else {
                        allClearLibrary.hidden = true;
                        allClearLibrary.setAttribute('aria-hidden', 'true');
                    }
                }
            } else {
                if (inlineSecondary) {
                ...
                var allClearRescanHide = hero.querySelector('[data-bbai-li-all-clear-rescan="1"]');
                if (allClearRescanHide) {
                    allClearRescanHide.hidden = true;
                    allClearRescanHide.setAttribute('aria-hidden', 'true');
                }
                secondaryCta = rowSecondary || ensureLoggedInHeroCtaNode(
                    hero,
                    '[data-bbai-li-secondary-cta]',
                    'bbai-li-btn-secondary bbai-btn bbai-btn-secondary bbai-button-secondary',
                    'data-bbai-li-secondary-cta'
                );
                configureLoggedInHeroCta(secondaryCta, heroData.secondary_cta || null, false);
                if (secondaryCta && heroData.secondary_cta && heroData.secondary_cta.label) {
                    secondaryCta.hidden = false;
                    secondaryCta.removeAttribute('aria-hidden');
                }
            }
        }

        renderLoggedInHeroSummary(hero, stateData);
        updateLoggedInHeroDonut(hero, stateData);
        updateLoggedInHeroProgress(hero, stateId);

        var heroUsageSync = hero.querySelector('[data-bbai-hero-credit-usage]');
        if (heroUsageSync) {
            heroUsageSync.classList.toggle('bbai-credit-usage--all-clear-muted', stateId === 'ALL_CLEAR');
            var usageBlock = stateData && stateData.usage && typeof stateData.usage === 'object' ? stateData.usage : null;
            var limSync;
            var remSync;
            var usedSync;
            if (usageBlock && parseInt(usageBlock.total, 10) > 0) {
                limSync = Math.max(1, parseInt(usageBlock.total, 10) || 1);
                usedSync = Math.max(0, parseInt(usageBlock.used, 10) || 0);
                remSync = Math.max(0, parseInt(usageBlock.remaining, 10) || 0);
            } else {
                remSync = parseInt(heroUsageSync.getAttribute('data-bbai-hero-credits-remaining') || '0', 10);
                limSync = parseInt(heroUsageSync.getAttribute('data-bbai-hero-credits-limit') || '1', 10);
                usedSync = Math.max(0, limSync - Math.max(0, remSync));
            }
            updateGlobalCreditsSurfacesFromCounts(usedSync, limSync, Math.max(0, remSync));
        }
    ...
    function runDashboardApproveAll(trigger, e) {
        ...
        if (
            bbaiDashboardApproveAllInFlight ||
            (trigger && trigger.classList && trigger.classList.contains('is-loading'))
        ) {
            return false;
        }

        endpoint = getDashboardApproveAllEndpoint();
        nonce = getLibraryRestNonce();
        if (!endpoint || !nonce) {
            notifyApproveAllFeedback('error', __('Unable to approve these images right now.', 'beepbeep-ai-alt-text-generator'));
            return false;
        }

        bbaiDashboardApproveAllInFlight = true;
        restoreBusyState = setBusyStateForControls(
        ...
        ajaxApproveAllRequest(endpoint, nonce)
            .then(function(response) {
                ...
                var statsPayload = payload && payload.stats && typeof payload.stats === 'object' ? payload.stats : null;
                var dashboardState = payload && payload.dashboard_state && typeof payload.dashboard_state === 'object'
                ...
                notifyApproveAllFeedback(
                    approvedCount > 0 ? 'success' : 'info',
                    approvedCount > 0
                        ? sprintf(
                            _n('%d image approved', '%d images approved', approvedCount, 'beepbeep-ai-alt-text-generator'),
                            approvedCount
                        )
    ...
    function queueImages(ids, source, options, callback) {
        ...
        .done(function(response) {
            ...
            if (queueResult && queueResult.success) {
                ...
                if (queued > 0) {
                    callback(true, queued, null, ids.slice(0), responseData);
                } else {
                    window.BBAI_LOG && window.BBAI_LOG.warn('[AI Alt Text] No images were queued. Response:', response);
    ...
    function getBulkProgressDefaultTitle(source) {
        return source === 'fix-all-issues'
            ...
            : __('Continuing optimization…', 'beepbeep-ai-alt-text-generator');
    }
    ...
    function isBulkProgressCompleteState(state) {
        if (!state) {
            return false;
        }

        var total = Math.max(0, parseInt(state.total, 10) || 0);
        var current = Math.max(0, parseInt(state.current, 10) || 0);
        var processed = Math.max(0, parseInt(state.processed, 10) || 0);
        var failed = Math.max(0, parseInt(state.failed, 10) || 0);
        var skipped = Math.max(0, parseInt(state.skipped, 10) || 0);
    ...
    function buildBulkProgressHeaderTitle(state) {
        ...
        return __('Processing your images', 'beepbeep-ai-alt-text-generator');
    }
    ...
    function buildBulkProgressHeaderSubtitle(state) {
        ...
        return __('Optimising your images for SEO and accessibility. Closing this window does not cancel the job.', 'beepbeep-ai-alt-text-generator');
    }

    function buildBulkProgressSuccessSubtitleHtml(stateOrCount) {
        var state = stateOrCount && typeof stateOrCount === 'object' ? stateOrCount : null;
        var safeCount = Math.max(0, parseInt(state ? state.processed : stateOrCount, 10) || 0);
        var actionable = state ? getDashboardActionableStateSnapshot() : null;

        if (safeCount <= 0) {
            if (actionable && actionable.key === 'review' && actionable.reviewCount > 0) {
                return escapeHtml(
                    sprintf(
                        _n('%s image ready to review', '%s images ready to review', actionable.reviewCount, 'beepbeep-ai-alt-text-generator'),
                        formatDashboardNumber(actionable.reviewCount)
            ...
            if (actionable && actionable.key === 'complete') {
                return escapeHtml(__('All images optimised', 'beepbeep-ai-alt-text-generator'));
            }

            return escapeHtml(__('No images were optimised successfully', 'beepbeep-ai-alt-text-generator'));
        }
        var template = _n(
            '%s image optimised successfully',
            '%s images optimised successfully',
            safeCount,
            'beepbeep-ai-alt-text-generator'
        );

        return escapeHtml(sprintf(template, formatDashboardNumber(safeCount)));
    }

    function buildBulkProgressSuccessTitleHtml(stateOrCount) {
        var state = stateOrCount && typeof stateOrCount === 'object' ? stateOrCount : null;
        var safeCount = state
            ? Math.max(0, parseInt(state.processed, 10) || 0)
            : Math.max(0, parseInt(stateOrCount, 10) || 0);
        var attempted = state ? Math.max(0, parseInt(state.total, 10) || 0) : 0;
        var failed = state ? Math.max(0, parseInt(state.failed, 10) || 0) : 0;
        var template;

        if (safeCount <= 0) {
            if (attempted > 0 && failed > 0) {
                return escapeHtml(
                    __('Generation finished, but no images were optimised successfully.', 'beepbeep-ai-alt-text-generator')
                );
            }
            if (attempted > 0) {
                return escapeHtml(__('No images were optimised in this run.', 'beepbeep-ai-alt-text-generator'));
            }
            return escapeHtml(__('All images optimised', 'beepbeep-ai-alt-text-generator'));
        }
    ...
    function buildBulkProgressSuccessTitleText(stateOrCount) {
        var state = stateOrCount && typeof stateOrCount === 'object' ? stateOrCount : null;
        var safeCount = state
            ? Math.max(0, parseInt(state.processed, 10) || 0)
            : Math.max(0, parseInt(stateOrCount, 10) || 0);
        ...
        if (safeCount <= 0) {
            if (attempted > 0 && failed > 0) {
                return __('Generation finished, but no images were optimised successfully.', 'beepbeep-ai-alt-text-generator');
            }
            if (attempted > 0) {
                return __('No images were optimised in this run.', 'beepbeep-ai-alt-text-generator');
            }
            return __('All images optimised', 'beepbeep-ai-alt-text-generator');
        }
        ...
        return '\u2714 ' + sprintf(
            ...
            formatDashboardNumber(safeCount)
        );
    }

    function buildBulkProgressMeterLabel(current, total) {
        var safeCurrent = Math.max(0, parseInt(current, 10) || 0);
        var safeTotal = Math.max(0, parseInt(total, 10) || 0);

        if (safeTotal <= 0) {
            return __('Preparing your images…', 'beepbeep-ai-alt-text-generator');
        }
    ...
    function getBulkProgressCompletionCtaConfig(state) {
        var usage = getUsageSnapshot(state && state.quotaError && state.quotaError.usage ? state.quotaError.usage : null);
        var libraryUrl = getAltLibraryAdminUrl();
        var remaining = Math.max(0, parseInt(usage && usage.remaining, 10) || 0);
        var isTrialUser = isAnonymousTrialUsage(usage);
    ...
    function getBulkProgressDismissSecondaryCta() {
        return {
            label: __('Close', 'beepbeep-ai-alt-text-generator'),
            action: 'dismiss-completion'
        };
    }

    function getBulkProgressCompletionPresentation(state) {
        var actionable = getDashboardActionableStateSnapshot();
        var root = getDashboardRootNode();
        var hasConnectedAccount = !!(root && root.getAttribute('data-bbai-has-connected-account') === '1');
        var reviewUrl = buildDashboardReviewLibraryUrl(
            (root && (root.getAttribute('data-bbai-needs-review-library-url') || root.getAttribute('data-bbai-library-url'))) ||
            getAltLibraryAdminUrl()
        );
        var libraryUrl = String((root && root.getAttribute('data-bbai-library-url')) || getAltLibraryAdminUrl() || '');
        var processedN = Math.max(0, parseInt(state.processed, 10) || 0);
        var attemptedN = Math.max(0, parseInt(state.total, 10) || 0);
        var failedN = Math.max(0, parseInt(state.failed, 10) || 0);
        var title = buildBulkProgressSuccessTitleHtml(state);
        var successLine;

        if (processedN > 0) {
            successLine = __('🎉 Your images are now SEO-ready', 'beepbeep-ai-alt-text-generator');
        } else if (attemptedN > 0) {
            successLine = failedN > 0
                ? __('Some images could not be processed. Check the library for details.', 'beepbeep-ai-alt-text-generator')
                : __('No images were optimised in this run.', 'beepbeep-ai-alt-text-generator');
        } else {
            successLine = __('Review the result and try again.', 'beepbeep-ai-alt-text-generator');
        }
        var ctaConfig = getBulkProgressCompletionCtaConfig(state);
        var supportingLine = '';

        if (actionable.key === 'review') {
            if (processedN > 0) {
                successLine = __('Review generated ALT text before publishing.', 'beepbeep-ai-alt-text-generator');
            }
            if (hasConnectedAccount) {
                ctaConfig = {
                    primary: {
        ...
        } else if (actionable.key === 'complete') {
            if (processedN > 0) {
                successLine = __('🎉 Your images are now SEO-ready', 'beepbeep-ai-alt-text-generator');
            }
            if (hasConnectedAccount) {
                ctaConfig = {
                    primary: {
                        label: __('View results', 'beepbeep-ai-alt-text-generator'),
                        action: 'library',
                        url: libraryUrl
                    },
                    secondary: getBulkProgressDismissSecondaryCta()
        ...
        if (processedN > 0 && root && root.getAttribute('data-bbai-is-premium') !== '1') {
            ...
            if (isFinite(uUsed) && isFinite(uLim) && uLim > 0) {
                ...
                supportingLine = [creditNote, supportingLine].filter(function(s) { return !!s; }).join(' ');
            }
        }
        ...
        return {
            ...
            automationCta: processedN > 0 && root && root.getAttribute('data-bbai-is-premium') !== '1'
        };
    }

    function getDashboardAuthMode() {
        var root = getDashboardRootNode && typeof getDashboardRootNode === 'function' ? getDashboardRootNode() : null;
        var mode = root ? String(root.getAttribute('data-bbai-auth-mode') || '') : '';
        return mode || String(window.BBAI_AUTH_MODE || '');
    }

    function readPostGenerationCountsSnapshot() {
        var root = getDashboardRootNode && typeof getDashboardRootNode === 'function' ? getDashboardRootNode() : null;
        if (!root) {
            return {
                missing: 0,
        ...
        var missing = Math.max(0, parseInt(root.getAttribute('data-bbai-missing-count') || '0', 10) || 0);
        var review = Math.max(0, parseInt(root.getAttribute('data-bbai-weak-count') || '0', 10) || 0);
        var optimized = Math.max(0, parseInt(root.getAttribute('data-bbai-optimized-count') || '0', 10) || 0);
        var total = Math.max(0, parseInt(root.getAttribute('data-bbai-total-count') || '0', 10) || (missing + review + optimized));
        return {
            missing: missing,
    ...
    function computePostGenerationNextAction(snapshot) {
        snapshot = snapshot && typeof snapshot === 'object' ? snapshot : {};
        var missing = Math.max(0, parseInt(snapshot.missing, 10) || 0);
        var review = Math.max(0, parseInt(snapshot.ready_for_review, 10) || 0);
        var optimized = Math.max(0, parseInt(snapshot.optimized, 10) || 0);
        var authMode = String(snapshot.auth_mode || getDashboardAuthMode() || '');
        var trialExhausted = !!snapshot.trial_exhausted;
        var creditsRemaining = snapshot.credits_remaining !== undefined && snapshot.credits_remaining !== null && snapshot.credits_remaining !== ''
            ? Math.max(0, parseInt(snapshot.credits_remaining, 10) || 0)
            : null;
        ...
        if (review > 0) {
            return {
                headline: __('ALT text generated', 'beepbeep-ai-alt-text-generator'),
                supporting: __('Your images are ready to review.', 'beepbeep-ai-alt-text-generator'),
                primary: { label: __('Review ALT text', 'beepbeep-ai-alt-text-generator'), action: 'review-result' },
                secondary: { label: __('Back to dashboard', 'beepbeep-ai-alt-text-generator'), action: 'dashboard' }
            };
        }

        if (missing > 0 && (creditsRemaining === null || creditsRemaining > 0) && authMode !== 'guest_logged_out') {
            return {
                headline: __('ALT text generated', 'beepbeep-ai-alt-text-generator'),
                supporting: sprintf(
                    _n('You still have %s image missing ALT text.', 'You still have %s images missing ALT text.', missing, 'beepbeep-ai-alt-text-generator'),
                    formatDashboardNumber(missing)
                ),
                primary: { label: __('Fix remaining images', 'beepbeep-ai-alt-text-generator'), action: 'fix-more' },
                secondary: { label: __('Review ALT text', 'beepbeep-ai-alt-text-generator'), action: 'review-result' }
            };
        }

        if (authMode === 'authenticated_free' && creditsRemaining !== null && creditsRemaining <= 0) {
            return {
                headline: __('ALT text generated', 'beepbeep-ai-alt-text-generator'),
                supporting: __('You’re out of credits. Review your results or upgrade to continue.', 'beepbeep-ai-alt-text-generator'),
                primary: { label: __('Upgrade to continue', 'beepbeep-ai-alt-text-generator'), action: 'upgrade' },
                secondary: { label: __('Review ALT text', 'beepbeep-ai-alt-text-generator'), action: 'review-result' }
            };
        }
        ...
        if (missing <= 0 && review <= 0) {
            return {
                headline: __('ALT text generated', 'beepbeep-ai-alt-text-generator'),
                supporting: __('Your library is fully optimised.', 'beepbeep-ai-alt-text-generator'),
    ...
    function resolveBulkProgressCompletionCtaConfig(state) {
        var presentation = isBulkProgressCompleteState(state)
            ? getBulkProgressCompletionPresentation(state)
            : null;

        if (state && state.postGen && state.postGen.ctaConfig) {
            return state.postGen.ctaConfig;
        }
        return presentation ? presentation.ctaConfig : getBulkProgressCompletionCtaConfig(state);
    }

    function trackBulkProgressQuotaCtaShown($modal, state, ctaConfig) {
        ...
        dispatchAnalyticsEvent(
            ...
            buildBulkProgressAnalyticsPayload(state, {
                cta_label: ctaConfig && ctaConfig.primary && ctaConfig.primary.label
    ...
    function updateBulkProgressStatusNote($modal, message, tone) {
        var $note;
        ...
        $note = $modal.find('[data-bbai-bulk-progress-note]');
        if (!$note.length) {
            return;
        }

        if (!message) {
            $note.text('').prop('hidden', true).attr('data-tone', '');
            return;
        }

        $note.text(String(message)).prop('hidden', false).attr('data-tone', String(tone || 'neutral'));
    }

    function animateBulkProgressNumber(node, targetValue) {
        var finalValue = Math.max(0, parseInt(targetValue, 10) || 0);

        if (!node) {
            return;
        }

        node.textContent = formatDashboardNumber(finalValue);
    }

    function animateBulkProgressCompletion($modal, state) {
        var animationKey;
        var countNode;
        var checkNode;
        ...
        animationKey = [
            Math.max(0, parseInt(state.processed, 10) || 0),
            Math.max(0, parseInt(state.failed, 10) || 0),
            Math.max(0, parseInt(state.skipped, 10) || 0),
            state.quotaBlocked ? 1 : 0
        ].join(':');

        if ($modal.data('bbaiBulkCompleteAnimationKey') === animationKey) {
            return;
        }

        $modal.data('bbaiBulkCompleteAnimationKey', animationKey);
        countNode = $modal.find('[data-bbai-bulk-progress-success-count]').get(0);
        checkNode = $modal.find('[data-bbai-bulk-progress-success-check]').get(0);

        if (checkNode) {
            checkNode.classList.remove('is-visible');
        }

        window.requestAnimationFrame(function() {
            $modal.find('.bbai-bulk-progress__bar-fill, [data-bbai-bulk-progress-bar-fill]').css('width', '100%');
            $modal.find('.bbai-bulk-progress__bar[role="progressbar"]').attr('aria-valuenow', '100');
    ...
    function scrollBulkProgressResultsIntoView() {
        var target = document.querySelector('[data-bbai-trial-preview="1"]') ||
            document.querySelector('#bbai-dashboard-status-card') ||
            document.querySelector('[data-bbai-dashboard-status-card="1"]');
        var activePill;
        ...
        activePill = target.querySelector('[data-bbai-dashboard-status-pill].is-active, [data-bbai-dashboard-status-pill][aria-current="true"]');

        window.setTimeout(function() {
            if (activePill && typeof activePill.focus === 'function') {
                activePill.focus();
            }
        }, 180);
    ...
    function handleBulkProgressCompletionAction(actionConfig) {
        var action = actionConfig && actionConfig.action ? String(actionConfig.action) : '';
        var libraryUrl = actionConfig && actionConfig.url ? String(actionConfig.url) : getAltLibraryAdminUrl();
        var fixTrigger;
        var $modalDismiss;
        var reloadTimer;

        if (action === 'dismiss-completion') {
            $modalDismiss = $('#bbai-bulk-progress-modal');
            if ($modalDismiss.length) {
                reloadTimer = $modalDismiss.data('bbaiDashboardReloadTimer');
                if (reloadTimer) {
                    clearTimeout(reloadTimer);
                    $modalDismiss.removeData('bbaiDashboardReloadTimer');
                }
            }
            hideBulkProgress();
            return;
        ...
        if (!action) {
            minimizeBulkProgress('completion_action');
            return;
        }
        ...
        if (action === 'login') {
            ...
            if (typeof openAuthLoginModal === 'function') {
                openAuthLoginModal();
                return;
            }
            // Fallback: open auth modal in login mode if present.
            if (typeof window.showAuthModal === 'function') {
                window.showAuthModal('login', 'login');
            }
            return;
        ...
        if (action === 'upgrade') {
            minimizeBulkProgress('completion_upgrade');
            if (typeof window.showUpgradeModal === 'function') {
                window.showUpgradeModal('post_generation');
                return;
            }
            var upgradeBtn = document.querySelector('[data-action="show-upgrade-modal"]');
            if (upgradeBtn && typeof upgradeBtn.click === 'function') {
                upgradeBtn.click();
            }
            return;
        }
        ...
        if (action === 'upload') {
            minimizeBulkProgress('completion_upload');
            window.location.assign('upload.php');
            return;
        }

        if (action === 'review-result') {
            minimizeBulkProgress('completion_review');
            if (!scrollBulkProgressResultsIntoView() && typeof scrollToAltTable === 'function') {
                scrollToAltTable({ focusFirstItem: true });
            }
            return;
        }

        if (action === 'fix-more') {
            minimizeBulkProgress('completion_fix_more');
            fixTrigger = document.querySelector('[data-bbai-fix-dashboard="1"][data-action="generate-missing"]');
            if (fixTrigger && typeof fixTrigger.click === 'function') {
                window.setTimeout(function() {
                    fixTrigger.click();
                }, 60);
            } else {
                scrollBulkProgressResultsIntoView();
            }
            return;
        }
    ...
    function renderBulkProgressState($modal, state) {
        ...
        function logRender(eventName, payload) {
            if (!window.BBAI_LOG || typeof window.BBAI_LOG.info !== 'function') {
                return;
            }
            ...
            window.BBAI_LOG.info('[bbai-progress-modal]', out);
        }

        function mustFind($node, selector) {
            if ($node && $node.length) {
                return true;
            }
            logRender('progress_modal_missing_element', { selector: selector });
            return false;
        }

        // Defensive: if the modal skeleton is missing key nodes, bail without leaving a blank broken state.
        if (!mustFind($modal.find('.bbai-bulk-progress__title'), '.bbai-bulk-progress__title')) {
            return;
        }
        if (!mustFind($modal.find('[data-bbai-bulk-progress-log]'), '[data-bbai-bulk-progress-log]')) {
            return;
        }
        if (!mustFind($modal.find('[data-bbai-bulk-progress-complete]'), '[data-bbai-bulk-progress-complete]')) {
            return;
        }
        ...
        // Complete-mode CTAs stay focused on review; the modal close path preserves existing background handling.

        var prevMode = String($modal.attr('data-bbai-bulk-progress-mode') || '');
        if (prevMode !== mode) {
            logRender('progress_modal_mode_change', { from: prevMode || '(unset)', to: mode });
        }
        ...
        if ($subtitle.length) {
            if (isComplete && !hasError) {
                ...
                var sub = snap2 && snap2.supporting
                    ...
                    : __('Your images are ready to review.', 'beepbeep-ai-alt-text-generator');
                $subtitle.text(sub).prop('hidden', false);
            } else if (isComplete) {
                $subtitle.text('').prop('hidden', true);
            } else {
        ...
        if ($helper.length) {
            if (isComplete || !$helper.text()) {
                $helper.text('').prop('hidden', true);
            }
        }

        $meterLabel = $modal.find('[data-bbai-bulk-progress-meter-label]');
        $barContainer = $modal.find('[data-bbai-bulk-progress-bar-container]');
        if ($meterLabel.length) {
            if (isComplete) {
                $meterLabel.text('').prop('hidden', true);
            } else {
                $meterLabel.text(buildBulkProgressMeterLabel(state.current, state.total)).prop('hidden', false);
            }
        }
        if ($barContainer.length) {
            // Keep bar visible for complete/error (must show 100% in complete mode).
            $barContainer.prop('hidden', false);
        }

        $logContainer = $modal.find('[data-bbai-bulk-progress-log-container]');
        if ($logContainer.length) {
            // Never hide the log; completion should still show the full run history.
            $logContainer.prop('hidden', false);
            var $log = $modal.find('[data-bbai-bulk-progress-log]');
            if ($log.length && !$log.children().length && !isComplete) {
                $log.html(
                    '<div class="bbai-bulk-progress__log-empty" data-bbai-bulk-progress-log-empty>' +
                    escapeHtml(__('Preparing the first image…', 'beepbeep-ai-alt-text-generator')) +
                    '</div>'
                );
            }
        }
        ...
        if ($steps.length) {
            ...
            var current = Math.max(0, parseInt(state.current, 10) || 0);
            var total = Math.max(0, parseInt(state.total, 10) || 0);
            $steps.find('[data-bbai-bulk-progress-step]')
                .removeClass('is-done is-active is-error bbai-step--complete bbai-step--active bbai-step--pending')
                .addClass('bbai-step--pending');

            var setStep = function(key, cls) {
                var node = $steps.find('[data-bbai-bulk-progress-step="' + key + '"]');
                var alias = cls === 'is-done'
                    ? 'bbai-step--complete'
                    : (cls === 'is-active' ? 'bbai-step--active' : '');
                if (node.length) {
                    node.removeClass('bbai-step--pending').addClass(cls + (alias ? ' ' + alias : ''));
                }
            };

            setStep('prepare', 'is-done');
            if (!isComplete) {
                setStep('send', 'is-active');
                if (current > 0) {
                ...
                setStep('send', 'is-done');
                setStep('save', 'is-done');
                setStep('review', processed > 0 || total === 0 ? 'is-done' : 'is-active');
            }
        }

        $complete = $modal.find('[data-bbai-bulk-progress-complete]');
        if (!$complete.length) {
            return;
        }

        $runningActions = $modal.find('[data-bbai-bulk-progress-running-actions]');
        if ($runningActions.length) {
            $runningActions.prop('hidden', isComplete);
        }

        if (!isComplete) {
            $complete.find('[data-bbai-bulk-progress-success-check]').removeClass('is-visible');
        ...
        function applyActionButton($btn, cfg, which) {
            if (!$btn || !$btn.length) {
                logRender('progress_modal_missing_element', { selector: which });
                return;
            }
            var label = cfg && cfg.label ? String(cfg.label) : '';
            if (!label) {
                $btn.text('').attr('data-bbai-bulk-progress-action', '').attr('data-bbai-bulk-progress-url', '').prop('hidden', true);
                return;
            }
            $btn
                .text(label)
                .attr('data-bbai-bulk-progress-action', cfg.action ? String(cfg.action) : '')
                .attr('data-bbai-bulk-progress-url', cfg.url ? String(cfg.url) : '')
                .prop('hidden', false);
        }

        $primaryAction = $complete.find('[data-bbai-bulk-progress-primary]');
        applyActionButton($primaryAction, ctaConfig.primary || null, '[data-bbai-bulk-progress-primary]');

        $secondaryAction = $complete.find('[data-bbai-bulk-progress-secondary]');
        applyActionButton($secondaryAction, ctaConfig.secondary || null, '[data-bbai-bulk-progress-secondary]');

        if ($supportingLine.length) {
            $supportingLine.empty();
            if (presentation && presentation.supportingLine) {
                $supportingLine.append(document.createTextNode(presentation.supportingLine));
            }
            if (presentation && presentation.automationCta) {
                if (presentation.supportingLine) {
            ...
            $supportingLine.prop('hidden', !(presentation && (presentation.supportingLine || presentation.automationCta)));
        }

        trackBulkProgressQuotaCtaShown($modal, state, ctaConfig);
        animateBulkProgressCompletion($modal, state);
        logRender('progress_modal_complete_rendered', { total: state.total, completed: state.current });
    }
    ...
    function logBulkProgressQuotaSkip(skipCount, quotaCause) {
        ...
        updateBulkProgressStatusNote(
            $modal,
            __('Some images still need credits to finish.', 'beepbeep-ai-alt-text-generator'),
            'warning'
        );
    }
    ...
    function startLicensedBulkJobFlow(normalizedIds, source) {
        ...
        C.run({
            ...
            source: src,
            strings: {
                startingGeneration: BBAI_GENERATION_BUSY_LABEL,
            ...
            syncModalProgress: function(ctx) {
                ...
                if (terminal) {
                    ...
                    titleMsg = __('Processing your images', 'beepbeep-ai-alt-text-generator');
                } else {
                ...
                /* Bar uses finished item counts only (finishedSlots / totalN), not job.percentComplete. */
    ...
    function startInlineGeneration(idList, source) {
        ...
        var progressTitle = __('Processing your images', 'beepbeep-ai-alt-text-generator');
        var modalState;
        ...
        if (window.bbaiJobState) {
            window.bbaiJobState.start(progressTitle, normalized.length, source || 'generate-missing');
        }
    ...
    /**
     * Debug breadcrumbs for unified bulk generation (queue success → progress UI → inline/API job).
     *
     * @param {string} eventName
     * @param {Record<string, *>=} detail
     */
    function logGenerationFlowDebug(eventName, detail) {
        ...
        if (window.BBAI_LOG && typeof window.BBAI_LOG.info === 'function') {
            window.BBAI_LOG.info('[bbai-generation-flow]', payload);
        }
    }

    /**
     * Single entry: show bulk progress, then start inline/licensed generation. Call only after queue success.
     *
     * @param {Array<number|string>} idList Raw attachment IDs (normalized in startInlineGeneration).
     * @param {Object=} options
     * @param {string=} options.source Analytics / mode (e.g. generate-missing, bulk, bulk-regenerate, fix-all-issues).
     * @param {string=} options.entry Call site label for logs (e.g. dashboard_hero, handleGenerateMissing).
     * @param {string=} options.progressLabel Initial modal title (optional).
     * @param {*=} options.responseData Optional AJAX data for syncDashboardRuntimeStateFromBackendJob.
     * @param {number=} options.queued Server queued count for logs.
     * @return {boolean} False when ids or bulk config unusable.
     */
    function startGenerationFlow(idList, options) {
        options = options || {};
        var source = options.source || 'bulk';
        var entry = options.entry || '';
        var len = idList && idList.length ? idList.length : 0;

        logGenerationFlowDebug('bulk_queue_success', {
            idsCount: len,
            queued: options.queued !== undefined && options.queued !== null ? options.queued : len,
            source: source,
            entry: entry
        });

        if (!idList || !idList.length || !hasBulkConfig) {
            logGenerationFlowDebug('progress_start_aborted', {
                reason: !hasBulkConfig ? 'no_bulk_config' : 'empty_ids',
                source: source,
                entry: entry
            });
            return false;
        ...
        if (options.responseData) {
            syncDashboardRuntimeStateFromBackendJob(options.responseData);
        }

        var label = options.progressLabel || __('Processing your images', 'beepbeep-ai-alt-text-generator');
        var count = idList.length;

        showBulkProgress(label, count, 0);

        if (source === 'fix-all-issues' && typeof setBulkProgressHelperText === 'function') {
            setBulkProgressHelperText(getBulkOptimizationProcessingLabel(count));
        }

        logGenerationFlowDebug('progress_ui_started', {
            idsCount: count,
            source: source,
            entry: entry
        });

        startInlineGeneration(idList, source);

        logGenerationFlowDebug('inline_generation_started', {
            idsCount: count,
            source: source,
            entry: entry
        });
    ...
    function processInlineGenerationQueue(queue, batchSize) {
        ...
        function processNext() {
            ...
            var generatingTitle = sprintf(
                __('Fixing image %1$d of %2$d…', 'beepbeep-ai-alt-text-generator'),
                ordinal,
                total
            );
    ...
    function finalizeInlineGeneration(successes, failures, skipped, quotaError) {
        ...
        // Completion UI order:
        // - append final log rows
        // - progress to 100%
        // - set complete mode + title/buttons
        appendBulkProgressLogEntry(
            $modal,
            quotaError || failures > 0 ? 'error' : 'success',
            quotaError || failures > 0
                ? __('Generation finished with errors', 'beepbeep-ai-alt-text-generator')
        ...
        var state = syncBulkProgressState($modal, {
            ...
            complete: true,
            activeTitle: __('Generation complete', 'beepbeep-ai-alt-text-generator'),
        ...
        if (quotaError || failures > 0) {
            nextRuntimeState = 'generation_failed';
        } else if (successes > 0) {
        ...
        var _resultStatus;
        if (quotaError) {
            _resultStatus = failures > 0 ? 'error' : (successes > 0 ? 'partial' : 'no_changes');
        } else if (failures > 0 && successes === 0) {
            _resultStatus = 'error';
        } else if (failures > 0) {
            _resultStatus = 'partial';
        } else if (successes === 0) {
            _resultStatus = 'no_changes';
        } else {
        ...
        window.bbaiGenerationResult = {
            ...
            failed:    failures,
            skipped:   skipped || 0,
            unchanged: Math.max(0, total - successes - failures - (skipped || 0))
        };

        // Mark the modal as complete so minimizeBulkProgress() knows to reload the SSR dashboard.
        if ($modal.length) {
            $modal.data('bbaiComplete', true);
        }

        // Generation finished (success, partial, or failure) — re-enable CTAs.
        setGenerationInProgress(false);
        document.dispatchEvent(new CustomEvent('bbai:generation:finished', {
            detail: {
                source: source,
                successes: successes,
                failures: failures,
        ...
        // Post-generation retention loop: fetch fresh counts once and update the success UI without reload.
        (function postGenerationSuccessFlow() {
            if (!$modal.length || quotaError) {
                return;
            }
            var runId = String(Date.now());
            window.bbaiCurrentGenerationRunId = window.bbaiCurrentGenerationRunId || runId;
            var authMode = getDashboardAuthMode();
            var root = getDashboardRootNode && typeof getDashboardRootNode === 'function' ? getDashboardRootNode() : null;
            var trialExhausted = !!(root && root.getAttribute('data-bbai-trial-exhausted') === '1');
            var creditsRemaining = root ? parseInt(root.getAttribute('data-bbai-credits-remaining') || '', 10) : NaN;
            creditsRemaining = isFinite(creditsRemaining) ? Math.max(0, creditsRemaining) : null;

            function applySnapshot() {
                var counts = readPostGenerationCountsSnapshot();
                var snap = {
                    generated_count: Math.max(0, parseInt(successes, 10) || 0),
                    missing_count: counts.missing,
                    ready_for_review_count: counts.ready_for_review,
                    optimized_count: counts.optimized,
                    auth_mode: authMode || '',
                    trial_exhausted: trialExhausted,
                    credits_remaining: creditsRemaining
                };

                var nextAction = computePostGenerationNextAction({
                    missing: snap.missing_count,
                    ready_for_review: snap.ready_for_review_count,
                    optimized: snap.optimized_count,
                    auth_mode: snap.auth_mode,
                    trial_exhausted: snap.trial_exhausted,
                    credits_remaining: snap.credits_remaining
                });

                snap.headline = nextAction.headline;
                snap.supporting = nextAction.supporting;
                snap.ctaConfig = { primary: nextAction.primary, secondary: nextAction.secondary };
                snap.next_action = nextAction && nextAction.primary ? String(nextAction.primary.action || '') : '';

                // Attach to modal state and rerender complete UI.
                var current = getBulkProgressState($modal);
                current.postGen = snap;
                syncBulkProgressState($modal, current);
                renderBulkProgressState($modal, current);

                // Tracking once per run.
                try {
                    window.bbaiTelemetrySeen = window.bbaiTelemetrySeen || new Set();
                    var baseKey = 'post_generation_success_shown:' + String(window.bbaiCurrentGenerationRunId || runId);
                    if (!window.bbaiTelemetrySeen.has(baseKey)) {
                        window.bbaiTelemetrySeen.add(baseKey);
                        dispatchAnalyticsEvent('post_generation_success_shown', {
                            generated_count: snap.generated_count,
                            missing_count: snap.missing_count,
                            ready_for_review_count: snap.ready_for_review_count,
                            auth_mode: snap.auth_mode,
                            trial_exhausted: snap.trial_exhausted,
                            credits_remaining: snap.credits_remaining
                        });
                    }
                    var naKey = 'next_action_shown:' + String(window.bbaiCurrentGenerationRunId || runId);
                    if (!window.bbaiTelemetrySeen.has(naKey)) {
                        window.bbaiTelemetrySeen.add(naKey);
                        dispatchAnalyticsEvent('next_action_shown', {
        ...
        if (
            !quotaError &&
            successes > 0 &&
            typeof window.bbaiRefreshLoggedInDashboardTruth === 'function' &&
            !getLibraryWorkspaceRoot() &&
            document.querySelector('[data-bbai-li-hero="1"]')
        ) {
            window.bbaiRefreshLoggedInDashboardTruth('inline_generation').catch(function () {});
        }
        ...
        if (
            ...
            // Previously we forced a full reload after success to refresh SSR.
            // This caused “X doesn’t close” reports when reloads are blocked/slow.
        ...
        var finishFeedback = function() {
            ...
            dispatchDashboardFeedback(
                ...
                feedbackPayload.message,
                {
                    ...
                    skipToast: true,
                    detail: feedbackPayload.detail,
                    markSuccess: !!feedbackPayload.markSuccess
                }
            );
    ...
    function applyInlineGenerationTrialMetaToUi(data) {
        ...
        // Guest trial completion: immediately transition the guest funnel into the exhausted state.
        // Without this, the dashboard can remain in a transient "generation_complete" runtime state until refresh.
        if (exhausted || rem <= 0) {
            // Success confirmation (toast-style) without requiring a refresh.
            notifyLibraryFeedback(
                'success',
                __('Done — your free ALT text has been generated.', 'beepbeep-ai-alt-text-generator')
            );

            // Clear transient runtime state so the baseState (logged_out_trial_exhausted) can render immediately.
            if (typeof setDashboardRuntimeState === 'function') {
                setDashboardRuntimeState('idle');
            }
            // Re-apply once the runtime state is stable.
            window.setTimeout(function () {
                try { applyDashboardState(false); } catch (e) {}
            }, 180);

            // Locked preview copy: keep preview locked but clarify results are ready.
            try {
                var ctx = document.querySelector('[data-bbai-guest-preview-context]');
                if (ctx) {
                    ctx.textContent = __('Your generated ALT text is ready. Create a free account to review and manage it.', 'beepbeep-ai-alt-text-generator');
                }
            } catch (e2) {}
     ...
     * @param {string} label
     * @param {number} total
     * @param {number=} unusedInitialCompleted Legacy third argument (ignored); progress always starts at 0.
     */
    function showBulkProgress(label, total, unusedInitialCompleted) {
        ...
        $modal.removeData('bbaiBulkLastCompletedForLiveFeed');
        $modal.removeData('bbaiBulkLastProcessingForLiveFeed');
        ...
        // Ensure the live feed is never empty on first paint.
        appendBulkProgressLogEntry($modal, 'success', __('Image list ready', 'beepbeep-ai-alt-text-generator'), { dedupe: true });
        if (state.total > 0) {
            appendBulkProgressLogEntry(
                $modal,
                'processing',
                sprintf(__('Processing image %1$s of %2$s…', 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(1), formatDashboardNumber(state.total)),
                { dedupe: true }
            );
        }
    ...
    function formatBulkProgressLogDoneLabel(state) {
        ...
        return sprintf(
            __('%1$s of %2$s done', 'beepbeep-ai-alt-text-generator'),
            formatDashboardNumber(current),
            formatDashboardNumber(total)
        );
    }

    function resetBulkProgressLog($modal) {
        var $log;
        ...
        $modal.removeData('bbaiBulkLogSeen');
        $log = $modal.find('[data-bbai-bulk-progress-log]');
        if (!$log.length) {
            return;
        }
    ...
    function appendBulkProgressLogEntry($modal, type, message, options) {
        var $log;
        var seen;
        var key;
        var tone;
        ...
        entryHtml =
            ...
            '    <span class="bbai-bulk-progress__log-icon" aria-hidden="true">' + escapeHtml(icon) + '</span>' +
            '    <span class="bbai-bulk-progress__log-text">' + escapeHtml(message) + '</span>' +
            '    <span class="bbai-bulk-progress__log-time">' + escapeHtml(timestamp) + '</span>' +
            '</div>';
    ...
    function createBulkProgressModal() {
        var modalHtml =
            ...
            '                <h2 class="bbai-bulk-progress__title">' + escapeHtml(getBulkProgressDefaultTitle('generate-missing')) + '</h2>' +
            ...
            '                </div>' +
            '            </div>' +
            '            <p class="bbai-bulk-progress__status-note" data-bbai-bulk-progress-note hidden></p>' +
            '            <div class="bbai-modal-footer bbai-bulk-progress__running-actions" data-bbai-bulk-progress-running-actions>' +
            '                <button type="button" class="bbai-btn bbai-btn-primary bbai-bulk-progress__action bbai-bulk-progress__action--primary" data-bbai-bulk-progress-background>' + escapeHtml(__('Run in background', 'beepbeep-ai-alt-text-generator')) + '</button>' +
            '            </div>' +
            '            <div class="bbai-bulk-progress__complete" data-bbai-bulk-progress-complete hidden>' +
            '                <div class="bbai-bulk-progress__success-check" data-bbai-bulk-progress-success-check aria-hidden="true">' +
            '                    <span class="bbai-bulk-progress__success-checkmark">✓</span>' +
            '                </div>' +
            '                <h3 class="bbai-bulk-progress__complete-title" data-bbai-bulk-progress-complete-title></h3>' +
            '                <p class="bbai-bulk-progress__complete-subtitle" data-bbai-bulk-progress-complete-subtitle></p>' +
            '                <div class="bbai-bulk-progress__complete-actions">' +
            '                    <button type="button" class="bbai-btn bbai-btn-primary bbai-bulk-progress__action bbai-bulk-progress__action--primary" data-bbai-bulk-progress-primary></button>' +
            '                    <button type="button" class="bbai-btn bbai-btn-secondary bbai-bulk-progress__action bbai-bulk-progress__action--secondary" data-bbai-bulk-progress-secondary hidden></button>' +
            '                </div>' +
        ...
        $modal.on('click', '[data-bbai-bulk-progress-background]', function() {
            minimizeBulkProgress('run_background');
        });

        $modal.on('click', '[data-bbai-bulk-progress-stop]', function() {
            updateBulkProgressStatusNote(
                $modal,
                __('Generation is already running and will continue in the background.', 'beepbeep-ai-alt-text-generator'),
                'warning'
            );
        });
        ...
        $modal.on('click', '[data-bbai-bulk-progress-primary], [data-bbai-bulk-progress-secondary]', function() {
            ...
            handleBulkProgressCompletionAction(actionConfig);
        });
    ...
    function updateBulkProgress(current, total, imageTitle, progressOptions) {
        ...
        if (!tot) {
            tot = Math.max(0, parseInt(bulkState.total, 10) || 0);
        }
        ...
        if (completed > lastCompleted) {
            for (var i = lastCompleted + 1; i <= completed; i++) {
                appendBulkProgressLogEntry(
                    $modal,
                    'success',
                    sprintf(__('Image %s processed', 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(i)),
                    { dedupe: true }
                );
            }
        ...
        if (!isBulkProgressCompleteState(displayState) && nextProcessing > 0 && nextProcessing <= tot) {
            if (parseInt($modal.data('bbaiBulkLastProcessingForLiveFeed'), 10) !== nextProcessing) {
                appendBulkProgressLogEntry(
                    $modal,
                    'processing',
                    sprintf(__('Processing image %1$s of %2$s…', 'beepbeep-ai-alt-text-generator'), formatDashboardNumber(nextProcessing), formatDashboardNumber(tot)),
                    { dedupe: true }
                );
                $modal.data('bbaiBulkLastProcessingForLiveFeed', nextProcessing);
            }
        }
    ...
    function logBulkProgressError(errorMessage) {
        ...
        appendBulkProgressLogEntry(
            $modal,
            'error',
            errorMessage || __('An image could not be processed.', 'beepbeep-ai-alt-text-generator')
        );
        updateBulkProgressStatusNote(
            $modal,
            __('Some images may still need a manual review.', 'beepbeep-ai-alt-text-generator'),
            'warning'
        );
    }
    ...
    function logBulkProgressSuccess(successMessage) {
        ...
        appendBulkProgressLogEntry(
            $modal,
            'success',
            successMessage || __('Generation step completed.', 'beepbeep-ai-alt-text-generator'),
            { dedupe: true }
        );
    }
    ...
    function minimizeBulkProgress(minimizeVia) {
        ...
        // If generation has finished, dismiss the modal immediately.
        // We avoid forced reloads here because reloads can be blocked/slow and feel like the close button is broken.
        if ($modal.length && !!$modal.data('bbaiComplete')) {
            var t = $modal.data('bbaiDashboardReloadTimer');
            if (t) {
                window.clearTimeout(t);
            }
            $modal.removeData('bbaiDashboardReloadTimer');
            $modal.removeData('bbaiComplete');
            $modal.removeClass('active');
            clearBodyScrollLocks();
            ensureBbaiDashboardMainVisible();
            if (window.bbaiJobState) {
    ...
    window.bbaiBeginDashboardGenerateMissingProgress = function(idList) {
        ...
        return startGenerationFlow(idList, { source: 'generate-missing', entry: 'compat_bbaiBegin' });
    };
    ...
    window.bbaiApproveAllNeedsReview = runDashboardApproveAll;
    ...
    function handleDelegatedAdminClick(e) {
        ...
        // Prevent duplicate generation job starts while a run is active.
        var genAction = String(trigger.getAttribute('data-action') || '').toLowerCase();
        var genBbaiAction = String(trigger.getAttribute('data-bbai-action') || '').toLowerCase();
        var isGenerationAction = (
            genAction === 'generate-missing' ||
            genAction === 'generate-selected' ||
            genAction === 'regenerate-all' ||
            genAction === 'regenerate-selected' ||
            genAction === 'regenerate-single' ||
            genAction === 'phase17-improve-alt' ||
            genBbaiAction === 'generate_missing' ||
            genBbaiAction === 'reoptimize_all'
        );
        if (isGenerationAction) {
            if (bbaiIsGenerationLocked && bbaiIsGenerationLocked()) {
                if (e) {
        ...
        if (action === 'show-auth-modal' || action === 'show-dashboard-auth') {
            ...
            // Trial-complete conversion CTAs (click tracking).
            try {
                var trialCta = trigger.getAttribute('data-bbai-trial-complete-cta');
                if (trialCta) {
                    var dashRoot = document.querySelector('[data-bbai-dashboard-root="1"]');
        ...
        if (action === 'approve-all') {
            runDashboardApproveAll(trigger, e);
            return;
        }
        ...
        // data-action: legacy dashboard/library; data-bbai-action: logged-in hero (hyphen/underscore pair).
        if (action === 'generate-missing' || (!action && bbaiAction === 'generate_missing')) {
            if (e && typeof e.preventDefault === 'function') {
                e.preventDefault();
            }
            if (isOutOfCreditsFromUsage() || isLockedBulkControl(trigger)) {
    ...
    function initDashboardOnce() {
        ...
        document.addEventListener('keydown', function(ev) {
            ...
            var el = ev.target;
            if (!el || !el.getAttribute || el.getAttribute('data-action') !== 'show-upgrade-modal') {
                return;
            }
            if (el.getAttribute('role') !== 'link' && !el.classList.contains('bbai-header-free-credits-upgrade-line')) {
                return;
            }
            ev.preventDefault();
        ...
        document.addEventListener('bbai:credits-low-threshold', function() {
            try {
                if (sessionStorage.getItem('bbai_upgrade_modal_low_credits_once') === '1') {
                    return;
                }
            } catch (errRead) {
                return;
            }
            if (typeof hasVisibleLimitModal === 'function' && hasVisibleLimitModal()) {
                return;
            }
            try {
                sessionStorage.setItem('bbai_upgrade_modal_low_credits_once', '1');
            } catch (errWrite) {}
            openUpgradeModalCoalesced('low_credits_threshold', { source: 'credits_remaining_10_or_fewer' });
        }, false);
        document.addEventListener('click', function(ev) {
            var t = ev.target && typeof ev.target.closest === 'function'
                ? ev.target.closest('[data-action="generate-missing"], [data-bbai-action="generate_missing"]')
                : null;
            if (!t || t.getAttribute('data-bbai-locked-cta') === '1') {
                return;
            }
            if (t.getAttribute('aria-disabled') === 'true' || t.hasAttribute('disabled')) {
                return;
            }
            if (!isSoftUpgradeNudgeEligible()) {
                return;
            }
            if (bbaiGenerateMissingUpgradeNudgeTimer) {
                window.clearTimeout(bbaiGenerateMissingUpgradeNudgeTimer);
                bbaiGenerateMissingUpgradeNudgeTimer = null;
            }
            bbaiGenerateMissingUpgradeNudgeTimer = window.setTimeout(function() {
                bbaiGenerateMissingUpgradeNudgeTimer = null;
                if (isBulkProgressModalActive()) {
                    return;
                }
                if (typeof hasVisibleLimitModal === 'function' && hasVisibleLimitModal()) {
                    return;
                }
                openUpgradeModalCoalesced('generate_missing_click', { source: 'generate_missing' });
            }, 2400);
        }, true);
        ...
        updateLibrarySelectionState();
        syncSharedUsageBanner(null);
        var hasLoggedInTruth = document.querySelector('[data-bbai-li-state-truth-url]');
        var dashboardMain = document.getElementById('bbai-dashboard-main');
        if (dashboardMain && !hasLoggedInTruth) {
            if (typeof window.requestAnimationFrame === 'function') {
                window.requestAnimationFrame(function() {
                    window.requestAnimationFrame(function() {
                        markDashboardMainHydrated();
                    });
                });
            } else {
                window.setTimeout(function() {
        ...
        } else if (hasLoggedInTruth) {
            window.setTimeout(function() {
                var root = document.getElementById('bbai-dashboard-main');
                if (
                    root &&
                    root.getAttribute('data-hydrated') === 'false' &&
                    root.getAttribute('data-bbai-startup-truth-resolved') === '1'
                ) {
                    markDashboardMainHydrated();
                }
            }, 4000);
        ...
        $(document)
            ...
            .on('bbai:usage-updated.bbaiDashboardState bbai:stats-updated.bbaiDashboardState', function(event) {
                ...
                syncLibraryMissingBulkBar();
                var uSnap = getUsageSnapshot(null);
                if (uSnap) {
                    updateGlobalCreditsSurfacesFromUsage(uSnap);
                }
            });
...
(function($) {
    ...
    function bbaiApproveRowTransition(row) {
        ...
        var activeFilterBtn = document.querySelector(
            '#bbai-review-filter-tabs button[data-filter].bbai-filter-group__item--active,' +
            '#bbai-review-filter-tabs .bbai-filter-group__item--active[data-filter]'
        );
    ...
    function bbaiCheckLibraryEmptyAfterRemoval() {
        ...
        if (remaining.length === 0) {
            ...
            if (weakFilterBtn) {
                weakFilterBtn.classList.remove('bbai-filter-group__item--attention');
            }
        }
    }
